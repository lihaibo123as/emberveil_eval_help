...
| **1.71.2** | 🧙 条件归组 + 配置窗整理 + 📂 模版外置 | 施法族条件改名并统一归入「技能状态」组（施法中/施法时间/施法剩余时间 ‖ 目标施法中/目标施法时间/目标施法剩余时间）；配置窗底部**删掉与开关重复的 [接收] 按钮**、隐藏「开关」标题、[分享] 加**指引 tooltip**；案例模版库拆到 `examples/` 按职业分文件；★修掉一批**静默失败**缺陷（光环判定失败即报错而非当「没有」、条件导出丢失、生命条 1px 阴影、测试桩共享位置） |
