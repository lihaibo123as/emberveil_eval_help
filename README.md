...
| **1.71.15** | 🖱 图标库右键 = 设为小地图按钮图标 | 自动挑的按钮图标用户不满意（「图标调整的不对」）→ 她的图标让她自己挑：图标库里**右键**任意一枚即设为小地图按钮图标（存配置、重登不丢、tooltip 有提示），**亲自挑的永远优先**于自动挑；左键仍是打印路径；`/eh go mbicon [reset]` 查看/清掉自定义；★断言走真实右键 OnClick，变异 3/3 全捕获 |
| **1.71.14** | 🧹 全局页「一键宏」组底部对齐（修与「状态信息UI」重叠） | 「界面」组加两行子开关后，「一键宏」组头与「状态信息UI」直接叠上（用户截图红圈）→ 整组改**从底部按钮行向上锚**：去抖行下缘与导航/关闭按钮行上缘留 6px，组内行距不变；★断言读**真实控件**位置（不重叠 / 底部对齐 / 行距 / 不出窗口），变异 3/3 全捕获 |
| **1.71.13** | 🖼 小地图配置按钮改图标钮 + 💬 tooltip 美化 | 按钮 = 一枚宏图标（运行时从宏图标表**按名字优先级**挑：扳手 > 工程学 > 齿轮，不写死路径），26×26 与图标库单格同边长、图标占满整钮、无边框；挑不到自动退回旧「金框 + EH」（不留空白按钮）；tooltip 改三行功能描述（**全职业一键宏 / 常用工具箱 / 世界数据库检索**，三语言）；顺带新探针 `/eh go bind`（快捷键可行性验证） |
