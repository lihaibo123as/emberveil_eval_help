...
> EmberVeil（1.12.1 / Lua 5.1）职业操作辅助插件：宏正文一行 `/run EVAL_GO()`，**26 种条件**规则引擎驱动——技能/条件/方案全部可配，不限职业。

| 模块 | 入口 | 一览 |
| :-- | :-- | :-- |
| 🗡️ **通用一键宏** | 宏 `/run EVAL_GO()` | 动作条任意技能可入方案；多方案 ≤4，可绑不同按键直触 |
...
| **1.32.0** | 🧰 技能五级分类 | 行为/技能/宠物/选目标/物品，二级下拉一键喝药换装 |
