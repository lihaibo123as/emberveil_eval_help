...
> EmberVeil（1.12.1 / Lua 5.1）**全职业施法辅助插件**：宏正文一行 `/run EVAL_GO()`，规则引擎驱动——**技能五分类**（角色行为/角色技能/宠物行为/目标选取/物品使用）× **49 种条件类型**（分组下拉）× 多方案绑定按键，全部可视化配置，不限职业。另附两个内嵌页：**工具箱**（商人 / 社交 / 任务自动化）与**数据检索**（任务 / 物品 / 怪物·NPC + 世界地图标注层，依赖 UnrealQuest）。
...
| 🧰 **工具箱** | 配置窗 Tab3 | 商人助手（自动修理 / 售卖灰色 / 按名购买·丢弃）+ 队伍社交（就位自动确认 / 屏蔽公会上下线）+ 任务自动交接（默认 Shift 按住临时暂停）+ 任务通知频道 |
...
![配置窗口·一键宏设置页](preview/cfg1.png)

**技能编辑窗**（点 [编] 或技能图标打开）：条件逐行独立配置——条件类型/参数全部下拉选择（49 种条件类型，按 自身/目标/光环/技能 分组），关系列切 &（组内全过）/ |（组间任一），实时预览
