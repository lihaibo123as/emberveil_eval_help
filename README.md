...
**数据检索（Tab4）**：任务 / 物品 / 生物·NPC / 对象 四类检索，输入即搜（前 10 项匹配）→ 点条目看详情（掉落来源+掉率、容器、售卖、关联任务、给予/结束任务的 NPC、物品描述），条目可无限级联下钻；物品行悬停显示游戏内物品 tooltip

![数据检索](preview/dataset.png)

**数据检索 · 地图定位与地图标注**：点条目行尾的地图图标 → 打开世界地图并切到该区域，标出这个实体的刷新点（同一实体同色，便于分辨）；控制行的「地图标注(N)」可勾选 16 类标注（采集物 / 服务设施），随地图切换实时显示
