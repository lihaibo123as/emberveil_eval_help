...
- **当前版本**：**1.71.9**（**已提交** `release: v1.71.9`（tag `v1.71.9`）——技能列表调序按钮 `^`/`v` → **真三角 ▲/▼** + 图标库一页 6 → **9 行**（162 枚/页，原 108）；★**尚未推送远端**；★用户已定：**以后自测全绿即可直接提交版本**）。
  - 上一版 **1.71.8**（案例模版窗改**分组分两列 + 组内同行自动换行**；已提交 + tag）；**1.71.7**（队伍/团队案例模版审计：一律「指定技能 + 队伍/团员条件」；已提交 + tag）；**1.71.6**（案例模版窗改流式自动换行；已提交 + tag）；**1.71.5**（补「一键队伍/团队驱散」+ 两列改「最优切点」；已提交 + tag）；**1.71.4**（模版窗两列改版 + 扩到 11 组 25 条 + 职业过滤支持候选者条件；已提交 + tag）；**1.71.3**（停止攻击 / 跟随 / 分享与接收重做 / 成员过滤（职业·小队）/ 图标库 Tab / 全角冒号静默失效修复 等一整批；已提交 + tag）。
...
1. **1.71.9** — 技能列表调序/滚动按钮 `^`/`v` → **真三角 ▲/▼**；图标库一页 **6 → 9 行**（162 枚/页，原 108；行数是先算清可用空间定的）。
2. **1.71.8** — 案例模版窗改**分组分两列 + 组内同行自动换行**（11 组 27 条；中文 660×240）。
3. **1.71.7** — **队伍/团队案例模版审计**：17 条模版改「指定技能 + 队伍/团员条件」。
4. **1.71.6** — 案例模版窗改**流式（自动换行）布局**（中文 660×180，5 行）。
5. **1.71.5** — 案例模版补「一键队伍驱散 / 一键团队驱散」（覆盖魔法 + 疾病）。
6. **1.71.4** — 案例模版窗两列改版 + 模版扩到 **11 组 25 条**；「职业过滤」支持候选者条件（解析/导出/UI 三处一起放开）。
7. **1.71.3** — 一整批：`停止攻击`（停读条+自动射击+魔杖+普攻）· `跟随`（`FollowByName`）· 分享删团队频道 · 接收端数据规则重做 · 成员过滤（职业多选/小队多选）· 第 5 个 Tab「图标库」· **全角冒号「：」静默失效修复** · 屏蔽频道进出信息。
8. **1.71.2** — 开关分组改横排 + 并入「接收方案」；修一批**静默失败**（空标题空白日志、`ipairs` 遇 nil、黑块、下拉行池静默截断、光环纹理优先级）。
9. **1.71.1** — **队友/团员扫描 + 四种状态条件**（两轮确认的需求）；`1.70.45/46/47` 三批在途改动随此版一次发布。
10. **1.71.0** — **方案分享**（`Share.lua` 独立载入；hex 分片 + 聊天事件接收 + 自绘导入弹窗）。
11. **1.70.47** — 队伍/团员选取落地 + **官方文档逐条核对**（F3 节）；修「多字节字符进字符集 / 全角冒号」的静默失效。
12. **1.70.46** — UnrealQuest 依赖的 **5 状态探测** + 友好引导面板（依赖缺席时收起交互控件）。
13. **1.70.45** — 配置窗加宽到中文 660 / 西文 800（`cfWinWidth()` **单一来源**，技能编辑窗同宽）。
14. **1.70.44** — 定案「点定位后地图空白」= 同一变量两个绑定（全局 × 局部）。
15. **1.70.43** — 猎人案例模版；修「导出→导入」**静默丢弃**（11 个取反条件无法解析 + 无条件技能行被丢）。
16. **1.70.42** — 自愈语义收敛：只在「本该有钉子却一个都没显示」时才重建（不盲目重建）。
17. **1.70.41** — 「类型: 全部」按钮整颗不显示（layout 常量 local 作用域，第 10 次）；实体色改为 **id 派生的确定性哈希**（不能每次现摇随机）。
18. **1.70.40** — **删掉**「定时强刷 1.5s / 慢速兜底 5s / 稳定期闸门 0.8s」三件套 → **签名一变立刻重绘**（用户选 A 方案）。
19. **1.70.39** — 根因定案：tick 帧挂 `UIParent`，而全屏世界地图会隐藏 UIParent → OnUpdate 停摆；改挂 **`WorldFrame`**（3D 视口，永不被 UI 隐藏）。
20. **1.70.38** — 地图诊断浮层 `/eh ds hud`（把状态画在画面上，替代反复猜日志）；**诚实放置**（`PositionWorldMapPin` 失败就不 Show、不算成功）。
