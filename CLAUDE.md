...
- 当前版本：**1.34.0**（i18n P0，**在 i18n 分支**：EVAL_LOCALES 注册制+EVAL_L 查找（zhCN 基准回退/缺键显示键名/pcall format）；EVAL_SET_LANG 白名单+cfg.lang+新语言提示/reload；ehResolveLang=cfg.lang→GetLocale→GetClientLocale(ru-RU/en/zh-Hans)→zhCN；配置窗标题栏右上国旗行 CN/EN/RU（拷 UnrealQuest tga 进 media/Flags/+ASCII徽标兜底/透明度选中态/EVAL_HELP_CFGWIN 全局桥给 80 行的 LANG_REFRESH 用——local 声明在 1928 看不到）；P0 只翻配置窗外壳+帮助，数据层（条件词/技能名/方案文本）永不翻译；待游戏内验证俄文渲染后 P1 全量）｜master **1.33.4**（/eh go add 路径 8 技能上限残留清除——1.33.0 漏网第三处；README 简介/场景/速览表全面同步五分类+28条件+物品使用）｜**1.33.3**（热修：1.33.0 引入 ROWS 作用域报错——ROWS 是 cfgBuild 内 local，EVAL_WAR_TAB_REFRESH 拿不到（函数作用域陷阱又一实例）；改用 table.getn(warUI.rows) 行池实长）｜**1.33.2**（探针破案根治：本客户端 GameTooltip 的 SetPlayerBuff/SetUnitBuff/SetUnitDebuff/SetAction 返回值恒 nil 但 tooltip 实际已填充——`if oks and built` 门槛把读名卡死；三处光环读名+wactionName 全部拆 built 门槛只留 oks。教训：本客户端 tooltip widget 方法返回值不可信，读文本就好）｜**1.33.1**（/eh go probe buff 探针：GPB/UnitBuff 枚举+SetPlayerBuff/SetUnitBuff 读名+BuffButton0/BuffFrame 存在性原始值打印到聊天框+UELog，诊断药品 buff 不进下拉——待用户回报探针输出再定修复）｜**1.33.0**（方案技能 8 上限全取消：SE保存/导入解析两处检查已删；配置窗技能列表滚动——warUI.offset+^/v 按钮（仅超高显示）+OnMouseWheel（arg1 方向全局，pcall EnableMouseWheel 兜底）；行操作 chk/上/编/删 全部改 ri+offset 索引；删「/eh wdebug 或 /eh debug」旧提示）｜**1.32.10**（药品类 buff 修复：UPDATE_STATE 的 playerBuffs 并入 UnitBuff(\"player\",i) 1基索引枚举兜底——GetPlayerBuff 过滤列表可能漏药品食物；EVAL_PLAYER_BUFF_LIST 名称全空时换 SetUnitBuff 读名路径；wiki 已确认 UnitBuff/SetUnitBuff 存在）｜**1.32.9**（sDrop 光环下拉重构：并行 names 表替代序号边界；剔除 pet/targetSel/item 伪技能；已学习名 ◇ 并入（debuffTex 跨会话，buff 消失可选）；auraTexOf 配图标配图标；EVAL_WAR_TAB_REFRESH 入口补扫自愈——init 重扫早于动作条就绪时图标全灰的修复）｜**1.32.8**（IO 窗 EditBox 装进 ScrollFrame 精确裁剪——SetScrollChild+UpdateScrollChildRect+SetVerticalScroll 自动滚底，本客户端 EditBox 不裁剪自身高度会溢出可视框；ScrollFrame 创建失败回退旧直挂布局）｜**1.32.7**（配置窗全局页帮助区重整：删猛击Alt战士残留，补技能五分类/Shift+按宏切方案/重扫按钮/导入导出现行信息）｜**1.32.6**（petIconOf：有宠物时 GetPetActionInfo(1..10) 学习指令图标——命令槽返回 token 非中文名，PET_CMD 加 token 字段双索引；持久化 cfg.war.petIcons 跨会话；回退 GetPetIcon 家族图标→问号；注意走 EVAL_HELP_CONFIG 全局因为该处 local cfg 未声明）｜**1.32.5**（条件类型下拉分组美化：SE_TYPE_GROUPS 四类（自身/目标/... (line truncated to 2000 chars)
