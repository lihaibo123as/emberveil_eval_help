...
- ★★★1.73.8 **底部行要与 [关闭]「同一行」→ 几何必须从生产的单一来源取，不能各写一份**（用户：「分页信息这行移动到底部和关闭同行.对齐关闭,别重叠」）：图标库的状态行 + [上页][下页][重扫] 从顶部挪到底部，判据 = ①底部行中线 == 关闭的中线（`EVAL_IB_TEST_BOTTOM().midY == EVAL_TEST_CFG_LAYOUT().closeMidY`）②按钮组右端 `<=` 关闭左边缘 − 4 ③状态文字右端 `<=` 按钮组左端 − 2 ④状态行在**整个网格下面**。★`CLOSE_MIDY/CLOSE_LEFT` 是 EvalHelp 的 local，别的文件读不到 → 新增**生产读值口 `EVAL_HELP_CFG_BOTTOM`**（不是测试专用口）；写死 -439 在 660 宽下**照样绿**，所以专门做了「写死 + 把生产关闭位置上移」的对照变异（M90）来钉「它真的在读单一来源」。变异 M86~M90 全捕获。★1.73.9 **工具箱照同一条来**（用户：「工具箱滚动样式参考图标库那边的滚动方式,然后放置在底部和关闭同行.右对齐关闭旁边」）：撤掉右上 ▲ 与列表下方 ▼+计数 → `[上翻][下翻]` 文字按钮（`TB_UP/TB_DN` 三语言）+ 计数文字同排左侧；**凡「与关闭同行」的控件都从这一个读值口取几何**，判据同上一组（中线一致 / 右端不压关闭 / 文字不压按钮 / 整行在列表下面 / 不再是 ▲▼ 字形 / 点真实 OnClick 仍能滚）；变异 M91~M95 全捕获。
