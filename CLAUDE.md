...
- ★★★1.72.3 **分享分片必须限频发送**（用户实测：「长方案对方有时候接收不到」，短方案 1 片从不出问题）。
  ...
  【修法】第 1 片立即发（手感不变），其余进 FIFO 由 OnUpdate 每 `SH_SEND_RATE=0.35s` 滴一片，队列上限 `SH_MAX_QUEUE=200`（超出如实取消）。
...
- ★★★1.72.3 **收不齐（在途传输超时）绝不静默**：`shSweepBuf` 原来到点直接 `SH.buf[k] = nil` ——
  接收方只看到「什么都没发生」，发送方也永远不知道（用户报的「对方有时候接收不到」正是这个静默态）。
  修法：超时丢弃时 `shWarn` 如实报「谁发的 + 收了几片 / 共几片」并请对方重发（提示本身仍走 5s 限频，且限的是「说不说」不是「丢不丢」）。
  ★判据 = 组 106（点名发送者 + 报 `n-1/n` + **迟到的那片不再凑成一笔**）；变异 M8（退回静默）→ 当场炸。
