...
- 当前版本：**1.32.1**（新增条件类型「目标存在」hasTarget bool——文本 目标存在/有目标/无目标）｜**1.32.0**（技能五级分类：角色行为/角色技能/宠物行为/目标选取/物品使用——SE 技能名二级下拉；选取目标由条件提为技能级 targetSelOf（rule.skill="选取目标:xxx"，原条件类型 hidden 兼容存量）；物品使用 itemOf+wFindBagItem 背包扫描+UseContainerItem（消耗品用/装备穿，官方文档无保护），就绪走 GetContainerItemCooldown；wuse/wready/wicon/RULE_RUN/亮金五处统一豁免范式；审计修复：1.27.0 cfg.war.debuffTex 未创建致学习不持久化（ENSURE_PROFILES 补建）、1.31.0 层数反向 op 语义反转降级、1.26.0 DD multi sel 兜底）｜此前 **1.31.0**（debuff 层数条件：UnitDebuff 第二返回值层数入 st.targetDebuffs[tex]=数字（非堆叠归一1，旧布尔兼容 cnt==true→1）；hasDebuff cd.n=最小层数、noDebuff cd.n=视为无的上限(cnt<n)；文本 有debuff:名>=N/无debuff:名<N（>N→>=N+1）；解析用 auraStack 后缀匹配；编辑窗 skill 行加 row.stk 层下拉 x266 仅 debuff 类显示）｜1.30.0（特殊技能「宠物指令」：rule.skill=宠物:攻击 等 8 种（攻击/跟随/停留/停止攻击/被动/防御/主动姿态/解散——刻意不含 PetAbandon 放弃/改名/兽栏系）；PET_CMD 表+petCmdOf 解析放 wtex 后（作用域顺序）；EVAL_RULE_RUN/wuse 豁免 wslots 检查直调 Pet API+HasPetUI 无宠跳过；wicon 图标兜底 GetPetIcon/问号（原四处直接 s.tex 已改）；战斗信息UI亮金放行 petCmdOf；EVAL_GO_SKILL_CHOICES 末尾追加宠物段）｜1.29.0（选取目标扩展：targetTarget=TargetUnit("targettarget")、byName=TargetByName(cd.nm)——TARGET_SEL 表加 arg/needsName 字段，byName 未设名规则不触发；EVAL_NEARBY_ENEMY_NAMES 循环枚举最近5敌名（TargetNearestEnemy 边切边收+TargetByName 还原原目标，遇重复名即停止=循环完）；EVAL_TN_OPEN 通用回声行输入弹窗（frameLevel 130>SE100）；sDrop 对 byName 改弹名称下拉，选完种类立即续弹）｜1.28.0（新增条件类型「连击点数」：GetComboPoints() 无参 0-5 非贼德恒 0；数值比较型 {k="combo",op,n}，st.combo 入状态表；文本 `连击>=3`；/eh st 能量行贼德显示连击；编辑窗 26 种）｜1.27.0（光环类条件下拉实时项：EVAL_TARGET_DEBUFF_LIST/EVAL_PLAYER_BUFF_LIST 用 GameTooltip SetUnitDebuff/SetPlayerBuff 读名，名称→纹理 learnAuraTex 学进 cfg.war.debuffTex（未加载落 EVAL_DEBUFF_TEX_LEARN 兜底），texOf→auraTexOf 学习表回退——非动作条光环可匹配；sDrop 用序号边界区分实时项，不做多字节前缀字符串解析；DD 行池 24→48）｜1.26.0（新增条件类型「目标职业」：{k="tClass",cs={WARRIOR=true,...}} 多选=或关系，UPDATE_STATE 存 st.tClass=UnitClass 英文 token；EVAL_DD_OPEN 加 opts.multi 多选模式——点按切换 √金标 不关面板，onPick(序号,选中态)；seGroupsToLinear 对表字段深拷一层防污染；文本格式 `目标职业:战士/法师`，顿号/中文逗号须先 gsub 成 / 再切——多字节字符直接进 [^] 字符类会按字节误切汉字）｜此前 1.25.0（新增条件类型「选取目标」：TargetNearestEnemy/nearFriend/nearParty/nearRaid/lastEnemy/lastTarget/clear 七种下拉——副作用... (line truncated to 2000 chars)
