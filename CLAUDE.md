...
> ★★★**字节预算限制已取消（用户 2026-09-17）**：原先 DSH 的「工作区指令预算 65,536 字节」会在超限时把本文件**从尾部截断**（并打印 `Workspace instruction budget ...`）；现已在 **`~/.dsh/profiles/web/cordis.patch.yml`**（user patch 层，最后生效）把 `@deepseek-ai/dsh-agent-instructions` 的 `maxBytes` 改成 **1048576（1 MiB）**。
> 　⇒ **不必再为了字节数把内容往外搬**；但**仍然只放「必须随时可见的铁律与判据」**——每多 1 KB 都是每次请求的固定上下文成本（踩坑经过 / 版本叙事 / 截图佐证照旧进参考卷附录 R）。
...
- `/eh cfg` 配置窗 660×420（中文）/ 800×420（西文），**六个 Tab**：① 全局（日志/界面/帮助）② 一键宏设置（方案栏 + 技能列表 + `[添加技能]` + `[导入导出]`）③ 工具箱（`Toolbox.lua`；含**猎人助手 → 一键喂食**等助手）④ 数据检索（`DataSearch.lua`）⑤ **图标库**（`IconBrowser.lua`）⑥ **抓宠帮手**（`PetHelper.lua`+`PetData.lua`）。
...
- 分享行形态（1.73.43）：只有「一段色码 + 链接」能画（色码无链接 / 无色码 / 7 位码 = 整条被吞）⇒ 色码打头 + 带链接 + 每条一段 8 位码 + 间隔 ≥1s；色清单从 `SH_CHUNK_COLOR`/`SH_SEAL_TIERS`/`SH_TITLE_COLORS`/`SH_TITLE_CUSTOM_COLOR` 现取；组 144/158/159。
...
- ★★★**「测试文件存在但没人跑」是头号静默失效**（1.74.34）：harness 漏加载一个 `tests/tools/*.lua` ⇒ 整组断言消失、套件照样 ALL TESTS PASS；源码检查漏 require 同理。⇒ 用**跑到底握手**（文件末行 `EVAL_TEST_MOD_DONE("模块")`）+ **文件数对账** + `TOOL TEST FILES CHECK`（共用 `EVAL_TEST_EQ` / 握手 / PASS 行 / 每个 `tests/checks/*.js` 都被 require+调用 / `tests/` 不进 toc 与发布清单）钉住；★模块测试文件还要有**组号清单**检查（`DF GROUP ROSTER CHECK`：组号集合 + 升序 + 每组都有真实 `eq(` 与 PASS 收尾行）——「整组被删 / 被掏空」只有它抓得住；变异 `tmp/mut_tests.js` 10/10（其中「加载器静默跳过」只有握手对账能抓）。
...
- 分享/封皮：`EVAL_SHARE_SEAL_INFO` · `EVAL_SHARE_SEAL_TIER_RGB` · `EVAL_SHARE_DISPATCH` · `SH_POP_GOT_NAME`；头衔/彩蛋：`EVAL_TITLE_STATE` · `EVAL_TITLE_REFRESH` · `EVAL_TITLE_SET_CUSTOM`（只认第一次）。
- 聊天/名字缓存：`tbNcHarvest` · `tbNcAt` · `tbNameClassEnsureRoster` · `EVAL_TB_NAMECLASS_PROBE`；工具箱：`tbBtn` · `tbMenuBuild` · `tbMigrateQuest` · `EVAL_TB_PAINT_ROW`。
