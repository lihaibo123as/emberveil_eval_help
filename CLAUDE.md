...
- ★★★1.73.51 **名字染色缓存「什么时候载入」**（用户真机：「插件刚载入 → 染色失败；即使玩家查询完成还是失败；
  **只有打开公会信息才开始染**」）—— 先把载入点列全（这是本轮分析的结论，别再靠猜）：
  ...
  · ★★★**1.73.52 真机探针结果（用户跑 `/eh go 名字缓存`，一测定案）**：所有来源 API **都在**（有），但
    **公会名册 本地条数=0 · 查询结果=0 · 好友=0**，缓存只有 **2** 条（自己 + 目标）⇒ **懒加载被证实**：
    不开公会窗时，公会聊天里的人名**任何免费来源里都没有**（这就是「只有打开公会信息才开始染」的全部原因）。
    ★另一条重要事实：探针当时只打了 **AddMessage 那一层**的计数（经过 0 条 / 上色 0 条）—— 而本客户端那层**本来就是死的**
    （1.73.12 实测写入被吞），真正干活的是 `ChatFrame_OnEvent` 那层 ⇒ **探针已同时打两层**（「诊断必须逐个入口分开计数」的又一次实据：
    只看一层会把「另一层在正常干活」误判成「入口全断」）。
  · **决定（用户要求「缓存应该能支持染色」）**：加**一次登录一发**的 `GuildRoster()`（`tbNameClassEnsureRoster`，唯一入口），
    四道闸门：①「主动查询」开关关着就一个都不发（＝退回老行为：等玩家自己开公会窗）②不在公会里不发
    ③本地名册已在手上（`GetNumGuildMembers() > 0`）不发 ④**本会话只发一次**（`TB.rosterAsked`）；
    结果由**已有的** `GUILD_ROSTER_UPDATE → tbNcHarvest("guild")` 路径接（不另写一套采集）。
    ★源码检查 `CHAT COLOR WIRING CHECK` 相应更新：从「**一律禁止** GuildRoster()」改成「**只许出现在那一个函数体内 + 四道闸门必须齐全**」
    （变异 M487 当场报「名册查询缺少『主动查询开关』这道闸门」）。判据 = 组 161④⑤（只发一次 / 关开关不发 / 有本地名册不发 / 不在公会不发）。
    ★★**「冗余守卫」实录**：M486（删掉「只发一次」闸门）**一开始 SURVIVED** —— 被「本地已有名册就不发」那道闸门顶住了
    ⇒ 夹具改成「**名册一直不回来**（`GetNumGuildMembers` 恒 0）」后当场捕获（got 3 want 1）。
...
- ★★★1.73.43h **这个客户端会「整条吞掉」一条消息里**多段色码 / 多要素**的聊天消息**（真机变异测定案）：
  ...
  ★★1.73.43o 补（用户：「还是没显示玩家的**品级**」）：身份其实显示了、但**没有颜色**（当初为「一行只放一段色码」把它当纯文本并进主行）。
    ⇒ 按用户原则「**一行一行发**」恢复**A 身份行**（单独一条、**只带身份色**、色码打头、无链接、无方括号）：
      `|c<身份色><头衔>|r <名字> 分享了`；B 品阶行不再带身份尾巴。**两条依次排在分片之后**（A → B），各自 ≤250B 且有长度守卫。
    ★当初 A 行不显示是夹在**限流窗口**里（v3 那轮整体被限流，v4 同形态正常）——现在间隔 1 秒，判定改为「能用」。
  ★★1.73.43p 再分析（用户第三张截图：**同一次分享、1 秒间隔**的干净对照）：分片 ✓ 与 **B 行 ✓ 都显示**，
    **只有 A 行（一段色码 + 纯文本、无链接）没出现** ✗ ⇒ 与 B 逐项对比，唯一干净差别 = **有没有链接**：
    · **有链接 + 一段色码** → 显示（分片 ✓ B ✓ 变异测 v2 的 [1]/[4] ✓）；
    · **只有一段色码、没有链接** → **不显示**（A 行三次都是这个形态）✗；
    · 纯文本（无色码）→ 显示 ✓（v1 [1]/[2]/[3]、v4 [7]/[8]）；
    · 我们自家 `EVAL_HELP:` 行不带链接也有色 —— 但那走的是 **AddMessage**（另一条通道），**不能拿来当反例**。
    ⇒ **发色码消息一律带上链接**（借链接那个渲染通道）；A 行因此改成与 B **同款结构**：
      `|c<身份色>|HEHPF:<id> 0/1:0|h[<头衔>]|h|r <名字> 分享了`（= v2 [4] 那个已验证能画的形态）。
      B 的载荷序号改 `0/1:1`（避免两条完全同形）。★**1.73.50 定案**：B 行那份无色身份兜底**已删**
      （用户真机截图确认 A 行正常显示后：「分享信息：方案右侧的玩家角色可以不显示」）⇒ 身份**只在 A 行出现一次**，
      B 行 = `|c<品阶色>|HEHPF:<id> 0/1:1|h[<符号><品阶>秘籍·<名>]|h|r  <评语>`；判据 = 组 144①
      （从 A 行取出身份文字，断言 B 行链接之后**不含**它；变异 M480 捕获）。
  ...
  ★★★**色码发送规则（现已定稿，写清楚省得再试错）**：
    ...
    🧪 **实测工具（1.73.44，用户要求「将现在所使用的所有颜色色码都测试下」）**：`/eh go 色码测`（别名 `colortest` / `颜色测` / `palette`）
       —— 把**分享真正会发到聊天的每一个色码**各发一条**编号**消息（分片传输色 + 5 品阶 + 5 头衔 + 彩蛋专属色 = **12 条**），
       每条 = **色码打头 + 8 位 + 只一段色码 + 带链接**（= 上面三条规则的可画形态），间隔 1 秒走同一条限频队列；用户只需回报「哪些编号出现了」。
       ★清单**从色表现取**（`SH_CHUNK_COLOR` / `SH_SEAL_TIERS` / `SH_TITLE_COLORS` / `SH_TITLE_CUSTOM_COLOR`）——**不许写死**，
       写死就是「改了色表却漏测」（不报错、只是漏）；判据 = 组 158（覆盖/8 位/单段/带链接/编号/队列/落盘证人）
       + 源码检查 **`SHARE PALETTE CHECK`**（色表逐项 8 位 + 命令必须引用四张表 + 命令内不许出现字面色码 + 入口接线）；变异 M444~M449 全捕获。
       ★跑完请**等 ≥30 秒**再跑第二次（短时间多条会被吞，结果不可信）。
    🎨 **彩蛋名号色（1.73.45，用户：「彩蛋头衔 颜色设置霸气一点」）**：默认 **淡金 fffff2c8 → 烈焰赤 ffff2400**（用户从候选里选定；帝王金等 4 个仍可选），
       并给**候选表 + 命令**：`/eh go 名号色`（候选**逐条编号预览**，形态与分享行同款）· `/eh go 名号色 <n>`（当场选用 + 存档 `cfg.title.customColor`，立刻生效、重登仍在）。
       ★判据（源码检查 `SHARE PALETTE CHECK` 扩展 + 组 159）：① 候选**全部 8 位**；② **默认 = 候选表第 1 项**（单一来源，改表即改默认，永不漂移）；
       ③ 候选与品阶/头衔/分片色**两两不撞**（同屏分不清就白做 —— 现在 16 个在用色码全局去重）；④ 读值口 `shTitleCustomColor()` **只认 8 位**存档色
       （手改成 6 位 → 退回默认，**绝不照发会被吞的码**）；⑤ 非法编号**如实 nil 且不改档**（不假成功）。变异 M450~M458 全捕获。
       ★★**又一次弱判据（写下来省得再犯）**：接线检查原写「**含** `go 名号色`」—— 被同一分支那句 `string.find(msg, "^go 名号色%s")` **顶住**，
       M457 实测 **SURVIVED**（命令明明断了线却报绿）⇒ 正解 = 断言**精确分支写法** `msg == "go 名号色"`；M457/M458（色码测同款变体）复验全中。
       ★★**1.73.44 实测结论（用户真机跑过，我对截图做了逐像素取样核对）**（★色码本身没问题；档色后来改过：源代码档现为亮蓝 00bfff）：12 个在用色码在「色码 + 链接」形态下
         **全部正常显示、颜色全对**（9ad4ff / ffffff / 1eff00 / a335ee / ff8000 / **00bfff** / 8fa8b8 / 00e5ff / 4f9bff / ff4fd8 / 5cffd2 / fff2c8）；
         ★1.73.47 用户又改一档：**源代码档 = 亮蓝 00bfff**（原暗金 b87333 太暗）—— 上面那次实测的是旧色，改色后请重跑 `/eh go 色码测`（★后续 1.73.60 又改成 **神级 / 深红 c00000** —— 同样要重跑）。
         ⇒ **色码本身从来不是问题**（也反证了「头衔里有特殊字符」这条假设）。之前「身份看不到 / 品级没颜色」的真因是**消息结构**：
         **一段色码但不带链接的整条不画**（A 行三次干净对照）· **链接打头且无色码的整条不画**（v4 [6]）·
         **纯文本与「色码 + 链接」都正常** ⇒ 以后再排「分享行不显示」，**先看有没有链接**，别先往色表 / 色码长度 / 头衔字符上赖。
...
### 5.4 UI 与布局
- ★★1.73.67 **删品阶行的重复文字**（用户接着 1.73.66 的截图：「重复了」「删除红线划着的那个下面的那个」）：
  1.73.66 把方案名加进摘要行后，品阶行（图标 + `[品阶秘籍·名]`）变成了重复 —— 用户要**删掉下面那行**。
  · 删法（最小且忠实）：品阶行**只留图标、删掉重复的文字**（sealHead 清空 + Hide）；布局上移收紧不留空档
    （SH_SEAL_ROW_Y -50→-46 · SH_DETAIL_Y1 -74→-66 · dPanel 顶 -46→-42）。
  · ★**没有连图标一起删**：`EVAL_SHARE_SEAL_ROW`（品阶/评分/颜色/图标的验算读值口）被组 145 等十几处用，
    整行连图标删会拆散一堆判据、且方案名/品阶色的展示验证反而没了着落 → 只删重复的文字，图标保留。
  · 判据 = 组 145（摘要行带名字 · 品阶行文字已删但图标仍在 · 解析不出方案仍整行隐藏 · 几何密封性跟随新坐标）；
    变异 M562（文字又显示）/ M563（连图标也删）捕获。
  · ★**「删一个重复的 UI 行」也可能缠着一堆判据** —— 先盘点它的读值口被多少断言用，再决定删到哪一层；
    删多了拆散判据，删少了用户嫌没删干净。
- ★★1.73.66 **分享弹窗摘要行带上品阶色方案名**（用户截图圈出摘要行的「方案」+ 划出下面品阶行的 [品阶秘籍·名]：「按照图片内的要求替换」）：
  摘要行从「X 分享的方案（N 个技能）」→「X 分享的 **[品阶秘籍·名]**（N 个技能）」，名字带品阶色（**直接复用品阶行的 head**，
  那正是图里下划线的那段）。★这是对 1.73.46「名字只留一处（品阶行）」的**反转** —— 用户改主意了，两处都显示。
  三语言模板 `SH_POP_GOT_NAME` 改成带名字占位符；判据 = 组 145（标题行带 [品阶秘籍·名] + 带品阶色 + 不再有笼统「方案」+ 品阶行不变）；
  变异 M560/M561（退回笼统方案 / 剥掉色码）捕获。
  ★**「用户截图圈 A 划 B = 把 A 换成 B」**是 UI 文案调整的最常见来源；改完要**同步更新旧判据**（1.73.46 那组
  「名字只留一处」的断言必须跟着反转，不然它红 —— 本轮就是先红了才改的）。
- ★★★1.73.65 **反应句里的方案名 = 可点的品阶色链接**（用户：「能否将盗贼替换成上面的有颜色的文字.点击能同时打开方案分享接收页面」）：
  · 做法：`shReactionNameLink(p)` 把方案名造回 B 行**同款结构**：`<色>|HEHPF:<传输id> 0/1:1|h[<符号><品阶>秘籍·<名>]|h|r`，
    嵌在反应句里（text + 链接 + text —— 变异测 [4] 已验证能画的形态；一条消息**恰一段色码**）。
  · ★**点它为什么能打开**：`EVAL_SHARE_CLICK_OPEN("EHPF:<id>")` 按**传输 id 后缀**在 `SH.recent` 里找方案 ——
    而 `SH.recent` 是**收齐时** `shRecentPut(发送者#id, …)` 存的（那批分片的 id 就是反应句里的 `p.idh`）。
    ⇒ 反应句里的 id **必须等于收到方案时的那个传输 id**（用 `SH.pending.idh`，**不是新造一个**）。
  · ★**长度守卫**：带上链接后整条 > `SH_MSG_MAX` → **退回纯名字**（绝不做丢整条 —— 「超长会被客户端吞」的老账）；
    缺传输 id / 算不出品阶 → 也退回纯名字。判据特意用 **60 字方案名**逼守卫**真执行**
    （不逼的话守卫永远走不到 —— 变异 M559「去掉守卫」因此**先存活后捕获**，正是这类「判据没踩到」的盲区）。
  · 判据 = 组 82⑤b（反应句**真的是链接** · 链接带**本次传输 id** · **点它真的重开接收页**）+ ⑦c（长名字退回纯名字但名字还在）+
    组 82/170 期望串按**同款链接**重建（不再按纯名字拼）+ 源码检查 `SH FUN CHECK` 扩展（链接函数 / id 链接 / 被调用 / 超长守卫）；
    变异 M556~M559 全捕获。
  · ★**判据里嵌链接的期望串**：测试侧 `reactLinkOf(p)` 与生产 `shReactionNameLink` 走**同一套 EVAL_SHARE_SEAL_*** 算 ——
    链接是普通字符串拼接、两边一致；**这类断言别再按纯名字拼期望串**（纯名字那对不上带链接的输出）。
- ★★★1.73.64 **彩蛋「角色扮演反应」**（用户：「根据当前秘籍等级 和接收者自身的头衔等级 做出导入/忽略的角色扮演反应会话」
  + 「说/队伍 加入」）：
  · 机制：[导入]/[忽略] 两个按钮 → 按**接收者头衔档（EVAL_TITLE_STATE().tier）× 收到的秘籍品阶档（收到的文本现算）**
    从 50 格（5×5）里**抽奖式**挑一句发到**来源频道**（公会/说/队伍 —— 原来只有公会，键 SH_FUN_CHAN）。
    语气矩阵：秘籍高2档+=看天书 · 高1档=惊讶虚心 · 同级=切磋互不相欠 · 低1档=傲娇 · 低2档+=不屑/自大。
  · ★★**文案用嵌套表，不是 500 个扁平键**：SH_FUN_IMP / SH_FUN_IGN 每语言**各一个键**，值 = 头衔档 → 秘籍档 → 10 条。
    关键前提 = **L() 能返回表**（无参时直接回 t[key]，表格照给）⇒ 两键就够，LANG KEY CHECK 也只查这两个键。
    每条**恰 2 个 %s**（发送者、方案名，**顺序不能错**）—— 单占位符会把发送者填到方案位（生成器当场拦下 14 处，
    比如 `Skip the %s` 会把发送者名填成「Skip the 彩虹」）。
  · ★★**判据夹具的时间点**（本轮实测的真坑）：EVAL_SHARE_SEAL_INFO（造分享行）会调 EVAL_TITLE_REFRESH，
    而我临时放神级方案进库 → 头衔被「只升不降」锁到 5 ⇒ **测「低头衔」时必须在点击那一刻才钉 title.tier**；
    在发送前设会被重算掉（探针实测 titleTier 从 1 变回 5）。多片方案的夹具还要 EVAL_TEST_SHARE_DRAIN()
    抽干限频队列（第 1 片立即发、其余排队，不抽干收不齐）。
  · 判据 = 组 82（三频道都发 · 命中 10 条候选之一 · 不在公会不发 · 反应不替代导入）+ 组 170（三语言 50 格×10 条
    齐全 · 每条恰 2 占位符 · 低头衔收神级命中 [1][5] 端到端）+ 源码检查 `SH FUN CHECK`（三频道 / 两路 op / 抽奖式 / 两表）；
    变异 M549~M554 全捕获。
  · ★**test_engine.js 里每个 CHECK 都是 IIFE、有的还嵌套** —— 追加新 CHECK 别锚在某个 CHECK 的 console.log 行后面
    （会把新块插进那个 IIFE **内部**）；要锚在外层 IIFE 的**收尾 `})();`** 之后。本轮就嵌进了 TITLE GACHA，
    看懂「两层 `})();`」才明白为什么 node --check 当时不报错（嵌套合法，只是不该这么写）。
- ★★★1.73.63 **彩蛋「创世者亲临」的触发闸门**（用户：「排查下 之前的彩蛋是如何触发的. 调整彩蛋触发规则…」）：
  · **以前的触发方式（排查结论，先把事实记下）**：彩蛋**没有任何游戏条件** —— 只有一条命令 `/eh go 创世`
    （或 `/eh go 创世 <名号>` 直接落笔）。源码原话：「触发机制以后再接（写方案达到一定机制）；现在先给命令手动触发」
    ⇒ 任何人任何时候敲那条命令就能开；唯一限制是**用过一次就不再给**（EVAL_TITLE_SET_CUSTOM 把守）。
  · **新规则（用户定，三条件同时成立才触发）**：① 自己**手动创建**的方案里有一个达到**神级**（评分 ≥25）；
    ② 手动创建的方案 **≥3 个**；③ **头衔满档**（抽卡档位 5）。不满足时**如实说还差什么**（绝不静默拒绝）。
  · ★★**「手动创建」怎么判**（本轮新概念）：方案表加 `src` 字段 —— 手工插入点（配置窗 `[+]`、`/eh go newprof`）写 "manual"；
    **凡从文本解析出来的**（导入文本 / 案例模版 / 分享接收 / examples）在**唯一解析出口** EVAL_PROFILE_FROM_TEXT
    盖 "text"（**不去每个入口分别打标** —— 那样迟早漏一处，本项目「两份状态」的老账）。
    **老存档没有 src ⇒ 按手动算**（不能让老玩家升级一次就永远够不到自己的彩蛋）。
  · **闸门必须堵两条路**：EVAL_TITLE_CREATOR_OPEN（窗口）与 EVAL_TITLE_CREATOR_TRY（`/eh go 创世 <名号>` 的命令后路）——
    只堵窗口 = 留后门（源码检查 `CREATOR GATE CHECK` + 组 169④ 各验一次）。
  · **自动触发** EVAL_TITLE_EGG_CHECK()：挂在 EVAL_WAR_TAB_REFRESH（方案/技能任何改动的汇聚点）+ 登录；
    达标且未领过 → 播报 + 非战斗时弹窗；**只播报一次**（cfg.title.creatorOffered），否则以后每次编辑技能都刷屏。
  · **一次机会照旧**：EVAL_TITLE_SET_CUSTOM 只认第一次；窗口/命令再开一律如实拒绝（组 150⑥ 守着）。
  · 判据 = 组 169（三条件**各条单独不满足都要关门** · 导入来的不算手动 · 老存档按手动 · 窗口与后路都堵 ·
    自动触发一次且**接在生产路径上**）+ 组 150（闸门满足后的窗口行为）+ 源码检查 `CREATOR GATE CHECK`；变异 M542~M548 全捕获。
  · ★**真机取证**（本轮顺手做的，很值）：用 fengari 加载**真实存档**（`%LOCALAPPDATA%…\SavedVariables\EvalHelp.lua`，
    ★**带 BOM** ⇒ loadbuffer 前要切掉前 3 字节）再调 `EVAL_TITLE_CREATOR_GATE()` —— 一次摊开每个方案的 name/score/tier/src。
    实测该存档：11 个方案、其中 **3 个已是神级**（武器战 38 / 骑士一键 25 / 武器战 30）、头衔 5 档、`title.custom = "阿凡提"`
    ⇒ **三条件早就够，但机缘已经用掉了**（所以看不见）；要重测得先 `/eh go 重置名号` 或 `/eh go 重置头衔`。
- ★★★1.73.62 **标题栏顺序对调 + 新增「当前方案名」**（用户：「战斗信息 标题 品阶和头衔 位置对调. 描述调整 头衔 品阶.方案名称」）：
  三个文字从左到右定为 **头衔 → 品阶 → 方案名**（都挂在玩家名右边，依次锚**前一个的右边缘**）。
  · ★改在**公用件** uiTitleBadgesMake 里 ⇒ 战斗信息UI 与状态信息UI **一起变**（1.73.59 要的就是「两窗口同一套布局」，
    所以这种「改某一个窗口」的指令落到共用件上是对的；反过来若**只想**改一个窗口，就得先拆开共用件 ——
    别默默只改一处，那会把 1.73.59 的「同一套」破坏掉）。
  · 方案名颜色 = **该方案的品阶色**（uiProfileTierRGB，与紧挨着的品阶徽标同源）；没有方案 → 如实清空 + 中性灰。
  · 读值口：EVAL_TEST_UI_TITLEBAR / EVAL_TEST_ST_TITLEBAR 都多交 plan / planRelTo / planColor 等（**读真控件**）。
  · 判据 = 组 164（顺序链 relTo：头衔←玩家名 · 品阶←头衔 · 方案名←品阶 · 方案名 = **当前激活**方案 ·
    换方案后跟着换 · 颜色 = 品阶色）+ 组 168（两窗口逐字相同）+ 源码检查 UI TITLE BADGES CHECK（三处 SetPoint 链）；
    变异 M537~M541 全捕获（顺序退回 / 不建方案名格 / 方案名写死第 1 个 / 颜色写死 / 名字写死）。
- ★★★1.73.61 **品阶阈值重排：25 分以上才算神级**（用户：「品阶计算 25 以上算神级，其他依次调整下」）：
  · 旧的四个档位各 3 分宽（≤3/6/9/12），神级门槛只比绝版高 1 分（13）—— 现在按**同一宽度**重排：
    **≤6 普通 / 7-12 稀有 / 13-18 珍稀 / 19-24 绝版 / ≥25 神级**（每档 6 分，24 封顶）。
  · 只改 SH_SEAL_TIERS[].max 四个数（+ 样例代表分）—— 该表是**唯一来源**，方案列表/模版/封皮/徽标/方案行五处自动跟。
  · ★**判据改成「每个边界的两侧都钉」**（6/7 · 12/13 · 18/19 · 24/25，另加 1 与 999）：原来只钉上沿，
    于是门槛偏一格（24→25）照样绿 —— 变异 M533/M534/M536 现在都当场响，M532（整表退回旧阈值）也捕获。
  · ★★**改阈值 = 所有按分数写的夹具都要跟着走**（本轮踩了一串，全靠测试自己报出来）：
    ① 代表分数组 `{ 1, 4, 7, 10, 13 }` → `{ 3, 9, 15, 21, 26 }`（**6 处**）；
    ② 造「神级方案」的 `txt(13)` / `txt145(14)` / `prof151(14)` 之类 → 必须 ≥25；
    ③ 头衔抽卡夹具要「普通 + 稀有」两档 → `prof147(2), prof147(9)`（5 分在旧表里是稀有，新表里是普通）；
    ④ 组 164/167/168 的 `mk(n)`（n 条技能各 1 条件 = 2n 分）→ 重挑成跨三档的 **4 / 14 / 28**；
    ⑤ `EVAL_SHARE_SEAL_DEMO` 的 `reps`（秘籍样例每档代表分）漏改 ⇒ 样例串档（变异 M535 捕获）。
  · ★教训：**「分数」是本项目的硬编码耦合点** —— 阈值一动，凡是写死分数的夹具（含样例命令）都要重挑；
    测试会当场报出来，但**要按档位含义重挑**，别只把数字凑到能过（那样会把「跨三档」悄悄降成「跨两档」）。
- ★★★1.73.60 **品阶 5 改名 + 改色：「源代码」→「神级」· 亮蓝 00bfff → 深红 c00000**（用户：「方案品阶: 源代码
  调整->神级 颜色深红」）：
  · 只改**一处**就全生效：档位文案走 Locales（SEAL_TIER_5）、色码走 SH_SEAL_TIERS[].color ——
    方案列表 / 案例模版 / 分享封皮 / 标题栏徽标 / HUD 方案行**五处共用它**（1.73.58 抽的读值口这一轮直接受益）。
  · 三语言一起改：zhCN 神级 / enUS Divine / ruRU Божественный（★只改中文 = 另两个语种孤悬）。
  · ★★**深红不能与已有的红撞车**：名号候选里已经有 烈焰赤 ff2400 / 龙血赤 dc143c / 暗血赤 b22222 ⇒
    取 c00000（比暗血赤亮、比龙血赤深，8 位码不重复）；源码检查 SHARE PALETTE CHECK 守着「在用色码两两不许相同」
    （变异 M530 把色改成 b22222 → 当场报「色码撞车（同屏会分不清）」）。
  · 判据 = 组 146（中文评语/品阶逐字 + **三语言品阶名绝对值** + 色码留在源码）；变异 M528~M531 全捕获。
  · ★★**「自己比自己」的判据是假绿**（M531 一开始存活）：③ 那几条拿 EVAL_SHARE_SEAL_TIER(13).name 去比
    EVAL_LOCALES[lg].SEAL_TIER_5 —— 两边**同源**，英文词打错了照样绿 ⇒ 品阶名必须另写**绝对值夹具**
    （tiersAll146：三语种 × 5 档），这才叫钉住定稿。
  · ★**评语是用户定稿，改名只换那一个词**：SEAL_C5_2 我顺手顺手改成「此乃**神级之法**…」→ 组 146 的逐字夹具
    （wantZh146[5]）当场报长度 372/366 不符 ⇒ 改回「此乃**神级**，改一个字符都会天崩地裂。」。
    **别在用户定稿的中文句子上顺手润色**。
- ★★★1.73.59 **状态信息UI 标题栏 = 名称 + 档位 + 头衔（与战斗信息UI 同一份实现）**（用户：「状态信息名称和头衔
  参考战斗信息做相同的布局」）：
  · 做法 = **徽标登记表**（uiTitleBadgeSets，按**窗口键**登记：重建时覆盖同一个键，不会越积越多）+ 公用件
    uiTitleBadgesMake(key, parent, z, titleFs) + 一份刷新实现 uiTitleBadgeRefresh()（pairs 遍历全部登记项）。
    两个窗口只负责「把自己那一对登记进来」⇒「名字 → [品阶] → 头衔」这条规矩**只有一份实现**。
  · ★每个窗口**各自**在自己的 BUILD + tick 里调刷新（两个都开着时谁调都行；★**只开一个窗口时就必须靠自己那条**）
    —— 判据正是照这个盲区设计的：组 168⑤「**关掉战斗信息UI**、只留状态信息UI」再改方案库，ST 徽标也必须跟着变
    （M526 删掉 ST 那处调用**一开始存活** —— 因为 HUD 的 tick 替它刷了；改成单窗口场景后当场捕获）。
  · ★★**pcall 的返回值数错一格**（本轮实踩，组 164 当场报 got=nil）：EVAL_SHARE_SEAL_TIER 返回
    **档位序号 + 品阶表**两个值 ⇒ pcall 之后是 ok, 序号, 品阶表；抽公用件时写成 local okT, tier = pcall(...)
    拿到的是**序号**（number）→ type(tier) ~= "table" 恒成立 → 整个徽标恒为 nil。
    **凡 pcall 一个多返回值函数，先数清它返回几个**（本项目老账的又一次）。
  · 判据 = 组 168（标题贴左 · 两个徽标都在且 relTo 分别是「标题文字 / 档位徽标」且 relPoint == RIGHT ·
    两窗口文案与颜色**逐项相同** · 改方案库两边同时换档 · **只开 ST 时也跟**）+ 源码检查 UI TITLE BADGES CHECK
    扩展（公用件存在 + 两个窗口都走它 + ST 标题贴左 + 刷新 ≥4 处）；变异 M519~M527 全捕获。
- ★★★1.73.58 **战斗信息UI 方案行「按品阶染色」+ 品阶判定抽成唯一读值口**（用户：「方案根据品阶染色」）：
  · uiProfileTierRGB(prof) → rgb, tierIdx, score（**算不出如实 nil**）；uiProfBtnPaint(pb, prof, sel) 用它上色：
    文字 = 品阶色、底色 = 品阶色 ×（激活 0.45 / 否则 0.22）—— 与**配置窗方案列表 / 案例模版**同一套规则；
    配置窗那处也改成走 uiProfileTierRGB（原来自己内联了一份判定 —— 同一规则两处实现的老账）。
  · ★建按钮时**当场**上色 + tick 每 0.15s 跟着刷（EVAL_PROFILE_SCORE 只做 table.getn 级累加，无 API 调用）；
    算不出品阶 → **如实退回**原来的暗金底 + 暖色字。
  · 判据 = 组 167（文字色 = 品阶色 · 底色 = 品阶色 × k（激活/非激活两档）· 算不出退回暗金 · 换激活后跟着换）；
    组 160（配置窗方案列表）仍绿。
  · ★★**只比一个通道 = 假绿**：④ 一开始只比 bgRGB[1]，而暗金激活底的 R 恰好也是 0.45 ⇒ M519（tick 不按品阶
    染色）**存活**；改成**三个通道一起比**后当场捕获。**颜色类断言一律三通道**，别给「巧合同值」留缝。
- ★★★1.73.57 **标题栏「玩家头衔」+ 色码解析口抽成唯一一个**（用户：「显示玩家的头衔」）：
  头衔徽标接在**档位徽标**右边（锚前一个控件的右边缘 ⇒ 顺序 玩家名 → [◆品阶] → 头衔，全自动跟随、无需量宽）。
  · ★单一来源：文案与颜色都取自 EVAL_TITLE_CURRENT()（彩蛋自定义**优先**、抽卡头衔兜底、**没入档如实 nil**）——
    UI 侧**不自己判一次档位/卡池**（那是 Share.lua 的规矩）；**没有头衔就如实清空 + 收回中性灰**。
  · ★★**色码 → RGB 抽成唯一解析口 EVAL_COLOR_RGB**（8 位 |cAARRGGBB，恰好 10 字节、不切 alpha）：
    原来只有 EVAL_SHARE_SEAL_TIER_RGB 一份实现，头衔色再加一份就是「同一规则两处实现」（本项目老账）；
    现在品阶色 / 头衔色共用它，并由源码检查钉住「Share.lua 里 string.sub(code, 5, 6) 只许出现 1 次」。
  · 判据 = 组 166（显示抽到的头衔 + 颜色 = 头衔自己的色码 · 锚 relTo == **档位徽标**且 relPoint == RIGHT ·
    彩蛋自定义在下一个心跳压过抽卡结果且用专属名号色 · 没有头衔 → 如实清空 + 中性灰）+ 源码检查
    UI TITLE BADGES CHECK（1.73.55 的 UI TIER BADGE CHECK 扩成两个徽标：档位 + 头衔 + 单一色码解析口）；
    变异 M511~M518 全捕获。
  · ★**工具坑（本轮实踩）**：run_code 的字符串里**别出现反引号** —— harness 传过来时反引号会提前结束模板串，
    后面的 |c 被当代码解析（报 ReferenceError: c is not defined，因为它把 | 当位运算、c 当标识符）。
    ⇒ 要在生成的文件里放反引号，就改写成数组 join 或单引号拼接。
- ★★★1.73.56 **「数据刷新」不许顺手改可见性**（用户真机 bug：「Tab 切到全局等非一键宏页之后，关闭重新打开，
  会把一键宏的技能列表页显示出来」）：
  根因 = EVAL_WAR_TAB_REFRESH() 是**数据刷新**，却顺带 Show/Hide 自己那页的控件（技能行的 8 件套 / 方案 [删] 钮 /
  方案品阶图标 / 滚动条）—— 而它**不只被「切到本 Tab」调用**：打开配置窗时被**无条件**调一次（EVAL_HELP_CFG_TOGGLE）、
  方案管理弹窗应用改名时也调（EVAL_PM_APPLY）⇒ 停在别的 Tab 时这些控件被重新 Show 出来，**盖在当前那一页上**
  （用户截图：全局页上叠着一整套技能列表）。
  ★修法 = **可见性只由 Tab 切换负责**（EVAL_HELP_CFG_SETTAB 的控件清单，本项目的可见性契约）：
  刷新函数开头算一个 local warTabOn = (cfgWin.tab == 2)，**只在它成立时**才 Show/Hide —— 数据（文字 / 配色 /
  图标 / 品阶）照旧每次都刷（用户 1.73.49 的要求「打开配置窗就重算品阶、与落在哪个 Tab 无关」必须保住）。
  · 判据 = 组 165（切到别的 Tab 后技能行 / [删] 钮**读真控件 IsShown == false** · 此时刷新**仍然**刷新数据
    （品阶重算计数增加）· 关闭重开**停在原 Tab 且列表没叠出来** · 切回本 Tab 照旧显示）+ 读值口扩展
    EVAL_TEST_WAR_ROWS()（除文案外交出每行的 shown / profDel / tab）；变异 M506~M510 全捕获。
  · ★★**夹具也要站在用户的视角上**：修好之后老组 160（方案行品阶图标）立刻红了 —— 因为它读的是「Tab=2 时」
    的样子，却**没有先切到 Tab 2**（读到的是别的 Tab 留下的隐藏态）。⇒ 改可见性类问题之后，**凡断言某页控件的组，
    都要显式把自己那页切出来**（不是产品坏了，是夹具默认了「全局可见」这个错前提）。
- ★★★1.73.55 **HUD 标题栏中段的「方案档位」徽标**（用户截图圈出标题栏中段：「这位置增加显示玩家方案的档位」）：
  新增 uiProfileTierText()（现算当前**激活方案**的品阶）+ uiTitleTierRefresh()（刷新口）——
  徽标 = [<符号><品阶名>]（如 [★源代码]），位置 = tierFs:SetPoint("LEFT", title, "RIGHT", …)，
  即**锚在玩家名那条 FontString 的右边缘**（宽度随文字自适应 ⇒ 天然跟着名字走，**不用手工量宽** ——
  本项目的手工量宽是给「按名字排布的一排按钮」用的；而退化估算会把拉丁字母宽度算错、徽标就压到名字上）。
  · ★★**两条纪律**（都是本项目老账的再现）：① 档位**不缓存**，tick 里每 0.15s 现算
    （EVAL_PROFILE_SCORE 只做 table.getn 级别累加、不调任何客户端 API）——否则换了激活方案 / 改了方案内容之后，
    标题上挂的还是**上一档**的徽标（界面与真值自相矛盾）；② **算不出就如实清空文案 + 收回中性灰**：
    既不硬编一个档位，也**不沿用上一档的颜色**（那等于画出一个假档位）。
  · 符号 / 品阶名 / 色码全部取自 Share.lua 的那几个读值口（EVAL_SHARE_SEAL_SYMBOL / EVAL_SHARE_SEAL_TIER /
    EVAL_SHARE_SEAL_TIER_RGB）——**不另抄一张品阶表**（方案列表 / 案例模版 / 分享封皮 / 标题栏共用一张）。
  · 判据 = 组 164（BUILD 即算出 · 锚 relTo == 标题 FontString 且 relPoint == RIGHT · 文字色 = 品阶色 ·
    换激活方案 / **只改方案内容**都在下一个心跳换过来 · 拿掉 EVAL_PROFILE_SCORE 时**如实清空 + 中性灰** ·
    空方案表不打断 tick）+ 源码检查 **UI TIER BADGE CHECK**；变异 M497~M505 全捕获。
  · ★★**源码检查「取段」的教训**：这道检查一开始是**整个 EvalHelp.lua** 里找 EVAL_SHARE_SEAL_SYMBOL( 这类串 ——
    结果**别处**（分享 / 模版）的同类调用让它**假通过**（漏了 SYMBOL 还是绿的），而且徽标代码走的是
    pcall(名字, …) 形态、**根本不含那个括号**。正解 = **先把徽标那一段切出来**（uiProfileTierText →
    uiTitleToggle 之间）再逐项钉。与「检查前先摘注释」同族：**检查的作用域不对 = 假绿/假红**。
- ★★★1.73.54 **HUD 标题栏 = 标题左对齐 + 右侧两个快捷开关**（用户：「将上面两个开关（战斗区/方案区）在标题栏右侧
  ...
  **点一下 = 写同一份配置（`uiCfg().subCombat/subScheme`）+ 整帧重建**（窗口高度随之变化）+ 顺手刷新配置窗 ——
  ...
  `type(nm)=="string" and nm ~= ""`（只判 type 会把空串当名字写上去 —— 本项目反复踩的那条）。
  判据 = 组 162（UI/ST 两个窗口标题都 == 玩家名 · UnitName 给 nil **和空串**都退回窗口名 · 恢复后正常）+
  源码检查 **`UI TITLE NAME CHECK`**（两处必须都走 `uiTitleName(...)` + 不许再有 `title:SetText("字面量")`）；
  变异 M490（ST 标题写死）/ M491（取名函数恒退回）/ M492（空串没当取不到）全捕获。- ★★★1.73.48 **案例模版 / 方案列表：位置与配色「同一条规矩」两笔真 bug（用户真机截图定案）**：
  【用户原话】「方案标题没垂直居中。并且.右侧还有位置.但是标题却显示...」+「方案列表 的图标和配色方案 采用相同规则.」
  ① **没垂直居中**的根因 = 文字 FontString 上**两个锚点**：建控件时锚 `CENTER`，品阶分支又 `SetPoint("LEFT", …)`——
     真客户端里这是两个锚点（同一个控件被两头拽）→ 文字偏上几个像素；**旧测试桩只记最后一次锚点**，这个 bug 在测试里
     **完全不可见**（又一次「桩太宽松 → 断言失明」）。⇒ 修法：重锚前先 `ClearAllPoints`（**唯一锚点** = 真垂直居中）；
     桩补 `GetAnchorCount/GetAnchorNames`（同名覆盖、不同名累加、ClearAllPoints 清零）→ 判据「文字**只有一个**锚点 LEFT,y=0」当场抓（M470）。
  ② **右侧还有位置却出省略号**的根因 = 文字框宽度**正好等于**名字量宽（按钮宽 = 量宽 + 16 + 20，文字框 = 按钮宽 − 16 − 20）
     → 差一两像素就触发客户端省略号。⇒ 修法：按钮宽加 `TPL_MEASURE_SAFE=6`，文字框改成「按钮宽 − 图标位 − `TPL_TEXT_PAD=4`」
     （富余 18px）；判据 = 「文字框 ≥ 名字估算 + 10px **且** 按钮宽 − 文字框 ≥ 24」（M471/M472）。
     ★★**改量宽时立刻踩到老坑**：`rowsOf` 与「摆放」是**两处各算一遍按钮宽**，只改一处 → 自报行数 ≠ 真实占用行数，
     被**组 91 既有判据**当场抓住（`实测 32 vs 48`）⇒ 正解 = 抽成 `btnWof(name)` **单一来源**（两处共用）。
  ③ **方案列表「同一条规则」**（与案例模版同一套）：品阶图标（IconSem 真纹理 13px）+ 名称文字色 = **品阶色** +
     底色 = 品阶色 × 0.22（当前激活 × 0.45）；**算不出品阶**（老存档/坏数据/Share 未载入）→ 如实退回默认配色 + 图标隐藏
     （**不硬编一个品阶**）。判据 = 组 160 全读真控件（GetTexture/IsShown/GetTextColor/GetVertexColor）+ 读值口 `EVAL_TEST_WAR_PROF_ROWS()`；
     变异 M473~M476（不显示图标 / 不用品阶色 / 底色错 / 不回退）全捕获。
  ★★★**1.73.49 续（用户：「方案左侧的图片还没显示.品阶每次打开方案自动计算完善」）**：
  ④ **图标整块纯黑**的根因 = `EVAL_SHARE_SEAL_ICON` **返回两个值**（路径 + 文件名），而调用点把它**内联成实参**:
     `pcall(t.SetTexture, t, EVAL_SHARE_SEAL_ICON(i))` → Lua 把两个返回值**全展开** → 实际是 `SetTexture(路径, 文件名)`；
     本客户端的 `SetTexture` 第二参是 **wrap 模式**（现代签名）→ 拿到字符串 → **尺寸对、整块纯黑**（用户截图那个 13×13 黑方块）。
     ★模板窗那边写的是 `local tiPath = …` 再传 → 一直正常（**同一个函数、两种调用形态**，差别只在有没有先把返回值收进变量）。
     ⇒ 修法：只取第一个返回值 + 写完**读回来确认**（`GetTexture` 拿不到就记账）＋桩保真「**SetTexture 传了几个实参**」
     （旧桩只取第一参 → 「多传一个参数」在测试里完全不可见）。判据 = 组 160⑥「图标调用只传 1 个实参」；变异 M477b 捕获。
     ★教训推广：**凡函数返回多个值，别直接内联进可变参数的调用**（`pcall`/`SetPoint`/`...`）—— 收进变量再传。
  ⑤ **「品阶每次打开方案自动计算完善」**：`EVAL_WAR_TAB_REFRESH` 只在 Tab=2 时被 SETTAB 调到 ⇒ 打开配置窗时
     **显式再刷一次**方案列表（与落在哪个 Tab 无关），并加**重算计数** `EVAL_TEST_WAR_TIER_CALC_COUNT()`
     —— 「打开了就重算」这件事必须可断言（判据 = 组 160⑦：先把 Tab 切到 4 再开窗，计数仍要增加；变异 M478 捕获）。
  ⑥ **取证命令 `/eh go 方案图标`**（本项目「能做成命令就别让用户手工复现」）：把每行的**实际纹理 / 传参个数 / 是否显示**
     一次摊开 —— 真机上「没请求 / 请求了没生效 / 生效了没画出来」三种原因只能靠**读回来的纹理路径**分辨；
     接线由源码检查 `PROF ICON PROBE WIRING CHECK` 守着（变异 M479）。
  ★★**写判据夹具的两条纪律**（本轮都踩了）：① 夹具**走生产同一条路**（用 `EVAL_PROFILE_FROM_TEXT` 造方案）；
     手搓 `groups = { { 1 } }` 会被配置窗渲染行当成条件表 → `EVAL_COND_STR` 拿数字当场 `attempt to index a number value`；
     ② 断言要**只读目标那一行**（弹窗读值口第 1 条 = 标题），拼上详情行会被 `# 方案: 甲` 顶住（弱判据的老坑）。
...
- **当前版本（源码唯一真值）**：`EvalHelp.lua` 的 `local VERSION` == `EvalHelp.toc` 的 `## Version` = **1.74.0**
  ...
  ★★★**方案分享「隐藏载荷 + 信息行」（1.73.41 ~ 1.73.43n，分支 `probe/share-hide-link`）—— ✅ 已完成（用户真机实测通过）**：
    · **分片**（v2，载荷藏在自定义链接里）：`|cff9ad4ff|HEHPF:<id> <i>/<n>:<hex>|h[秘籍传输中...p%]|h|r`；
      ★1.73.47 用户：「方案:xxx 传输中 调整 -> 秘籍传输中...」⇒ 标签**不再带方案名**，只报进度；文案走
      `SH_CHUNK_LABEL`（三语言）——源码检查 **`SHARE LABEL I18N CHECK`**（必须 `L("SH_CHUNK_LABEL")` + 剥注释后不许有「传输中」字面量）守着。
      判据 = 组 144②（标签**逐字** == 语言包模板：只查「含前缀」会被「方案名缀在后面」骗过，M465 实测 SURVIVED）。
      同帧只发第 1 片，其余进队列**每 1 秒滴一片**（`SH_SEND_RATE=1.0`，原 0.5 秒会时好时坏）。
    · **分享信息行**（最后一步，排在所有分片之后，一段色码）：
      `|c<品阶色>|HEHPF:<id> 0/1:0|h[<符号><品阶>秘籍·<名>]|h|r  <评语>`（★1.73.50 起**不再带 `<头衔>` 尾巴**；身份只在 A 行一次）
      （`0/1:0` 让接收端按「i<1 越界」忽略 ⇒ 对收方案零影响；点它仍可直接导入）。
    · **接收端**：封皮行/身份/评语照旧解析（老格式也认）；分享弹窗**只留** `★[品阶秘籍·名]`（带品阶色、带品阶图标）。
      ★★★1.73.46 用户两条改版：① **点聊天里的分享链接 = 弹出「方案分享」窗让玩家自己确认，绝不自动导入**
      （旧行为是点一下直接写进方案库 = 一个没有确认的写操作；现在 [导入] 是唯一写入路径）；
      ② 弹窗里红框那两行（「品阶/身份」+「评语」）**整个删除**（原来是建出来再 Hide —— 那正是「看着还在」的来源），
      且**方案名只留一处**：标题行不再重复《方案名》，名字由下面那行带品阶色的 `★[品阶秘籍·名]` 承担
      （用户选定方案 B）；★解析不出品阶时标题**退回带名字的旧模板**（名字绝不能丢）；详情行上移 -96→-74、窗口 216→200。
      判据：组 144③（点链接真的弹窗 · **方案库条数不变**=没导入 · 如实播报「请自己确认」——「没导入」只认可观察的副作用）、
      组 145③④⑥（标题不再出现书名号/方案名 · 品阶行带品阶色 · 删行后几何仍不压按钮 · 解析不出品阶时名字退回标题）。
      变异 M461（回到直接导入）/ M462（弹窗+偷偷导入）/ M463（详情行压按钮）/ M464（品阶行不显示）/ M459b·M460b（标题重复/丢名字）全捕获。
      ★★**弱判据又一次**：弹窗读值口把「标题 + 详情行」拼起来断言「含方案名」→ 被详情行 `# 方案: 甲` 顶住，
        M459b 实测 **SURVIVED** ⇒ 正解 = **只取读值口第 1 条（标题行）**。
      ★小工具教训：**变异脚本的多行锚点会被 CRLF 挡住**（ANCHOR-FAIL 0，看着像「没问题」）→ 变异锚点尽量单行。
    ...
    · **取证工具**：`/eh go 封皮测`（10 条编号变异测：频道/载荷/色码三轴）、`/eh go 分享探针`（发送留痕 + 队列 + 是否弹出）、
      **`/eh go 色码测`（12 个在用色码逐条编号实发 —— 实测哪个色码画不出来；清单从色表现取，不写死）**、
      **`/eh go 名号色`（彩蛋名号色候选逐条预览 / `<n>` 选用并存档）。**
    · 相关判据：组 144（分片/信息行/最后一条/≤250B + **点链接只弹窗不导入**）、145（弹窗只留品阶行 + 名字只留一处）、
      156/157（结构 + 变异测）、**158（色码测：12 个在用色码逐条实发）**、**159（名号色候选/选用/非法拒绝）**；
      源码检查 `SHARE MSG SHAPE CHECK`、`COLOR CODE LEN CHECK`、**`SHARE PALETTE CHECK`**。**分支领先 origin 40+ 个提交（未推送）**。
...
## 九、版本要点汇总（最近 10 版，每版 1~2 行；完整记录见 CHANGELOG.md 与 git log）
...
2. **1.72.2** — 🔍 **分享方案光环修复 + 载入引导每次都在**：六个光环分支的早退让「按名字扫描」兜底**永远到不了**（别人角色没学过 debuff 纹理 → 条件恒假、技能不放）→ 改成**两级解析 + 三态**（命中可信 / 扫描干净才敢说「没有」/ 否则如实失败）+ `/eh go tex*` 诊断命令；引导改成每次加载都打；顺手修掉具名帧与同名全局函数冲突导致 `/eh ds hud` 静默失效（新增 `FRAME NAME CLASH CHECK`）。
3. **1.72.1** — 🧭 **新手上路友好化**：加载成功提示 + 四步新手指引（绑键首选「右键方案 → 方案管理 → 快捷键 → [保存]」，宏降备选）；入口三处 = 自动播报 / 配置窗 [使用引导] / `/eh guide`。
4. **1.72.0** — 🎉 **一次覆盖 v1.71.3~v1.71.24 的大版本发布**（上一次发布是 v1.71.2，中间积压 22 个版本 / 26 个提交）。头条 = **方案快捷键从「永远不触发」到真正生效**（四次试错：CLICK 劫持鼠标 → 裸命令名不派发 → **Bindings.xml 本客户端不读** → **接管 ActionButtonDown/Up**；零成本/不占宏位/不抢键位/运行中立即生效）；另有**右键功能合并**成「方案管理」窗、案例模版窗三次改版、图标库 Tab、战斗信息UI 系列精修。断言组扩到 102，各版本变异测试全捕获。
5. **1.71.24** — **右键功能合并成一个「方案管理」弹窗**（用户：「将这两个功能合并成一个弹窗管理.都是右键触发.」）：此前配置窗方案右键=重命名窗、战斗信息UI 方案右键=绑定窗（两个窗两套 chrome）→ 合并成一个窗，窗内两段 ① 方案名称 ② 快捷键，**[保存] 一次提交两段**（互不牵连：空名字只拒绝改名、绑键照常）；**两处右键都开同一个窗**、左键语义不变。★★关键决定 = **不为旧入口留别名**（留别名会让「调用点改回旧名字」的回归悄悄通过 —— 实测变异 M1/M2 正是这样 SURVIVED 的）→ 删掉旧名 + 补「数窗 / 旧名不存在」两条**结构断言**；组 102 + 变异 **8/8** 全捕获。
6. **1.71.23** — **清理四次试错留下的死探针**：删掉 `go bind2`/`go bind3`/`go actbar` 三个**写入型**探针（验的都是已被证伪的 CLICK / 裸命令名 / Bindings.xml 路线 —— 留着只会误导后人），`go bind` 从「写入试验」改为**只读现状检查**（派发前提 / 接管状态 / 逐方案绑定与占用格）；`EvalHelp.lua` 净减 **205 行**；`/eh go diag` 保留为唯一诊断入口。
7. **1.71.22** — ★方案快捷键**真正生效**：四次试错定案 —— ①CLICK 劫持鼠标、②裸命令名不派发、③**Bindings.xml 本客户端根本不读**（ArchiTotem 启用+完整重启后命令表里依然没有它的 `CAST_*`，恒 226 行；unrealUI 源码注释同日记录同一结论）/ ④**可行路线 = 命令名用客户端自带的 `ACTIONBUTTON<n>` + 插件接管全局 `ActionButtonDown/Up`**（`/eh go diag` 实测四者 `type=function`、`SetOverrideBindingClick=nil`）。零成本：不占宏名额/不占动作格/**不抢已有键位**/**运行中立即生效**；补回组合键守卫；清除时**把格交还客户端**；删掉已证伪的 `Bindings.xml`；组 98 重写 + 组 101 判据改写 + 变异 6/6 全捕获。
8. **1.71.21** — **Bindings.xml 随插件自带**（用户三问的定案：文件插件发 = 零配置 / 绑定走弹窗 = 零手动 / 用户只重启客户端一次；`EVAL_BIND_XML_STATUS()` 登录自检：命令表没有 EVAL_GO_PROF_* 就如实提醒重启一次）；★文档深挖定案：`RunBinding` 派发靠客户端侧 **press/release 处理器**、插件无法注册（`SetConsoleKey` = 写明「什么都不做」的兼容桩）→ 裸名永不触发、注册只走启动命令表；组 101（精确前缀计数）+ 变异 2/2 全捕获。
9. **1.71.20** — 左键点「方案」= **打印当前绑定情况**（逐行 方案=键、只打印不动绑定、没有就如实说没有）；★用户边界 = **游戏运行态无法生效的方案不考虑**（Bindings.xml 需重启客户端 → 删除出局）；附 `/eh go bind3` 无尾 CLICK 探针（候选元凶 = 隐藏按钮没禁鼠标/没挪出屏幕）；组 100 + 变异 4/4 全捕获。
10. **1.71.19** — 「方案」标签**可点化**：悬停三行美化说明（左键激活 / 右键绑定 / 右键标签全清）+ **右键 = `EVAL_BIND_CLEAR_ALL()`**（逐键解绑 + 清表 + 存档 + 返回个数，幂等）；组 99（真实右键/真实悬停 tooltip）+ 变异 4/4 全捕获。
