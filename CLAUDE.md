...
## ★★★记忆体写入纪律（上下文成本实测，1.74.9）

- **常驻卷本卷会随会话反复注入**（每次变更都算一份；实测本会话注入 **45 次**、单份 ≈2.4 万字符 ≈1.5 万 tokens）⇒ ① 改动**批量化**（攒一次改完，别边做边改）；② **明细只进参考卷**（§十 / §十一），常驻卷留「索引 + 一句话铁律 + CHECK 名」；③ 常驻卷**硬上限 65,536 字节**，超出＝AI **完全读不到尾部** ⇒ 新增前**先等量合并或删除**；④ 单条＝**一句话规则 + 追溯锚点**，经过/流水进 `CHANGELOG.md`。
- **省上下文的写法**：大段脚本 / 长文本**写进文件**再 `node xx.js`，**不要塞进 `run_code` 的 `code` 里**（正文计入上下文且长期留痕）；长输出先筛（`Select-String` / 只取尾部）再打印。
- **搬迁套路 + 「怎么量上下文」**（实测数据 · 校准法 · 解多帧 zstd 的 node 切片法）→ **参考卷 §十一**。
## ★★★答复语种 = **中文**（用户明确定）
