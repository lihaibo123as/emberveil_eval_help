...
- ★★★1.73.43h **这个客户端会「整条吞掉」一条消息里**多段色码 / 多要素**的聊天消息**（真机变异测定案）：
  用户跑 `/eh go 封皮测`（10 条编号形态）回来**只有 3 条出现**：[1] 一段色码+链接 · [4] 纯文本+一段色码+链接+标签+评语 · [10] 纯文本；
  被吞的 = 两段色码+链接 · 完整封皮（两三段色码）· 带方括号身份前缀的写法 ⇒ **能画的都只有一段色码**。
  ★与「7 位色码吞整条」同一族：**颜色码/要素一旦超出客户端的窄口径，整条消息无声消失**（`RunScript` 不报错、`SendChatMessage` 也不报错）。
  ⇒ 分享信息**拆成两条**依次发（A 身份行 = 一段色码 + 纯文本、**无链接**；B 品阶行 = 一段色码 + 链接 + `[标签]|h|r`），
  每条 **≤1 段色码**；判据 = 组 144 逐条数 `|c` 段数（行为）+ 源码检查 `SHARE MSG SHAPE CHECK`（字面量）。
  ★取证工具留档：`/eh go 封皮测`（编号 A/B 变异测，用户回报「哪些编号出现」一测定案）· `/eh go 分享探针`（发送留痕：脚本原文/是否弹出/队列）。
  ★★1.73.43j 续（用户澄清）：「**多色码一行走不通就单色码一行一行发**」；而且**A 身份行也是单色码却照样没显示** ⇒
    用户判断：「那就不是单双色码的问题，而是要查**哪些特殊字符**让这一行无法显示」。
    ★最终落地（用户确认**已正常显示**）：**只发一条** —— 身份并进主行链接**之后的纯文本**：
    `|c<品阶色>|HEHPF:<id> 0/1:0|h[★源代码秘籍·名]|h|r  阿凡提  <评语>`（一段色码 ✓，实测 ✓）。
    专测 A 行的变异测 v3 留在 `/eh go 封皮测`（[1] A 行原样 · [2] 换品阶色 · [3] 只色码+头衔 · [4] 无色码 ·
    [5] 头衔带方括号 · [6] 最简 · [7] 色码包整行 · [8] 去掉「分享了」 · [9] 现用 B 行对照 · [10] 纯文本）。
  ★★1.73.43k 再续：v3 那轮「带色码的全没出现、无色的都出现」，**与 v2 互相矛盾**（同一个形态 v2 出现过）⇒
    **不是纯内容/色码问题**，更像**短时间多条聊天被客户端/服务器限流**（能正常显示那次分享只有 **4 条**；
    出问题那次 **7 条**；变异测一次 **10 条**）。⇒ `SH_SEND_RATE` **0.5 → 1.0 秒**（每秒 1 条），
    变异测同样 1 秒一条并提示「跑完等 ≥30 秒再跑第二次」。★判据：节奏类夹具改用 0.8/1.1 秒对齐新 rate。
  ★★1.73.43m 定案（v4 变异测：把「频道 / 链接载荷 / 是否带色码」三条轴一次摊开）：
    · **[3] 色码+`0/1:0`链接 · [4] 色码+`1/1:00`链接 · [5] 色码+无序号链接** —— **三条都显示** ✓
      ⇒ **现用的信息行形态（一段色码 + `|HEHPF:` 链接 + `[标签]|h|r`）没有问题**，v3 那轮「带色码全不显示」
      是**前一轮限流造成的假象**（所以探针必须「跑完等 ≥30 秒再跑，期间别再分享」）。
    · 唯一没出现的 **[6]「以链接开头、且完全不带色码」**（`|HEHPF:…|h[…]|h|r` 直接打头）——
      与最早那版封皮（链接只带 id、无外层色码）不显示**一致** ⇒ **「链接打头且无色码」这种形态本客户端不画**。
      ★落地：分享信息行**保留外层色码**（`|c<品阶色>` 打头），不要写成裸链接开头。
    · 频道轴：[7] 说 / [8] 小队 都显示 ⇒ 说与小队的纯文本都没问题；PARTY 路径已验证可用。
  ★★★**色码发送规则（现已定稿，写清楚省得再试错）**：
    ✅ **能发**：`|cAARRGGBB`（**正好 8 位 hex**）；**色码打头**的消息；一条消息里**只放一段色码**（保守策略）；
       带链接时**前面先有色码或普通文字**；两条消息之间**间隔 ≥1 秒**（`SH_SEND_RATE=1.0`）。
    ❌ **不能发**：**6 位/7 位等非 8 位色码**（6 位被当原文画出，**7 位整条被吞**——v1 第 4 条唯一消失的那条）；
       **以 `|H…|h` 链接打头且完全不带色码**的消息（v4 的 [6] 是当轮唯一没出现的，与最早那版裸链接封皮不显示一致）；
       **短时间连发多条**（间隔 0.5s 时结果时好时坏；≥1 秒后稳定）。
    ⚠️ **证据不足、暂按保守处理**：「一条消息里放两段以上色码」——v2 那轮看着像不行，但同轮结果与后来互相矛盾，
       所以**没有干净对照**；当前实现一律**每行只放一段色码**（代价为零，就先这样）。
...
- ★★★1.73.34 **列表/弹窗「滚动条」的项目规范（审计清单，逐条核）**：
  ① **滚轮**：`EnableMouseWheel(true)` 必须显式装 + `SetScript("OnMouseWheel")`；方向**单一来源** `EVAL_WHEEL_DIR(a, b)`
  （上滚 = +1 = 回到前面 → `off = off - dir`）；`dir == 0`（不是滚轮事件）→ **链式交还原脚本**（`prevWheel(a, b)`）；
  ② **夹取**：`off` 双侧都要夹（≥0 且 ≤ `n - ROWS`）——只夹一侧就会越界或永远停不住；
  ③ **边界反馈**：到顶隐藏 ▲、到底隐藏 ▼、空列表两个都藏；
  ④ **滚动条要有自己的位置**：▲▼/滑块**不许压在数据列上**（1.73.34 实测：箭头压在「删除」按钮上，点删除会点到箭头）
  → 给窗口加宽、留一条**专用滚动槽**；
  ⑤ **页码/序号一律整数**：`off / ROWS + 1` 这种**浮点除法**在真机（Lua 5.1）会被 `%d` 静默截断、在 fengari 直接报错；
  并且 `off` 被夹到 `maxOff`（非 ROWS 整数倍）时页码要**特判成最后一页**；
  ⑥ **箭头与指示同一口径**：箭头按行滚而指示按页算 → 用户看到的页码永远对不上（1.73.34 把箭头改成按页滚）。
  ★★判据读**真实控件 + 真实脚本**：`GetScript("OnMouseWheel")` 存在性、`IsMouseWheelEnabled`、模拟滚轮走**真脚本**、
  几何上「箭头左边缘 ≥ 删除列右边缘」。
  ★★**判据的参照物要选「末行」**：判「指示不在数据行上」若只与**第一行**比 y，那么「锚在第 2~8 行」会漏网
  （M265 实测**存活**）→ 改与**最后一行**比才抓住。
  ★等价变异照实记：M263（页码退回浮点）对**真机可见行为**是等价的（Lua 5.1 截断 = 1），只有 fengari 会报错 →
  保留 `math.floor` 当防御性写法，并把可判的性质单独立一条（M263b：滚到底必须显示最后一页）。
- ★★★1.73.35 **右键菜单的「条件门」与「名字→unit」两条硬规矩**（用户要求：分享方案 / 交易 / 踢出公会 / 邀请公会 / 查询 / 踢出队伍）：
  ① **条目出不出现由权限与队伍决定**，不是「点了才说没权限」——CanGuildRemove() / CanGuildInvite() / GetNumPartyMembers()>0 在**建菜单时**就过滤；
  ② **名字→unit 解析只扫本机看得到的单位**（target/mouseover/player/pet/partyN/raidN 按 UnitName 逐个比对），解析不到就**如实退回**（需要 unit 的 API 不能硬来）；
  ③ **查询不许直调 SendWho** —— 那是热路径（会被服务器反滥用），必须走工具箱既有的**限频 who 队列** EVAL_TB_WHO_ENQUEUE（★源码检查 CHAT COLOR WIRING CHECK 当场抓到我第一版的直调）。
  ④ 版式：**两列 + 宽高由条目数算出来**（单一来源 EVAL_TB_MENU_LAYOUT，渲染与断言同一份）；**锚点固定屏幕右上角**（原来「跟鼠标走」在高缩放/多分辨率下会飘）。
- ★★★1.73.35 **「把聊天里的提示搬进弹窗」= 判据要跟着落点走**（用户：插件载入信息移到独立弹窗，[知道了] 下次不弹 / [关闭] 每次都弹）：
  引导文本从聊天搬到弹窗后，老断言（「聊天里要有四步引导」）会**当场红**——这不是回归，是**落点变了**：正解 = 把判据改到**弹窗正文**（读值口给 body 文本），
  并另立一条「知道了 → cfg.loadMsgSeen=true → 下次 EVAL_LOADPOP_SHOW() 返回 false」的行为断言；★两个按钮的判据都走**真实 OnClick**（GetScript），不是只调钩子。
- ★★★1.73.36 **「弹窗位置」这类要求要按字面拆解**（用户：「名字右键弹窗是**相对鼠标点击的位置右上角**」）：
  我 1.73.35 读成「屏幕右上角」→ 错了；正解 = **弹窗左下角贴光标**（`SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", x, y)`），
  于是它天然「在点击位置的右上方展开」。★配套两条：**坐标要除 `UIParent:GetEffectiveScale()`**（`GetCursorPosition` 给物理像素，不折算高缩放下会飘）；
  **贴到屏幕边要夹取**（`px + W > 屏宽 → px = 屏宽 - W`），否则菜单会跑出屏幕。
  ★桩保真：`GetCursorPosition` 必须可读可设（否则「跟着光标」这条判据只能读到 nil → 恒走兜底分支，判据等于没写）。
- ★★★1.73.37 **「点了没反应」的两个真根因**（用户报：右键菜单里 关闭/悄悄话/目标/查询/邀请/复制名字 都没反应，而 分享方案/交易 有反应）：
  ① **闭包捕获了「建菜单那一刻」的名字快照**：`EVAL_TB_MENU_SHOW` 原来**先 `tbMenuBuild()` 再写** `TB_NAME_MENU.name`，
     而首次右键正是建菜单的时刻（名字还是 nil）→ 二十几个动作全拿到 nil → 静默 `return false`（看起来就是「按键坏了」）；
     ★正解 = 名字**先写再建** + 闭包改成**点击时**才读名字（两条都要）；
  ② **「关闭」传了空函数**（`function() end`）→ 点了什么都不做。
  ★★★**判据必须走「真实 OnClick」**：1.73.35 我在测试里**直接调动作函数**（`EVAL_TB_NAME_SAY("Ionol")`），于是「按钮接线断了」这件事在测试里**完全看不见**
  （全绿）——正解 = 加读值口 `EVAL_TEST_TB_MENU_CLICK(标签)` 按标签找到按钮、走 `GetScript("OnClick")`，再断言动作真的发生（预填/入队/菜单关掉）。
  ★★**又是「冗余守卫必须整组拆」**：名字先写 + 点击时读，是**两条互相冗余的守卫**——单拆任一条都 SURVIVED（等价变异），
  只有**整组拆**（M270）才 CAPTURED。★判据还要在**真机同条件**下跑：测试里必须**先丢掉已建菜单**（`EVAL_TEST_TB_MENU_DROP()`）再右键，
  否则菜单早就建好了、快照里是有效名字，快照 bug 根本复现不出来（M267/M269 第一版就是这么存活的）。
- ★★1.73.38 **「接收方要支持私聊」= 先核对再落判据**：分享接收帧**本来就注册了** `CHAT_MSG_WHISPER`（且接收侧不看来源事件）
  → 结论是「已支持」，但**光看代码不算数**：补判据 = ① 帧上 `IsEventRegistered("CHAT_MSG_WHISPER")`（桩补了 `RegisterEvent/IsEventRegistered` 记账）；
  ② 用**真实接收路径** `EVAL_SHARE_ONMSG(msg, sender, "CHAT_MSG_WHISPER")` 喂一个密语分片 → 断言弹窗点名发送者、来源标「密语」、内容解开。
  ★用户侧前提照旧要写明：对方得**开着「接收方案」**（默认关）。
- ★★1.73.38 **tooltip 也要「列全 + 分层」**（用户：「将支持的功能都说明下，美化下样式」）：一个金色标题行 + 每个功能**一行**（`|cff9ad4ff· 名称|r（说明）`）
  + 一条灰色注意行；用 `\n` 在一条 `AddLine` 里分行即可（GameTooltip 支持多行串）。★判据 = **逐个功能名都要在 tooltip 里出现**（列全）+ 含颜色码 + 含换行（分层）——
  只验「有 tooltip」的话，功能加完忘了写进去照样绿。
- ★★1.73.39 **「加个边框」也要读**顶点色**判据**（用户：右键菜单「边框 白色高亮」）：四条 1px 纹理（BORDER 层）`tbSolid(1,1,1,0.75)`，
  判据 = ① 四条都在 ② `GetVertexColor` 的 r/g/b ≥ 0.9（白）③ alpha ≥ 0.5 ④ 上边框宽度 == 窗口宽、左边框高度 == 窗口高（**跨满**，不是一小段）。
  ★别只断言「有纹理」：改成金色照样绿（M271 就是靠顶点色抓的）。
- ★★1.73.39 **「A ↔ B 对调位置」= 改**数据顺序**、不是改坐标**：条目表顺序即「左→右、上→下」的显示顺序，交换两条 `add(...)` 即可；
  判据直接读渲染出来的条目文字（第 2 条/第 5 条），别去比对 x/y（那会把布局细节焊死）。
- ★★★1.73.40 **「圆角」在本客户端只能来自**原生边贴图**：`SetBackdrop` + `edgeFile = UI-Tooltip-Border`（vanilla 右键菜单/提示框同款，
  `bgFile = UI-Tooltip-Background` + `edgeSize 16` + `insets 4`）+ `SetBackdropBorderColor(1,1,1,0.75)` 白框；★`edgeSize` **必须整数**
  （参考插件实测：小数栅格化不可靠）、**不给 edgeFile 就没有边框**（它留在 backdrop 表里继续画）。★判据 = 源码检查
  `MENU ROUND CHROME CHECK`（整数 edgeSize / 白色边框色 / 有 `GetBackdrop` 读回 / 有平边退化 / **禁调本客户端不存在的
  `GetBackdropColor`·`GetBackdropBorderColor`**）+ 行为断言**两条路都验**（正常走圆角；桩里把 `SetBackdrop` 弄成 error 后必须退回四条平边）。
  ★断言读“没设过”的字段时注意：桩的 `__index` 兜底会把 **nil 变成一个空函数**（M280 实测打印出 `function: 0x...`）→ 判据要按「不是那张边贴图」判，不要按「== nil」判。
- ★★★1.73.40 **「末行按钮吊在框外」是最常见的高度公式错**（用户截图：「右键框未包含关闭按键」）：`h = -TOP + (rows-1)*行距 + PAD`
  **漏掉了最后一行的自身高度** → 末行整整 9px 在窗口矩形之外（背景没画到它 → 看起来「菜单里没有这一条」）。
  正解 `h = -TOP + rows * 行距 + PAD`；★判据别用同一个公式自证（`h == EVAL_TB_MENU_HEIGHT(n)` 是**自洽**的，永远绿）——
  要加**独立**的**包含性**断言：每条按钮的 `x/x+w/y/y-h` 必须落在 `[0,W]×[-H,0]` 内（M274 拿这条当场抓到「9(关闭)」）。
- ★★1.73.40 **悬停高亮走 `OnEnter`/`OnLeave` 改颜色**（本项目配方禁用 `SetHighlightTexture`）：底色/文字两处都改，且**离开要逐值复原**；
  判据 = 走**真实脚本**触发 + 从**真控件**读回顶点色（常态 vs 悬停要真的不同、复原要逐值相等）+ **每条**都挂了两个脚本
  （漏挂一条 = 那一条永不变色且不报错）。
- ★★★1.73.40 **「一共有多少条」这类计数必须由闸门守**（用户：「将当前方案添加到案例模版: 猎人分组->停止攻击&收回宠物」）：
  模版数据在 `examples/*.lua`，而「案例模版 11 组 N 条」这句话**散在 4 个地方**（`.toc` 的 Notes + 三语言 README 各 4 处）
  → 加/删一条只改数据不改文档，玩家看到的数字就是**骗人的**且**不报错**。★实据：1.73.25 加「猫德一键宏」时没同步，
  文档一直写 **27** 而实际已是 **28**（本轮新增后 **29**）——直到本轮才被数出来。
  ★判据落成源码检查 **`TEMPLATE COUNT CHECK`**（数 `examples/` 里以 `{ name = "` 开头的条目 == 4 处文档里的数字，
  逐处比对、一个都不许漏）；变异：README/toc 改错数 / 删掉一条模版 → 三发全中。
  ★另：模版内容要**逐条**验（条数 + 每行技能名 + **无条件行 groups 为空** + 顺序）——「能解析」不够：
  保命宏被加上条件就会被条件挡住，而那**不报错**。
- ★★★1.73.40 **样式需求会被下一轮「推翻」——断言必须跟着改方向**（同一个右键菜单：先「悬停亮金底 + 近白字」，
  下一轮用户改成「只要**文字**变色，**不用背景变色**，背景透明」）→ 旧断言「底色变大了」变成**反向需求**。
  ★新判据 = 两条**互相独立**的断言：①「悬停**背景逐值一点都没变**」②「文字**真的变白**（≥ 0.99）」——只验一条必漏（M297 只动底色 / M298 不动文字 各抓一条）。
  ★桩必须能读回**文字色**（`SetTextColor`/`GetTextColor`）——「只文字变色」这类需求，桩不记文字色就**完全判不出来**（旧桩靠 `__index` 兜底把 SetTextColor 变成空操作）。
  ★另：「背景透明」= 条目底 alpha **恰好 0**（不是「变小」），「外框背景加深」= 面板底 alpha **变大**（实例 0.62 → 0.88）——两层颜色**方向相反**，写成一个断言必错。
- ★★1.73.40 **「框增加 padding」= 宽高/标题/条目位置全部由同一组常量算**：`TB_MENU_PAD = { x, t, b }` + `TB_MENU_TOP = -(pad.t + TITLE_H)`，
  宽 = `pad.x*2 + 列宽*2 + 列缝`、高 = `-TOP + rows*行高 + pad.b`（改一处全都动，不会出现「只加了左边」这种残缺）。
  ★判据用**独立阈值**（条目左右/下、标题都离边 ≥6px，且标题不压首行）——**不要引用生产常量自证**（`x >= PAD.x` 是同一个数的两边，永远绿）；
  M301（去掉左右 padding）/ M302（标题贴边）/ M303（去掉下 padding）全捕获。
- ★★1.73.41 **同一份内容不要在两处各写一遍**（用户：「使用引导显示弹窗的插件载入的那个引导窗」）：
  引导原来有**两个展示处**（载入弹窗 vs `/eh guide` 往聊天打 7 行）→ 手动重看入口应指向**同一个窗**：
  入口全部走 `EVAL_HELP_GUIDE`（按钮 / 命令 / 载入提示三条路），它内部优先调 `EVAL_LOADPOP_SHOW`、**弹窗不可用才退回聊天打印**（降级也说出来，绝不静默）。
  ★判据要让 **force 语义承重**：把 `cfg.loadMsgSeen` 打成 true 再点「使用引导」/跑 `/eh guide`，**仍然必须弹**
  （否则点过一次「知道了」就再也看不到引导——同 1.72.2 那类「看不到」事故）；变异 M304（不走弹窗）/ M305（去掉聊天兜底）/
  M306（按钮不接线）/ M307（不 force）全捕获。
### 5.3 测试与断言
...
- **当前版本**：**1.73.0**（头条 = **抓宠帮手（配置窗第 6 个 Tab）**：
  ...
  ★★★**方案分享「隐藏载荷 + 信息行」（1.73.41 ~ 1.73.43n，分支 `probe/share-hide-link`）—— ✅ 已完成（用户真机实测通过）**：
    · **分片**（v2，载荷藏在自定义链接里）：`|cff9ad4ff|HEHPF:<id> <i>/<n>:<hex>|h[方案:名 传输中...p%]|h|r`；
      同帧只发第 1 片，其余进队列**每 1 秒滴一片**（`SH_SEND_RATE=1.0`，原 0.5 秒会时好时坏）。
    · **分享信息行**（最后一步，排在所有分片之后，一段色码）：
      `|c<品阶色>|HEHPF:<id> 0/1:0|h[<符号><品阶>秘籍·<名>]|h|r  <头衔>  <评语>`
      （`0/1:0` 让接收端按「i<1 越界」忽略 ⇒ 对收方案零影响；点它仍可直接导入）。
    · **接收端**：封皮行/身份/评语照旧解析（老格式也认）；分享弹窗**只留** `★[品阶秘籍·名]` + 方案名（带品阶色），
      品阶/身份/评语三行按用户要求**不显示**；点击封皮直接导入、最近的 8 笔可点。
    · **★色码与形态规则已定稿**（见 §5.2 1.73.43 条目）：8 位色码 · 色码打头 · 一行只放一段色码 ·
      **不要以 `|H…|h` 裸链接打头** · 两条消息间隔 ≥1 秒。
    · **取证工具**：`/eh go 封皮测`（10 条编号变异测：频道/载荷/色码三轴）、`/eh go 分享探针`（发送留痕 + 队列 + 是否弹出）。
    · 相关判据：组 144（分片/信息行/最后一条/≤250B）、145（弹窗只留品阶栏）、156/157（结构 + 变异测）；
      源码检查 `SHARE MSG SHAPE CHECK`、`COLOR CODE LEN CHECK`。**分支领先 origin 34 个提交（未推送）**。
  **1.73.1** 起的历史在途（供查）：
...
1. **1.73.34** — 🧰 **工具箱弹窗规程 + 💬 聊天名字右键菜单**（**本次发布**，跨越 v1.73.0 ~ v1.73.34）——
   弹窗**层级阶梯**（配置 10 · 编辑 100 · 输入 220 · 工具箱 200 · 下拉 250）+ 拖动柄**必须是 Button** + 列几何单一来源 +
   滚动条 6 项修复（滚轮单一来源 / 专用滚动槽 / 整数页码 / 边界隐藏）；聊天名字走**方案A**（吞行自拼）真正染色，
   右键给悄悄话 / 邀请 / 目标 / 公会邀请 / 复制名字（**只预填不发送**）；资源名 `UnitPowerType` 实时化；`luacheck` 改**逐文件**（27 个 `.lua`）。
   判据组 1~137 + 变异 M1~M266（等价变异如实记录）。
