...
- ★★★**测试纪律（用户 2026-09-25 收紧）**：「每次任务能否不要进行编译测试.等发现问题了再进行测试」⇒ **默认什么都不跑**；只在「跨文件 / 动载入期代码 / 语法风险高」时跑 luacheck（<1 秒）；`test_engine`（≈32 秒）**等发现问题再跑**（用户报障、我自己看到异常、或改动**动了接线/渲染/存档读写**）；**变异验证不再例行做**，只在**新增或改写判据**时做一次，且能跑单项就别跑整套。★代价要认：语法闸门**照不到**接线/渲染/存档这类静默失效。
...
- ★★★**「探索层」的原值必须带「地图身份」**（1.75.13；用户报障「某些地图打开之后会将探索层的坐标丢失、全部图层都集中在左下区域重叠」）：`WorldMapOverlay1..N` 是客户端**按序号逐图复用**的（`WorldMapFrame_Update` 每图重摆，用不到的 `:Hide()` 且**不清几何**）⇒ **同一个纹理名在不同地图上是完全不同的矩形**。真机存档实证：8 条原值里 **6 条完全相同**（155,-403 240×185 = 某图某条补丁的矩形）= 把「还没布局 / 本图不用」的残留几何记了下来，之后**跨图无条件写回**。修法五条（缺一即复发）：① 原值**按地图身份分桶** `mapFitOrig[GetMapInfo 文件名+纹理尺寸][纹理名]`，**换图当场作废内存记录**并重抓（`smFitNewMap`）；② 只碰**本图在用**的层（`smFitInUse`：`IsShown()==false` / `GetTexture()==nil` 一律跳过；★**读不到就放行**，别把功能判死）；③ 抓原值要**等版式摆稳**（`SMFIT_SETTLE=0.4s` —— 开图那一瞬抓到的就是残留几何，即 ① 的脏值来源）；④ 客户端自报 `GetNumMapOverlays()==0` ⇒ 一个几何都不碰；⑤ 折算**逐层记账**（`wroteKeys[名]=地图身份`）⇒ 还原只还**本图写过的**层（旧写法换图后还原又写一遍别的图的矩形，用户自己救不回来）。存档版本戳 `mapFitVer` **2→3**（旧污染整块丢弃自愈）。判据 = 组 253 + `SM MAP KEY CHECK`；取证 = `/ehm mapfit dump`（只读清单：地图身份 / 客户端 `GetMapOverlayInfo` 真值 / 每层「显示·贴图·在用·记录·已写」）。全案 **R20c**。
