...
- ★★★**框拖拽（`tools/DragFrames.lua`）/ 图层工具（`tools/LayerFix.lua`）判据总表**（★**原文已整批迁参考卷 §十六**，要查踩坑经过/完整写法去那里）：
  ...
  ④ ★★★**所有窗口都不许改宽/高**：唯一真值 **`DF_NO_SIZE`**（载入期给每个目标挂 `noSize`），读它三处 = 属性弹窗（**不建**宽/高行）· `dfAttrsApply` 的 **`allowSize` 门** · 老记录清理（清 w/h 并**还原 `ow/oh`**，没原始值**不去猜**）；**位置调整 = 属性弹窗 X/Y 两行绝对屏幕坐标**（一律换算成屏幕位移走 `dfPlaceFrom`）；**开窗重设 = A 链式接管 `OnShow` + C 有界短复查**（0.3s×4 / 墙钟 3s，**拖拽与战斗跳过**，到点摘脚本）；判据 = 组 206 + 组 208④⑥ + 组 220①③④ + 组 222 + `DF OPEN HOOK CHECK`。
...
- ★★★**任务线（quest/）线（1.75.0~1.75.21，明细 → 参考卷 §十四）**：数据以 `database.emberveil.org/quests` 为准、`quest/QuestData.lua` 是**生成物**（改 `chains.js` 后 `node quest/build.js` 重建，**绝不手改**）；列表 **cap=0 全量**、**跨块统一排序**、系列记录单一来源 `qcSeriesRec`、等级档 **L6=60+**（区间重叠）、未扫出装备画**宏 961 占位图**（点击=插队首优先）、请求队列**按列排 + 上限在排序后截断**、请求泵**常驻开启**（随弹窗 Hide ⇒ OnUpdate 停摆）、工具柄走 `EVAL_WTT_HANDLE()`；判据 = `QUEST TOC CHECK` + 组 191/192。★**闸门一条命令** `node check.js`（`--quick` 6.5s 跳组 192 / `--full` 加 `quest/audit.js`）；`mutate.js` 不在闸门里、**中断自动还原**。
### 5.6 内容 / 数据质量
