...
- **当前版本**：**1.71.1**（队友/团员扫描 + 前两批在途改动；已提交）。上游 **1.71.0**（方案分享 Share.lua）由另一分支提交并已合并进来。
...
- **★工作流铁律（用户明确定，1.70.15；1.71.1 用户明确要求「提交版本」，故本轮按指令发布）**：**默认**——用户实测验证通过之前，一律不提交、不发版、不推送 —— 改动只落工作区文件，让用户 `/reload` 直接实测；**报告里也不要催促提交**
- **数据检索 Tab4（在途主功能）**：
