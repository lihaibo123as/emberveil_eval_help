...
- 当前版本：**1.35.0**（i18n P1：SE/IO/RP/TN 弹窗+战斗UI tooltip+条件类型名（CT_*）+组名（CTG_*）+技能分类名（SK_CAT_*）全入语言包；SE_PICK_ITEM/SE_PICK_TGT 特殊下拉项改为 L() 运行时匹配；README_en.md/README_ru.md 新建，README.md 顶部加语言链接；俄文标签刻意短词防溢出）｜**1.34.1**（i18n 分支：宽语言自适应——WIDE=(EH_LANG~=zhCN) 窗口 560→700、RX 250→330、条件列 236→356、右按钮列（上/编/删/滚动/添加技能/导入导出）全部改 TOPRIGHT 右锚定（mkSmall 加 rightAnch 参数）；俄文渲染实测 OK）｜**1.34.0**（i18n P0，**在 i18n 分支**：EVAL_LOCALES 注册制+EVAL_L 查找（zhCN 基准回退/缺键显示键名/pcall format）；EVAL_SET_LANG 白名单+cfg.lang+新语言提示/reload；ehResolveLang=cfg.lang→GetLocale→GetClientLocale(ru-RU/en/zh-Hans)→zhCN；配置窗标题栏右上国旗行 CN/EN/RU（拷 UnrealQuest tga 进 media/Flags/+ASCII徽标兜底/透明度选中态/EVAL_HELP_CFGWIN 全局桥给 80 行的 LANG_REFRESH 用——local 声明在 1928 看不到）；P0 只翻配置窗外壳+帮助，数据层（条件词/技能名/方案文本）永不翻译；待游戏内验证俄文渲染后 P1 全量）｜master **1.33.4**（/eh go add 路径 8 技能上限残留清除——1.33.0 漏网第三处；README 简介/场景/速览表全面同步五分类+28条件+物品使用）｜**1.33.3**（热修：1.33.0 引入 ROWS 作用域报错——ROWS 是 cfgBuild 内 local，EVAL_WAR_TAB_REFRESH 拿不到（函数作用域陷阱又一实例）；改用 table.getn(warUI.rows) 行池实长）｜**1.33.2**（探针破案根治：本客户端 GameTooltip 的 SetPlayerBuff/SetUnitBuff/SetUnitDebuff/SetAction 返回值恒 nil 但 tooltip 实际已填充——`if oks and built` 门槛把读名卡死；三处光环读名+wactionName 全部拆 built 门槛只留 oks。教训：本客户端 tooltip widget 方法返回值不可信，读文本就好）｜**1.33.1**（/eh go probe buff 探针：GPB/UnitBuff 枚举+SetPlayerBuff/SetUnitBuff 读名+BuffButton0/BuffFrame 存在性原始值打印到聊天框+UELog，诊断药品 buff 不进下拉——待用户回报探针输出再定修复）｜**1.33.0**（方案技能 8 上限全取消：SE保存/导入解析两处检查已删；配置窗技能列表滚动——warUI.offset+^/v 按钮（仅超高显示）+OnMouseWheel（arg1 方向全局，pcall EnableMouseWheel 兜底）；行操作 chk/上/编/删 全部改 ri+offset 索引；删「/eh wdebug 或 /eh debug」旧提示）｜**1.32.10**（药品类 buff 修复：UPDATE_STATE 的 playerBuffs 并入 UnitBuff(\"player\",i) 1基索引枚举兜底——GetPlayerBuff 过滤列表可能漏药品食物；EVAL_PLAYER_BUFF_LIST 名称全空时换 SetUnitBuff 读名路径；wiki 已确认 UnitBuff/SetUnitBuff 存在）｜**1.32.9**（sDrop 光环下拉重构：并行 names 表替代序号边界；剔除 pet/targetSel/item 伪技能；已学习名 ◇ 并入（debuffTex 跨会话，buff 消失可选）；auraTexOf 配图标配图标；EVAL_WAR_TAB_REFRESH 入口补扫自愈——init 重扫早于动作条就绪时图标全灰的修复）｜**1.32.8**（IO 窗 EditBox 装进 ScrollFrame 精确裁剪——SetScrollChild+UpdateScrollChildRect+SetVerticalScroll 自动滚底，本客... (line truncated to 2000 chars)
