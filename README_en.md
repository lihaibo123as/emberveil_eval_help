...
| :-- | :-- | :-- |
| **1.74.16** | 🚫 Main icon no longer "uses the first consumable" | The main icon is the helper's **own special icon** (it represents no item) → items are used **from the row of icons**; both clicks on the main icon just **open/close the picker** (works even with nothing selected); the main icon **no longer shows a cooldown** (each item's CD is shown on its own icon) |
