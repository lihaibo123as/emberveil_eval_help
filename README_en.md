...
| **1.71.15** | 🖱 Right-click in the icon library = minimap button icon | The auto-picked button icon missed the mark ("the icon is wrong") → let her pick it herself: **right-click** any icon in the library to set it as the minimap button icon (persisted, tooltip hint included), and a **hand-picked icon always wins** over the auto-pick; left-click still prints the path; `/eh go mbicon [reset]` to view/clear the custom choice; ★assertions drive the real right-click OnClick, 3/3 mutations caught |
| **1.71.14** | 🧹 Global tab: "One-key macro" group bottom-aligned (overlap fix) | After two sub-switches were added to the Interface group, the macro group header overlapped the "State info UI" checkbox (user screenshot) → the whole group is now **anchored up from the bottom button row**: the debounce row's bottom edge sits 6px above the nav/close row, row pitch unchanged; ★assertions read **real control** positions (no overlap / bottom alignment / pitch / inside window), 3/3 mutations caught |
| **1.71.13** | 🖼 Minimap config button is now an icon + 💬 tooltip rewrite | The button is a single macro icon (picked at runtime from the macro-icon table by name priority: wrench > engineering > gear — no hard-coded path), 26×26 matching the icon-library cell, icon fills the whole button, no border; falls back to the old gold-ring "EH" style when no icon is found (never a blank button); the tooltip now lists the three features (**all-class one-key macro / handy toolbox / world database search**, 3 languages); also ships the `/eh go bind` probe (key-binding feasibility) |
| **1.71.12** | 🎛 Combat info UI: player-name title + "Combat / Profiles" sub-switches | The title now uses `UnitName("player")` (falls back to the locale text when the name is unavailable — never a blank title bar); two indented sub-switches added under "Combat info UI" on the Global tab — the **combat block** (health/power/combo/target bars + cast/swing bars + status line) and the **profile block** (profile row + skill icons) are toggled independently, and ★off means **the block is not drawn at all** (layout shifts up, the frame shrinks), not merely hidden; legacy configs without these keys → **nil = on** |
| **1.71.11** | 📐 Combat-info profile row: capped stretching | The previous build spread **all** leftover width per row, so a single short name became a **huge empty box** → now each cell is at most name-width + 24px and the rest stays at the row end; ★added a reverse sentinel "no empty boxes" (which deliberately does not read the production cap) |
