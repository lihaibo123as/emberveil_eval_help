...
| :-- | :-- | :-- |
| **1.71.11** | 📐 Панель боя: ограничение растяжения профилей | Раньше весь остаток строки распределялся **поровну**, и одно короткое имя превращалось в **огромную пустую кнопку** → теперь ячейка шире имени максимум на 24 px, остаток остаётся в конце строки; ★добавлен обратный контроль «без пустых кнопок» |
| **1.71.10** | 📐 Панель боя: строка профилей — ширина по имени + перенос | Сообщено: «некоторые профили выходят за ширину» — раньше ширина делилась **поровну**, и длинные имена налезали на соседей → теперь ширина каждой ячейки считается по **измеренному имени**, с жадным переносом, растяжением до края и ограничением текста; ★переименование/добавление пересобирает строку автоматически |
| **1.71.9** | ▲ Кнопки порядка — настоящие треугольники + 🖼️ библиотека иконок: 6 → 9 строк на страницу | Кнопки перемещения и прокрутки в списке навыков: `^`/`v` → **настоящие треугольники ▲/▼** (каретка выглядела как чёрточка, а `v` — как буква); в библиотеке иконок теперь **108 → 162 иконки на страницу** (18 столбцов × 9 строк, +102 px) |
| **1.71.8** | 🧩 Окно шаблонов: группы в две колонки + перенос строк внутри группы | Заголовок группы и кнопки шаблонов **идут в одну строку вправо, не влезло — перенос**; **группа — неделимая минимальная единица** (две колонки режутся по порядку, точка разреза выбирается по **наименьшей разнице в числе строк**); каждая группа начинается с новой строки; в китайском окне ширина колонки 310, 11 групп уложились в 8+7 строк, окно 660×240 |
| **1.71.7** | 🧑‍🤝‍🧑 Аудит шаблонов для группы/рейда (указанный навык + условия члена группы/рейда) | 17 шаблонов группы/рейда переведены на формат «**указанный навык + условия члена группы/рейда**» (строка самодостаточна: условие само сканирует людей и переключает цель); ★условия на членов должны стоять **в самом начале строки**; заодно исправлен порог шаманской «治疗波»; аудит закреплён **4 автоматическими воротами + сквозным тестом** |
| **1.71.6** | 🧩 Окно шаблонов переведено на потоковую раскладку (автоперенос) | Заголовок группы и кнопки **идут в одной строке вправо, не влезло — перенос на левую границу**; ширина кнопок измеряется через `GetStringWidth()`; высота окна считается по содержимому (660×180); ★на реальных данных перенос не воспроизводится → добавлена точка входа с **синтетическими данными**, чтобы заставить ветку переноса сработать |
| **1.71.5** | 🧹 В шаблоны добавлены «снятие с группы / снятие с рейда одной кнопкой» | Покрыты обе цепочки — **магия + болезнь**; ★разбиение на две колонки переведено с жадного «набиваем до половины и стоп» на **перебор всех точек разреза с выбором наименьшей разницы в числе строк** |
| **1.71.4** | 🧩 Окно шаблонов в две колонки + 📚 шаблоны расширены до 11 групп и 25 штук | Окно расширено и разбито на две колонки (**колонки по классам, группа никогда не разрывается**); у заголовка появилась золотая подсказка «!»; добавлены четыре группы — жрец / друид / чернокнижник / шаман — плюс функциональная группа для группы/рейда; ★«фильтр по классу» теперь поддерживает и **условия «кандидат»** |
| **1.71.3** | 🛑 Стоп-атака + 🤝 Следование + 📤 Переделка отправки/приёма + 🔇 Блокировка каналов + 🛍 Переделка автопокупки + 🖼 Вкладка «Иконки» + 🧑‍🤝‍🧑 Условия «кандидат» | Одно нажатие останавливает【каст + автовыстрел + авто-повтор жезла + ближнюю автоатаку】(≈ нажатие ESC); следование через `FollowByName` (плюс исправление **тихого отказа из-за полноширинного двоеточия**); из отправки убран канал «рейд», на приёме — **только последняя посылка + четыре ограничения + лимит частоты подсказок**; блокируются уведомления «加入 / 进入 / 离开频道»; автопокупка переведена на **покупку партиями + поштучную сверку + четырёхступенчатое ограничение**; добавлена 5-я вкладка «Иконки»; собственные иконки для семейства «选取目标:» + честный журнал; **условия «кандидат» + выбор людей по оператору сравнения**; в условия для группы/рейда добавлены «мультивыбор классов / мультивыбор отрядов» |
| **1.71.2** | 🧙 Группировка условий + чистка окна настроек + 📂 шаблоны вынесены из скрипта | Условия каста переименованы и собраны в группу **«Состояние навыка»** (Кастуется / Время каста / Осталось каста ‖ Цель кастует / Время каста цели / Осталось каста цели); удалена дублирующая кнопка **[Приём]**, скрыт заголовок «Переключатели», у **[Поделиться]** появилась пошаговая подсказка; библиотека шаблонов переехала в отдельные файлы по классам в `examples/`; ★исправлена серия **«тихих»** ошибок (неразрешимая аура теперь честно сообщается, а не считается «отсутствующей»; условия больше не теряются при экспорте; тень полосы; общая позиция в заглушке тестов) |
