...
- **🗂 Функции ПКМ объединены в одно «Управление профилем»** (1.71.24): имя + горячая клавиша, [Сохранить] отправляет оба; ★никаких псевдонимов для старых точек входа.
- **🖨 🎛 🖼 Доработка интерфейса** (1.71.9 → 1.71.20): кликабельная надпись «Профили» (ЛКМ печатает привязки / ПКМ снимает их), строка профилей с переносом + имя игрока, в библиотеке 6→9 строк + ПКМ назначает иконку миникарты.
- **🛑 🤝 Остановить атаку / следование** (1.71.3): одно нажатие останавливает каст + автовыстрел + жезл + автоатаку; следование через `FollowByName` (одна из двух не-Protected функций Movement).
- **📤 🔇 Обмен и уведомления переписаны** (1.71.3): канал «рейд» убран (в 1.12 лидерский чат идёт отдельными событиями), четыре ограничения приёма + уведомление не чаще 5с; входы и выходы из каналов подавлены.
- **🧑‍🤝‍🧑 🧙 Выбор участника и условия** (1.70.45 → 1.71.2): 4 условия «кандидат» + выбор по оператору сравнения; проверка остатка времени аур, условия каста сведены.
- **🧰 🗺️ 📡 Три новые вкладки** (1.68.0 → 1.71.1): инструменты (очередь + поэлементная проверка), поиск данных (16 категорий меток), обмен профилями (одна кнопка в канал).
- **📚 Шаблоны: 11 групп / 27 штук** (1.67.2 → 1.71.8): группировка по классам, один клик — импорт; шаблоны отряда и рейда всегда «конкретный навык + условие по участнику».
- **🧪 Инженерная инфраструктура** (1.52.0 → 1.72.0): 17 проверок исходников + мутационные тесты каждый раунд; «семейство молчаливых отказов» вычищено.
- **⚡ Модель исполнения и работоспособность** (1.52.1 → 1.64.0): выбор цели больше не съедает нажатие, параллельное исполнение, без остановки по общему кулдауну; причина — «обрезание множественного возврата в Lua».

## Журнал обновлений

| Версия | Тема | Главное одной строкой |
| :-- | :-- | :-- |
| **1.72.1** | 🧭 Дружелюбный первый запуск | Подтверждение загрузки + руководство из четырёх шагов при первом входе (боевой интерфейс / первый профиль / привязка клавиши / журнал способностей); повторить — вкладка «Общие» или `/eh guide` |
