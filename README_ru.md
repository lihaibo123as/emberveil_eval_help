...
| **1.71.0** | 📡 Обмен профилями | Отправка профиля в канал чата; союзники получают окно и кнопку «Импорт» |
| **1.70.44** | 🎯 Корень пустых точек позиционирования | ★диагноз подтверждён на практике: `dsSetOverlay` объявлен раньше → привязался к глобальной переменной, а `dsAnnDraw` идёт после → читает локальную; одна переменная оказалась привязана к двум местам → точки позиционирования не рисовались никогда. Блок состояния целиком перенесён выше; добавлен автостраж DECL ORDER CHECK (заодно выявил два однотипных места: DS_MAX_PINS / dsRndOn) |
