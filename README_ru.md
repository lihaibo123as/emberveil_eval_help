...
**Поиск данных (вкладка 4)**: четыре вида поиска — задания / предметы / существа·NPC / объекты; ввод ищет сразу (первые 10 совпадений) → клик по строке открывает подробности (источники дропа и шансы, контейнеры, продавцы, связанные задания, NPC, выдающий или завершающий задание, описание предмета), вложенность без ограничений; наведение на строку предмета показывает игровой tooltip

![Поиск данных](preview/dataset.png)

**Поиск данных · метки на карте**: клик по значку карты в конце строки открывает карту мира на нужной зоне и отмечает точки появления существа (одно существо — один цвет); кнопка «Метки карты (N)» включает 16 категорий меток (ресурсы / городские услуги), которые рисуются вместе с картой

![Поиск данных · метки на карте](preview/dataset_map.png)
