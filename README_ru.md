...
**🗂 Функции ПКМ объединены в одно «Управление профилем»** (1.71.24): имя + горячая клавиша, [Сохранить] отправляет оба; ★никаких псевдонимов для старых точек входа (псевдоним позволил бы возврату в вызове пройти незаметно).
**🖨 🎛 🖼 Доработка интерфейса** (1.71.9 → 1.71.20): кликабельная надпись «Профили» (ЛКМ печатает привязки / ПКМ снимает их), строка профилей с переносом + имя игрока + подпереключатели, в библиотеке 6→9 строк + ПКМ назначает иконку миникарты.
...
**🧑‍🤝‍🧑 🧙 Выбор участника и условия** (1.70.45 → 1.71.2): 4 условия «кандидат» + выбор по оператору сравнения; проверка остатка времени аур, условия каста сведены в «состояние навыка».
...
**🧪 Инженерная инфраструктура** (1.52.0 → 1.72.0): 17 проверок исходников + мутационные тесты каждый раунд; «семейство молчаливых отказов» вычищено (не найдено ≠ нет, экспорт → импорт).
**⚡ Модель исполнения и работоспособность** (1.52.1 → 1.64.0): выбор цели больше не съедает нажатие, параллельное исполнение, без остановки по общему кулдауну; причина — «обрезание множественного возврата в Lua» — путь условий обязан тестироваться.
