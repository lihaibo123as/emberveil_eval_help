...
| :-- | :-- | :-- |
| **1.71.12** | 🎛 Панель боя: заголовок — имя игрока + подпереключатели «Бой / Профили» | Заголовок берётся из `UnitName("player")` (при недоступности имени — текст из языкового пакета, заголовок не остаётся пустым); под «Боевая панель» на вкладке «Общее» добавлены два сдвинутых подпереключателя — **блок боя** (полосы + строка статуса) и **блок профилей** (строка профилей + значки) включаются независимо, ★выключено = блок **вообще не рисуется** (раскладка сдвигается вверх, окно уменьшается), а не просто скрывается; в старых настройках этих ключей нет → **nil = включено** |
