...
| 🧩 **Шаблоны примеров** | окно настроек, вкладка «Макрос», кнопка `[Шаблоны]` внизу | **12 групп, 34 штук**, сгруппированы по классам, **группы в две колонки + перенос строк внутри группы**; клик = мгновенный импорт готового профиля |
...
| :-- | :-- | :-- |
| **1.74.16** | 🚫 Главная иконка больше не «использует первый расходник» | Главная иконка — **своя специальная** (не представляет предмет) → предметы используются **из ряда иконок**; оба клика по главной **открывают/закрывают выбор** (работает и при пустом выборе); главная **больше не показывает КД** (КД каждого предмета — на его иконке) |
| **1.74.15** | ⏳ КД расходников теперь **у каждого свой** | Раньше был **один общий** КД: выпив одно зелье, вы блокировали все расходники и видели один и тот же отсчёт (неверно); теперь **на КД уходит только использованный предмет**, остальные доступны; каждая иконка в ряду показывает **свою** маску и отсчёт (главная — ту, что использует ЛКМ); пока A на КД, B доступен |
| **1.74.14** | 🧩 Новая группа **Общее** (12 групп / 34 шаблона) + ✂️ раздельное использование отменено + 🎯 иконка и счётчики | ①Общие шаблоны: 5 **независимых от класса** блоков (еда/питьё вне боя, зелье, спешиться, снять «Защиту», стоп-атака) — впервые с `物品:имя` и `取消自身buff:имя`; ②★**отмена разделения из 1.74.12**: выровнено с проверенным путём редактора навыков — поиск в сумках + один вызов `UseContainerItem`, без разделения; ③главная иконка — **специальная**, все выбранные предметы в ряд справа (расширяется только при >1); ④**задержка 0.5 с** перед обновлением счётчиков |
| **1.74.13** | 🔔 Каналы оповещений о заданиях теперь **мультивыбор** (группа > сказать > себе · каждый прогресс один раз) | В инструментах → оповещения о заданиях список стал мультивыбором: выкл/себе/сказать/группа; при нескольких отмеченных используется **только один по приоритету** (группа > сказать > себе), каждый прогресс объявляется **ровно один раз**; вне группы — откат внутри отмеченных (сказать → себе), только «группа» без группы **пропускается**; старый одиночный конфиг мигрирует в набор (выкл = очистка) |
| **1.74.12** | ✂️ Раздельное использование расходников (фикс «съедает всю стопку») + 🔍 команда-проба | ①`UseContainerItem` в этом клиенте съедает **всю стопку** → теперь `SplitContainerItem` отделяет **1 шт**, `PutItemInBag` кладёт в другую сумку, и используется она; ★если разделить нельзя — **честный отказ**, без отката на «всю стопку»; ②новая проба `/eh go 消耗品探针 [имя]` |
| **1.74.11** | ⏱️ Интервал кормления 2с (выше GCD 1.5с) + 🧹 фильтр типов в кандидатах | ①Интервал 1с → **2с** (запас над ГКД 1.5с, чтобы следующее кормление не начиналось до готовности ГКД); элемент очереди **удаляется при извлечении**; ②кандидаты обоих помощников **отсекают несъедобное**: оружие / броню / квестовые предметы / серое качество (руда, травы, хлам); предметы с нечитаемым типом **оставляются** (неизвестно ≠ несъедобно) |
| **1.74.10** | ✨ Снять бафф (я): мультивыбор + 🐛 фикс наложения + ✏️ длительность | ①Ячейка стала **мультивыбором**: скан баффов + записанные ауры + свой ввод, по умолчанию «Любой» = снять все (`取消自身buff:名1,名2`); ②строка «дебафф (я)» больше не накладывается («Любой тип» и «Осталось» разведены); ③«Осталось» переименовано в «Длит.» |
| **1.74.9** | 🐛 Фикс: пустой пункт в выпадающем списке навыков | Слот панели, чьё имя прочиталось пустым, проходил старую проверку `if name` (пустая строка в Lua — истина), и первая строка списка была пустой; такие имена теперь отбрасываются по правилу «неизвестно ≠ отсутствует», а **номер слота пишется в лог** |
| **1.74.8** | ✂️ Новое действие: снять бафф (я) | В плане можно указать `取消自身buff` (снять все снимаемые баффы) или `取消自身buff:<аура>` (только один); спец-навык без слота, рядом с «Отмена каста» / «Стоп атака» / «Следовать», всегда готов, иконка берётся у самой ауры, а пропуск **честно называется** |
| **1.74.7** | 🐎 Езда · Спешивание | Новая группа: значок на экране, ЛКМ — слезть (транспорт = отменяемая аура), меню по ПКМ, авто-спешивание |

> 📜 Подробности по каждой версии — в **[CHANGELOG.md](CHANGELOG.md)**; более ранняя история — в записях git.

## 🧩 Шаблоны примеров (12 групп, 34 готовых профилей)
...
| **Содержимое** | **12 групп / 34 профилей**: 战士 / 法师 / 通用法系 / 通用 / 盗贼 / 猎人 / 骑士 / 牧师 / 德鲁伊 / 术士 / 萨满 / 队伍·团队（заголовки групп — внутренние имена аддона） |
...
**Окно шаблонов**（кнопка `[Шаблоны]` внизу вкладки макроса）: 12 групп / 34 готовых профилей, **две колонки по группам + перенос внутри группы**, один клик — и профиль импортирован
