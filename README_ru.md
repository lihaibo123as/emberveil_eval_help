...
> Аддон-помощник каста **для всех классов** EmberVeil (1.12.1 / Lua 5.1): тело макроса — одна строка `/run EVAL_GO()`, движок правил — **пять категорий навыков** (действия персонажа / навыки персонажа / поведение питомца / выбор цели / использование предметов) × **28 условий** (выпадающий список с группами) × несколько профилей на разных клавишах. Полностью визуальная настройка, без привязки к классу.
>
...
| :-- | :-- | :-- |
| **1.52.0** | 📏 Проверка дистанции автоатаки | Автовыстрел/жезл вне дальности (IsActionInRange=0) автоматически понижаются до ближнего боя — охотник вплотную больше не машет луком вхолостую |
| **1.51.0** | 🎯 Условия состояния стрельбы | Новые условия Автовыстрел/Выстрел жезлом (переключатель да/нет); заодно исправлен старый баг: отрицание ! молча ломало все булевы условия |
| **1.50.0** | 🏹 Автоатака | «Перехват обычной атаки» переименован в «Автоатаку» + приоритет: Автовыстрел > Выстрел (жезл) > обычная атака, недоступное понижается автоматически, с подсказкой при наведении |
| **1.49.3** | 🚑 Исправление отмены каста | SpellStopCasting в этом клиенте Protected — прямой вызов из аддона не работает → заменён на тот же путь через чат-скрипт RunScript |
| **1.49.2** | 🔍 Аудит остатков | Обобщение перехвата (Автовыстрел охотника работает) + динамические логи ярости/стойки + удаление дубля T_RANGE + чистка остаточных файлов |
| **1.49.1** | ▶️ Пустые условия = прямой запуск | Навык без условий = выполняется безусловно (старое поведение: навык с пустыми условиями никогда не срабатывал) |
| **1.49.0** | 🎯 Удалён предварительный выбор врага | Нажатие макроса больше не хватает ближайшего врага первым — конфликтовало с правилами вида «选取目标:最近友方»; выбор цели полностью отдан правилам |
| **1.48.0** | 🧹 Белый список обнулён | Выпадающие списки навыков/параметров — чистое сканирование панели действий: маг больше не видит навыки воина; профиль по умолчанию при новой установке пуст (старт с шаблонов примеров) |
| **1.47.0** | 🏹 Расширение действий персонажа | Добавлены 取消施法/自动射击/射击; стойки поддерживают номера (姿态:2); исправлена связка состояния обычной атаки с переключателем перехвата |
| **1.46.0** | 🧩 Объединение полос иконок | Две строки иконок боевой панели объединены в одну (под строкой профилей, 8 ячеек в ряд, перенос до ≤16); исправлен баг ссылки на таблицу (все иконки ?) после пересканирования |
| **1.45.1** | 🔧 Выравнивание переключателей | Чекбоксы окна настроек и текст привязаны к общей центральной линии (старый вариант визуально занижал рамку) |
| **1.45.0** | 🎯 Отображение по профилю | Строка иконок боевой панели / отчёт пересканирования / обзор состояния — всё по навыкам активного профиля; старый фиксированный список воина упразднён, по-настоящему для всех классов |
| **1.44.1** | 🛟 Страховка входа макроса | При неполной загрузке движка EVAL_GO больше не выдаёт красную рамку nil — подсказка в чате про /reload и проверку включения аддона |
| **1.44.0** | 📋 Шаблоны примеров | В окно импорта добавлено меню «Шаблоны»: сгруппированы по классам, клик = импорт (первый — шаблон оружейного воина); исправлены остаточный лимит импорта и пропущенный мост condTrim |
| **1.43.0** | ⚔️ Навык смены стойки | В действия персонажа добавлено «姿态:X» — прямое переключение панели стоек, не занимает слот (CastShapeshiftForm не защищён), защита от повторной активации |
| **1.42.0** | 📚 Лимит профилей 12 | Профилей 4→12: компактная расстановка в окне настроек + 12 узких ячеек в строке боевой панели + полный набор прямого запуска EVAL_GO1~12 |
| **1.41.2** | 🌉 Финальный аудит мостов | Статический анализатор split_audit.js нашёл 6 пропущенных мостов разделения — все восстановлены, аудит CLEAN |
| **1.41.1** | 🚑 Хотфикс EVAL_GO | Голая ссылка на cfg в Engine (пропущенная форма прямого доступа) роняла макрос — заменена на прямое чтение глобали |
| **1.41.0** | 🎬 Три условия своего каста | «Кастуется» поддерживает любой каст (пустой параметр) + прошедшие/оставшиеся секунды своего каста; исправлен пропущенный мост разделения 1.39.0 |
| **1.40.0** | 🔮 Состояние каста цели | Условия «цель кастует / каст прошёл / каст осталось» — на событиях текста чата + самообучение длительности каста |
| **1.39.0** | 🧱 Модульная перестройка | Один файл 4758 строк разделён на модули Core/Engine/UI, межмодульный обмен через глобальные мосты-экспорты |
| **1.38.1** | 📊 Полоса каста | В боевой панели под полосой цели добавлена полоса каста (прогресс + оставшиеся секунды; в панели состояния строка каста уже была) |
| **1.38.0** | 🎬 Условие «кастуется» | Отслеживание каста на событиях SPELLCAST_*, «施法中:X» защищает от обрыва каста |
| **1.37.0** | 📏 Градация дистанции + условие радиуса | Отображение дистанции до цели по градациям (ближний бой/дистанция рывка/вне дальности) + условие «范围内:X» на IsActionInRange |
| **1.36.4** | 🚑 Хотфикс переключателя иммунитета | immBtn ошибочно вложен в вызов sDrop — ошибка nil при обновлении, возвращён на место (ветка i18n) |
| **1.36.3** | 🔘 Переключатель иммунитета | В зоне параметров «иммунитет цели к навыку» добавлен переключатель 免疫/未免疫 (ветка i18n) |
| **1.36.2** | 📏 Превью на месте | Превью окна редактора перенесено в строку справа от «+ Добавить условие», больше не перекрывает кнопки сохранения/отмены (ветка i18n) |
| **1.36.1** | 🧬 Тип условия «иммунитет» | Условия «免疫:技能/未免疫:技能» читают обученную таблицу — поведение при иммунитете программируется (ветка i18n) |
| **1.36.0** | 🛡️ Обучение иммунитету | Автообучение по текстовым событиям иммунитета (навык@имя моба) — одноимённые мобы больше не проверяют этот навык (ветка i18n) |
| **1.35.2** | 🧹 Похудание окна редактора | Все стрелки [v] у параметров условий убраны — текст параметра сам является кнопкой (ветка i18n) |
| **1.35.1** | 🛡️ Зонд иммунитета | /eh go probe immune — захват прототипов событий иммунитета (ветка i18n) |
| **1.35.0** | 🌐 Интернационализация P1 | Окна редактора/импорта-экспорта/диалоги/подсказки полностью переведены, 28 имён условий в языковом пакете, README на трёх языках (ветка i18n) |
| **1.34.1** | 📐 Адаптация под широкие языки | Окно для англ./рус. расширено 560→700, правая колонка кнопок привязана вправо, длинный текст больше не пересекается (ветка i18n) |
| **1.34.0** | 🌍 Интернационализация P0 | Каркас на трёх языках (кит./англ./рус.) + выбор флага в окне настроек, тексты UI идут через языковой пакет (ветка i18n) |
| **1.33.4** | 🧽 Очистка остатков лимита | Пропущенный лимит 8 навыков в /eh go add удалён; вступление и сценарии README полностью обновлены |
| **1.33.3** | 🚑 Хотфикс прокрутки | Хотфикс ошибки области видимости ROWS из 1.33.0 (теперь фактическая длина пула строк) |
...
**Окно настроек · вкладка «Макрос»**: слева панель профилей (до 12, клик = переключить / правая кнопка = переименовать) + список навыков (порядок = приоритет, галочка = включён, условия видны сразу) + [Добавить навык] / [Импорт/Экспорт]

![Окно настроек · Макрос](preview/cfg.png)

**Редактор навыка**（кнопка [Изм.] или клик по иконке навыка）: условия настраиваются построчно — тип условия/параметры только из выпадающих списков (28 типов условий, сгруппированы Себя/Цель/Ауры/Навык), колонка связи переключает & (все внутри группы) / | (любая из групп), превью в реальном времени

![Редактор навыка](preview/skill_cfg.png)

**Двухуровневый список навыков**: строка навыка = [категория ▾][конкретный пункт ▾] — два списка рядом, пункты с иконками — навыки персонажа (все навыки со сканирования панели действий):

![Двухуровневый список · навыки персонажа](preview/skill_person.png)

**Поведение питомца**: 8 команд питомца (прямой вызов Pet API, слот панели не нужен), иконки команд самообучаются по панели питомца:

![Команды питомца](preview/skill_pet.png)

**Выбор цели**: 9 способов переключения (ближайший враг / предыдущая цель / цель цели / по имени / сброс цели…), нажатие макроса сначала переключает цель, затем действует:

![Выбор цели](preview/skill_target.png)

**Использование предметов**: сканирование сумок в реальном времени (с иконками и количеством) + ручной ввод имени — расходники используются сразу, экипировка надевается автоматически:

![Использование предметов](preview/skill_item.png)

**Полная цепочка баффов от зелий**: баффы от зелий/еды (например «再生») можно показывать (○ — реальное время), выбирать, сопоставлять — условие «无buff:再生» запускает выпить зелье одной клавишей:

![Сопоставление баффа от зелья](preview/skill_status.png)
