...
| 🧩 **Шаблоны примеров** | окно настроек, вкладка «Макрос», кнопка `[Шаблоны]` внизу | **12 групп, 35 штук**, сгруппированы по классам, **группы в две колонки + перенос строк внутри группы**; клик = мгновенный импорт готового профиля |
...
| :-- | :-- | :-- |
| **1.74.27** | 🧹 Трансляция редких **вынесена в отдельный модуль** `tools/RareWatch.lua` + **выключатель в инструментах** | Блок переехал в `tools/` (в главном файле остались две точки подключения); в инструментах появился раздел с выключателем, читающим единственный источник истины |
| **1.74.26** | 🧹 Клик по имени **делает одно**: вызывает функцию выбора цели; промах — **сообщение об ошибке** | По требованию удалены все связи с механикой аддона заданий (координаты / ближайшая точка / проекция на карту / открытие карты / текст о расстоянии), токен ссылки сократился до `EHRW:<имя>`; один клик = один `TargetByName(имя)` |
| **1.74.25** | 📏 Исправление **сообщения** при выборе цели (названа причина — ограничение клиента + расстояние) + проба цели | На скриншоте редкий был в **556 ярдах**, а сообщение винило существо — на самом деле выбор по имени достаёт только **близкие** цели (документировано; вдали молча не срабатывает). Теперь так и написано + указано расстояние; команда пробы цели решает вопрос одной строкой |
| **1.74.24** | 🎨 Оповещение **окрашено по рангу + кликабельное имя** (ЛКМ = взять в цель / ПКМ = отметить точку на карте) | Имя окрашено по рангу (элитный — серебро / редкий — синий / редкий элитный — фиолетовый / босс — оранжевый), ровно один 8-значный цветокод на строку; имя — ссылка: **ЛКМ берёт редкого в цель** (TargetByName с проверкой до/после), **ПКМ открывает карту и отмечает ближайшую точку появления** |
| **1.74.23** | 🔔 **Трансляция оповещения о редких** (аддон заданий нашёл редкого → строка в чате) | В момент появления карточки редкого в UnrealQuest в чате сразу появляется строка: имя + ранг (элитный / редкий элитный / босс / редкий) + ярды до ближайшей точки появления + сколько ещё вошли в радиус. Реализовано **перехватом выхода карточки** (RareAlert:Show), а не опросом, с автоматическим резервным путём. Включатель и состояние: /eh go 稀有 |
| **1.74.22** | 🧩 «Божественная буря» в шаблонах паладина (поделился Rainbow) + 🎭 имена шаблонов пересобраны по ** lore Азерота × своему рангу** + 📝 у шаблонов есть **автор/примечание** + 🔆 божественный ранг перекрашен в **ярко-синий** | ①Схема «Божественная буря» из конфигурации пользователя (13 навыков, воздаяние) добавлена в шаблоны паладина: текст получен **собственным экспортёром** аддона (из сохранённой таблицы схемы) и прошёл проверку `export(parse(text)) == text`; ②**все 34 шаблона переименованы, описания переписаны** — имена взяты из lore Азерота по **рангу самого шаблона** (обычный = мир смертных · редкий = элита · самоцвет = герой · легендарный = легенда, напр. «Адский крик» / «Серебряная длань»; «Божественная буря» мифического ранга уже звучит как название заклинания → **оставлена как есть**); ③у записей шаблонов появились необязательные поля `author`/`note` — при наведении видно «Автор: Rainbow» и «Примечание: бескорыстная помощь паладина LM-Rainbow из Драконьего Края», импорт записывает этого автора как источник (шаблоны без `author` по-прежнему получают «Академию»); ④**цвет божественного ранга** сменён с тёмно-красного `c00000` на **ярко-синий `00bfff`** (единственный источник `SH_SEAL_TIERS` — окно настроек / окно шаблонов / обложка обмена / значок титула). |
| **1.74.21** | 🎯 Порог божественного ранга — **50 очков** | Диапазоны: ≤11 обычный / 12-23 редкий / 24-35 эпический / 36-49 легендарный / **≥50 божественный**; ограничения по охвату (23/35/49) и все производные значения следуют за таблицей автоматически |
| **1.74.20** | 👤 Настройки **по персонажам** (три помощника) | Помощники расходников / кормления / спешивания теперь хранят **всю** конфигурацию (выключатели · списки предметов и еды · кэш иконок · позицию иконки, 15 ключей) в **поперсонажной** переменной EVAL_HELP_CHAR — у каждого свои настройки; старые настройки наследуются один раз при первом входе; профили / язык / журнал / обмен остаются общими |
| **1.74.19** | 🏅 Ранги профилей и титулы **переделаны (намного сложнее)** | Старое правило: **пустой навык давал 1 балл** → 25 пустых навыков = божественный ранг (воспроизведено); условия стоили одинаково; каждая ступень титула требовала всего **один** профиль. Новое правило: **пустые навыки = 0** · условия взвешены (себя/навык 1 · цель/аура 2 · группа 3) · лимит 12 на навык · одинаковые условия считаются один раз · **ограничение по охвату** (меньше 4 групп условий — ниже божественного) · диапазоны ≤14/15-34/35-69/70-119/**≥120** · титулы 1/2/3/3/2 с **пошаговым** ростом |
| **1.74.18** | ⇕️ Полоса иконок в боевом HUD **переносится автоматически**, высота подстраивается | Раньше было жёстко 2 ряда по 8 — всё начиная с 17-го навыка молча пропадало; теперь полоса переносится по 8 в ряд под любое число навыков активного профиля, а высота окна растёт/уменьшается вместе с числом рядов (содержимое ниже сдвигается); изменение количества ловит heartbeat, а формула рядов у BUILD и heartbeat **одна** |

> 📜 Подробности по каждой версии — в **[CHANGELOG.md](CHANGELOG.md)**; более ранняя история — в записях git.

## 🧩 Шаблоны примеров (12 групп, 35 готовых профилей)
...
| **Содержимое** | **12 групп / 35 профилей**: 战士 / 法师 / 通用法系 / 通用 / 盗贼 / 猎人 / 骑士 / 牧师 / 德鲁伊 / 术士 / 萨满 / 队伍·团队（заголовки групп — внутренние имена аддона） |
...
**Окно шаблонов**（кнопка `[Шаблоны]` внизу вкладки макроса）: 12 групп / 35 готовых профилей, **две колонки по группам + перенос внутри группы**, один клик — и профиль импортирован
