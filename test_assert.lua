...
eq(tpl.name, "地狱咆哮", "weapon warrior template")
local prof26, err26 = EVAL_PROFILE_FROM_TEXT(tpl.text)
eq(prof26 ~= nil, true, "template parses: " .. tostring(err26))
eq(prof26.name, "地狱咆哮", "template profile name")
eq(prof26.skills[1].skill, "姿态:战斗姿态", "template uses stance skill")
eq(table.getn(prof26.skills), 10, "template skill count")
-- ★1.74.5 用户（截图）要求地狱咆哮模版按图补齐：新增「选敌」「冲锋」两行；压制加「战斗中」、撕裂加「未免疫/可用」
eq(prof26.skills[2].skill, "选取目标:最近敌人", "weapon template row2 = pick nearest enemy")
eq(prof26.skills[3].skill, "冲锋", "weapon template row3 = charge")
eq(prof26.skills[10].skill, "英勇打击", "weapon template last = heroic strike")
-- ★条件列表要逐条对得上（“范围内:冲锋”这种技能侧条件被静默丢掉就会少一条）
eq(table.getn(prof26.skills[3].groups), 1, "charge has one condition group")
...
do
  ...
  -- ★★★1.73.25 用户要求（截图三：他自己的猫德配置）：「加入翡翠追猎模版」——
  ...
  do
    ...
    for _, tpl in ipairs((druid60 and druid60.list) or {}) do
      if tostring(tpl.name) == "翡翠追猎" then cnm60 = tpl end
    end
    eq(cnm60 ~= nil, true, "★★★「翡翠追猎」模版存在（用户点名要的那个）")
  ...
  -- ★★★1.73.40 用户要求（截图 = 他自己在用的「呼哨唤兽」方案）：
  ...
  do
    ...
    for _, tpl in ipairs((hunter and hunter.list) or {}) do
      if tostring(tpl.name) == "呼哨唤兽" then petRec = tpl end
    end
    eq(petRec ~= nil, true, "★★★猎人组里有「呼哨唤兽」模版（用户点名要的那条）")
...
do
  ...
  -- ④ ★★★1.74.5 用户：「取消彩蛋」——[忽略] / [导入] **不再发**角色扮演反应（反向哨兵），改为**只发一句场景描述**。
  --   ★功能与三语言文案**保留**（用户明确「不是把彩蛋功能删除」）—— 只是两处调用点摘掉；结构判据仍在组 170，接线闸门在 SH FUN CHECK（它现在钉「机制都在 且 未接线」）。
  --     已随功能一并删除 —— 不留死代码。★注意别和另一个同名的「彩蛋：创世者亲临 / 自定义头衔」搞混，那套仍在。）
  EVAL_SHARE_RESET()
  TEST.inGuild = true
  EVAL_HELP_CONFIG.title = { tier = 2, draws = {} }
  for _, m in ipairs(other82) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_GUILD") end
  eq(EVAL_SHARE_PENDING() ~= nil, true, "④前置：别人的公会分享正常弹出")
  TEST.runScripts = nil
  local qBefore82 = EVAL_SHARE_TEST_QUEUE_LEN()
  eq(EVAL_TEST_SHARE_CLICK_IGNORE(), true, "④★点到了真实的 [忽略] 按钮（走它自己的 OnClick 闭包）")
  eq(table.getn(TEST.runScripts or {}), 0, "④★★★取消彩蛋：忽略**不再直发**任何一句（RunScript 0 条）")
  eq(EVAL_SHARE_TEST_QUEUE_LEN() == qBefore82 + 1, true, "④★★新行为：只把**场景描述**放进限频队列（+1 条，不直发）")
  eq(tostring(EVAL_TEST_SHARE_SCENE_LAST() or "") ~= "", true,
     "④★场景描述确实生成了：" .. tostring(EVAL_TEST_SHARE_SCENE_LAST()))
  eq(EVAL_SHARE_PENDING() == nil, true, "④★忽略后待导入清空")
  -- ⑤ 导入：同样没有反应句，但**照旧真的导入**
  EVAL_SHARE_RESET()
  for _, m in ipairs(other82) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_GUILD") end
  eq(EVAL_SHARE_PENDING() ~= nil, true, "⑤前置：再来一份公会分享")
  local before82 = table.getn(EVAL_HELP_CONFIG.war.profiles)
  TEST.runScripts = nil
  eq(EVAL_TEST_SHARE_CLICK_IMPORT(), true, "⑤★点到了真实的 [导入] 按钮")
  eq(table.getn(EVAL_HELP_CONFIG.war.profiles) > before82, true, "⑤★★导入照旧生效（取消彩蛋不影响导入）")
  eq(string.find(table.concat(TEST.runScripts or {}, "\n"), "秘籍·", 1, true) == nil, true,
     "⑤★★★取消彩蛋：导入也**不再发**那句带「秘籍·名」链接的反应句")
  eq(tostring(EVAL_TEST_SHARE_SCENE_LAST() or "") ~= "", true, "⑤★改为发场景描述（内容与兜底见组 180）")
  TEST.inGuild = false
  EVAL_SHARE_RESET()
...
do
  ...
  eq(findTpl91("通用法系", "小队奥智") ~= nil, true, "★用户要求：通用法系 → 小队奥智")
  eq(findTpl91("通用法系", "团队奥智") ~= nil, true, "★用户要求：通用法系 → 团队奥智")
  eq(findTpl91("通用法系", "小队真言") ~= nil, true, "★用户要求：通用法系 → 小队真言")
  eq(findTpl91("通用法系", "团队真言") ~= nil, true, "★用户要求：通用法系 → 团队真言")
  eq(findTpl91("队伍/团队", "小队救护") ~= nil, true, "★用户要求：队伍/团队 → 小队救护")
  eq(findTpl91("队伍/团队", "团队救护") ~= nil, true, "★用户要求：队伍/团队 → 团队救护")
  eq(findTpl91("队伍/团队", "小队祝福") ~= nil, true, "★用户要求：队伍/团队 → 小队祝福")
  eq(findTpl91("队伍/团队", "团队祝福") ~= nil, true, "★用户要求：队伍/团队 → 团队祝福")
  eq(findTpl91("队伍/团队", "小队驱邪") ~= nil, true, "★用户要求：队伍/团队 → 小队驱邪")
  eq(findTpl91("队伍/团队", "团队驱邪") ~= nil, true, "★用户要求：队伍/团队 → 团队驱邪")
  ...
  local function auditTplContent91()
    ...
    -- ① 治疗类：指定技能 + 队伍/团队血%
    local p95, s95 = skOf91("队伍/团队", "小队救护")
    eq(s95[1] and s95[1].skill, "快速治疗", "★★★小队救护：第一行就是**指定技能**「快速治疗」（不再先切目标）")
    ...
    local _, s95b = skOf91("队伍/团队", "团队救护")
    eq(cdOf91(s95b[1]) and cdOf91(s95b[1]).name, "团队", "★★★团队救护：范围 = 团队（40 人团靠它）")
    local _, s95c = skOf91("牧师", "小队救治")
    eq(s95c[1] and s95c[1].skill, "快速治疗", "★★牧师一键治疗：指定技能 + 队伍血%")
    eq(cdOf91(s95c[1]) and cdOf91(s95c[1]).k, "teamHp", "★★同上（牧师那套也改过来了）")
    local _, s95d = skOf91("德鲁伊", "自然之愈")
    ...
    local p94, s94 = skOf91("队伍/团队", "小队驱邪")
    eq(s94[1] and s94[1].skill, "驱散魔法", "★★★小队驱邪：第一行是**指定技能**「驱散魔法」")
    ...
    local _, s94c = skOf91("队伍/团队", "团队驱邪")
    eq(cdOf91(s94c[1]) and cdOf91(s94c[1]).name, "团队", "★★★团队驱邪：范围 = 团队")
    local _, s94d = skOf91("牧师", "小队净化")
    ...
    local p91, s91 = skOf91("通用法系", "小队奥智")
    eq(s91[1] and s91[1].skill, "奥术智慧", "★★★小队奥智：唯一一行就是**指定技能**「奥术智慧」")
    ...
    local _, s91b = skOf91("通用法系", "团队奥智")
    eq(cdOf91(s91b[1]) and cdOf91(s91b[1]).name, "团队", "★★团队奥智：范围 = 团队")
    local _, s91c = skOf91("通用法系", "小队真言")
    eq(cdOf91(s91c[1]) and cdOf91(s91c[1]).s, "真言术:韧", "★★小队真言：点名真言术:韧")
    eq(cdOf91(s91c[1]) and cdOf91(s91c[1]).cs, nil, "★★耐力人人有用 → 不带职业过滤（空 = 全部职业）")
    local _, s91d = skOf91("队伍/团队", "小队祝福")
    eq(s91d[1] and s91d[1].skill, "奥术智慧", "★★小队祝福：第一行是指定技能")
    eq(cdOf91(s91d[1]) and cdOf91(s91d[1]).k, "teamBuff", "★★而且带队伍buff 条件")
    eq(table.getn(s91d), 2, "★★两条链（智力 + 耐力）各一行")
    local t92 = findTpl91("骑士", "小队之力")
  ...
  local function auditTplGate91()
    ...
    local a1, _n1, u1 = runTpl91("队伍/团队", "小队救护")
    eq(a1, true, "★★★端到端：模版「小队救护」真的出手")
    ...
    local a2, _n2, u2 = runTpl91("通用法系", "小队奥智")
    eq(a2, true, "★★★端到端：模版「小队奥智」真的出手")
    eq(u2, 2, "★★★用的是指定的「奥术智慧」")
    eq(TEST.targetSel, "unit:party2", "★★★切到了「缺这个 buff 的那个人」（无队伍buff 条件在选人）")
    local a3, _n3, u3 = runTpl91("队伍/团队", "小队驱邪")
    eq(a3, true, "★★★端到端：模版「小队驱邪」真的出手")
    eq(u3, 3, "★★★用的是指定的「驱散魔法」")
...
do
  ...
  eq(type(ar) == "table", true, "①前置：拿到调序/滚动按钮的文本")
  eq(ar.rowUp, "▲", "①调序「上」按钮的真实文本 = ▲")
  eq(ar.rowDn, "▼", "①调序「下」按钮的真实文本 = ▼")
  -- ★★★1.74.17 **反转**：滚动按钮从「列表右栏的两个 ▲/▼」改成「底部行上的文字按钮 [上翻][下翻]」
  --   （用户：「查看系统记忆和图标库的滚动条实现方式，将技能列表的滚动条方式调整下。
  --     在有滚动条的时候上一页和下一页按钮放置在底部案例模版按钮左侧、右对齐」）。
  --   期望值取**语言包**（不是字面量）：三语言下标签各不相同，写死「上翻」在 enUS/ruRU 下必假。
  local Lz92 = EVAL_LOCALES[EVAL_GET_LANG()]
  eq(ar.scrollUp, Lz92["TB_UP"], "①技能列表「上一页」按钮 = 语言包的 " .. tostring(Lz92["TB_UP"]))
  eq(ar.scrollDn, Lz92["TB_DN"], "①技能列表「下一页」按钮 = 语言包的 " .. tostring(Lz92["TB_DN"]))
  eq(ar.scrollUp ~= "▲" and ar.scrollDn ~= "▼", true,
    "①反向哨兵★1.74.17：滚动不再是三角——★三角**只留给行的调序**，两套东西不再长得一样")
...
do
  ...
  -- ⑥ ★★★1.74.27 稀有提醒转播的**工具箱开关**（用户：「将以上功能提取到独立文件内 ./tools 然后再工具箱内设置开关.」）
  --   判据：① 模型里有这一行且是 `rw` 类型（读写走实现自己的 EVAL_RW_ENABLED/EVAL_RW_SET，**不存第二份**）；
  --         ② 模型 > 一页容量 ⇒ 它落在第 2 页（翻页后控件真的出现）；③ 走**真实 OnClick** 勾一下 →
  --            实现自己的真值跟着翻转（工具箱与 `/eh go 稀有` 同源），并回执一行状态；④ 复原、翻回第 1 页。
  do
    local m126 = EVAL_TEST_TB_ROWS()
    local rwRow126, rwIdx126 = nil, nil
    for i = 1, table.getn(m126) do if m126[i].key == "rareWatch" then rwRow126, rwIdx126 = m126[i], i end end
    eq(rwRow126 ~= nil, true, "⑥★★★工具箱模型里有「稀有提醒转播」这一行（idx=" .. tostring(rwIdx126) .. "）")
    eq(rwRow126 and rwRow126.t, "rw", "⑥★★它是 rw 类型：读写走 EVAL_RW_ENABLED/EVAL_RW_SET（不另存一份 tb.rareWatch，免得两处真值打架）")
    eq(type(rwRow126 and rwRow126.label) == "string" and string.len(rwRow126.label) > 0, true, "⑥★带标签（三语言齐由 LANG KEY CHECK 守）")
    eq(type(rwRow126 and rwRow126.tip) == "string" and string.len(rwRow126.tip) > 0, true, "⑥★带悬停说明")
    eq(allN120 > cap120, true, "⑥★模型 " .. tostring(allN120) .. " 条 > 一页容量 " .. tostring(cap120) .. " ⇒ 这一行在第 2 页")
    eq(EVAL_TB_TEST_SCROLL_CLICK("dn"), true, "⑥★点「下翻」（真实 OnClick）")
    eq(EVAL_TB_TEST_OFF() > 0, true, "⑥★确实翻到第 2 页（off=" .. tostring(EVAL_TB_TEST_OFF()) .. "）")
    local chk126, mark126 = EVAL_TEST_TB_CHK_FOR("rareWatch")
    eq(chk126 ~= nil, true, "⑥★★第 2 页真的建出了这一行的勾选框（列→条目映射没错位）")
    eq(mark126 ~= nil, true, "⑥★勾号也在（勾选状态才看得见）")
    local before126 = EVAL_RW_ENABLED()
    local okOn126 = false
    if chk126 then
      local okg, fn = pcall(chk126.GetScript, chk126, "OnClick")
      if okg and type(fn) == "function" then pcall(fn) okOn126 = true end
    end
    eq(okOn126, true, "⑥★走真实 OnClick 勾一下")
    eq(EVAL_RW_ENABLED(), not before126,
       "⑥★★★开关写进了**实现自己的真值**（EVAL_RW_ENABLED 跟着翻转：工具箱只是它的一个面板）")
    eq(EVAL_TEST_TB_CHK_FOR("rareWatch") ~= nil, true, "⑥★勾选后走完整刷新路径、控件仍在（没有把行刷丢）")
    -- 回执：勾完必须**打一行状态**（玩家要立刻看到开了/关了 + 挂上了没 + 已转播几次）
    eq(TEST.chat ~= nil and string.len(tostring(TEST.chat)) > 0, true, "⑥★★勾选后回执一行状态（不必再去敲命令确认）")
    if chk126 then
      local okg2, fn2 = pcall(chk126.GetScript, chk126, "OnClick")
      if okg2 and type(fn2) == "function" then pcall(fn2) end
    end
    eq(EVAL_RW_ENABLED(), before126, "⑥★再勾一次复原（跨用例不留残留）")
    eq(EVAL_TB_TEST_SCROLL_CLICK("up"), true, "⑥★翻回第 1 页")
    eq(EVAL_TB_TEST_OFF(), 0, "⑥★回到第 1 页（=0）")
    TEST.chat = nil
  end
...
-- 143) ★★★1.73.42/42n 分享显示行：头衔（抽卡身份）· 品阶（评分制）· 评语（每档 10 条随机）
do
  -- ★★★1.73.42n 头衔抽卡（替代原「境界 = 等级派生」）：5 档 × 15 张 = 75 个头衔
  eq(EVAL_TITLE_TIER_COUNT(), 5, "头衔 5 档（= 方案稀有度档数）")
  local seenTitle143 = {}
  for i = 1, 5 do
    eq(EVAL_TITLE_COUNT(i), 15, "第 " .. i .. " 档卡池 15 张（实际 " .. tostring(EVAL_TITLE_COUNT(i)) .. "）")
    for k = 1, 15 do
      local tk, tti, tkk = EVAL_TITLE_KEY(i, k)
      eq(tk, "TITLE_" .. i .. "_" .. k, "键名规范 " .. i .. "/" .. k)
      eq(tti == i and tkk == k, true, "取键不改档位/序号")
      local tnm = tostring(EVAL_L(tk))
      eq(tnm ~= tk and tnm ~= "", true, "★★头衔文案取得到（不是裸键）：" .. tostring(tk))
      eq(seenTitle143[tnm] == nil, true, "★★★75 个头衔不许重名：" .. tnm)
      seenTitle143[tnm] = true
    end
  end
  eq(EVAL_TITLE_KEY(0, 0) ~= nil and EVAL_TITLE_KEY(99, 99) ~= nil, true, "★越界（0/0、99/99）也返回合法键，不返回 nil")
  local function txt(n, cond)
    local s = "# 方案: 甲"
    for i = 1, n do s = s .. "\n- 技能" .. i .. (cond and " | 可攻击 & 敌对" or "") end
    return s
  end
  -- ★★★1.74.19 评分口径**重做**（用户：「审计方案的评级标准…增加评级难度」）：
  --   ① **空技能 0 分**（旧口径一条无条件技能白送 1 分 ⇒ 25 条空「攻击」= 120 分 = 神级，实测复现过）；
  --   ② 条件**按类加权**（目标类 2 / 光环 2 / 队伍·候选 3 / 自身状态与技能状态 1）；
  --   ③ 单条技能上限 12 分；④ 同一行里**完全一样**的条件只算一次。
  --   ★本组只钉「口径 + 档位边界」，细化判据在新组 196（含「25 条空技能不许神级」这条历史漏洞的回归哨兵）。
  eq(EVAL_SHARE_SEAL_SCORE(txt(3)), 0,
     "★★★评分：3 条**无条件**技能 = **0 分**（旧口径是 3 —— 空技能不再白送分）")
  eq(EVAL_SHARE_SEAL_SCORE(txt(2, true)), 10,
     "★★评分：2 条各带 2 个目标类条件（各 2 分）= 2 ×（1 + 4）= 10（旧口径是 6）")
  -- ★★★1.74.21 新档位（用户定：**神级 = 50**；低四档把 0~49 四等分）：
  --   ≤11 普通 / 12-23 稀有 / 24-35 珍稀 / 36-49 绝版 / **≥50 神级**。
  --   ★判据**每个边界的两侧都钉**（不是只钉上沿）：阈值挪 1 分就当场响（只钉一侧会漏掉「门槛偏高」那种错）。
  local tiers143 = { "普通", "稀有", "珍稀", "绝版", "神级" }
  local edges143 = { { 11, 12 }, { 23, 24 }, { 35, 36 }, { 49, 50 } }
  for i = 1, 4 do
    eq(select(2, EVAL_SHARE_SEAL_TIER(edges143[i][1])).name, tiers143[i],
       "品阶：" .. edges143[i][1] .. " = " .. tiers143[i] .. "（第 " .. i .. " 档上沿）")
    eq(select(2, EVAL_SHARE_SEAL_TIER(edges143[i][2])).name, tiers143[i + 1],
       "★品阶：" .. edges143[i][2] .. " = " .. tiers143[i + 1] .. "（第 " .. (i + 1) .. " 档下沿 —— 边界两侧都要对）")
  end
  eq(select(2, EVAL_SHARE_SEAL_TIER(1)).name, "普通", "品阶：1 = 普通（最低档）")
  eq(select(2, EVAL_SHARE_SEAL_TIER(999)).name, "神级", "品阶：999 = 神级（最高档无上沿）")
  local reps = { 1, 12, 24, 36, 50 } -- ★1.74.21 每档任取一个分（色码判据只看档位，不看具体分）
  ...
  eq(wantColor[5], "|cff00bfff", "★★神级 = **亮蓝**（★1.74.22 用户定；配色史：暗金 → 亮蓝 → 深红 → 亮金 → 亮蓝）")
  for i = 1, 5 do
    local set = {}
    for k = 1, 10 do
  ...
  -- ★★★1.74.19 造一份**能到神级**的文本：新口径下「神级」不只要分数够（≥120），
  --   还要**覆盖 ≥4 个大类**（覆盖封顶：1/2/3 类分别封顶 34/69/119）——
  --   所以模板必须**跨类**（自身 + 目标 + 技能 + 队伍各一个条件），单类堆量的模板会被封在稀有。
  --   每条 = 1 +（战斗中 1 + 可攻击 2 + 施法中 1 + 团队蓝量 3）= 8 分；16 条 = **128 ≥ 120**。
  local function txtGod(n)
    local s = "# 方案: 甲"
    for i = 1, n do s = s .. "\n- 技能" .. i .. " | 战斗中 & 可攻击 & 施法中 & 团队蓝量<30" end
    return s
  end
  local info = EVAL_SHARE_SEAL_INFO(txtGod(16), 10)
  eq(type(info) == "table", true, "整行算得出来")
  eq(info.score, 128, "整行评分 128（16 条 × 8 分）")
  eq(info.tierName, "神级", "整行品阶 = 神级（★1.74.19：≥120 分 **且** 覆盖 ≥4 个大类）")
  eq(info.coverage, 4, "★★整行报告的覆盖大类数 = 4（自身/目标/技能/队伍）")
  eq(info.capped, false, "★★覆盖 4 类 → **不封顶**（这才是能到神级的原因）")
  local curTitle143 = EVAL_TITLE_CURRENT()
  if curTitle143 then
    eq(type(curTitle143) == "table", true, "整行算得出来时**当前头衔也在**（有方案库就必然入档）")
    eq(string.find(info.line, curTitle143.color .. "[" .. curTitle143.name .. "]|r", 1, true) ~= nil, true,
       "★★★整行前缀 = 头衔（带该档颜色）：" .. tostring(curTitle143.name))
    eq(info.titleName, curTitle143.name, "★★读值口交出的头衔与整行一致")
  else
    -- ★诚实兜底：方案库为空（还没入档）→ 整行**不带**身份方括号，绝不用「无名小卒」之类顶上
    eq(EVAL_TITLE_STATE().tier, 0, "★方案库为空 → 0 档（还没入档）")
    eq(info.titleName, nil, "★★没有头衔时不编一个出来")
  end
  ...
  eq(string.find(info.line, "|cff00bfff", 1, true) ~= nil, true, "整行含**亮蓝**色码（神级档）")
  TEST.chat = ""
  ...
  eq(string.find(dc, "|cff00bfff", 1, true) ~= nil, true, "★样例里有**亮蓝**色码（神级档）")
  ...
  -- ★1.74.19 神级模板必须**跨类**（≥120 分 且 覆盖 ≥4 类）—— 见上面 txtGod；空技能模板现在只有 0 分
  local iSym = EVAL_SHARE_SEAL_INFO(txtGod(16), 10)
  ...
  print("  分享显示行：品阶评分边界(14/15·34/35·69/70·119/120) · 5 色(含**亮蓝**) · 每档 10 条评语随机 · 命令接线")
end
...
do
  ...
  EVAL_SHARE_RESET()
  local keepTitle144 = EVAL_HELP_CONFIG.title
  EVAL_HELP_CONFIG.title = { tier = 5, draws = { 1, 1, 1, 1, 1 } } -- ★本组测 A/B 行**格式**，需要一个确定的头衔（头衔怎么算在组 147/⑦b 测；1.74.5 起只算自创方案）
  ...
  TEST.chat = nil
  EVAL_HELP_CONFIG.title = keepTitle144
...
do
  ...
  -- ★1.74.19 每条技能带**跨类条件**（自身/目标/技能/队伍各一）⇒ 单条 8 分、覆盖 4 类（不封顶）。
  --   旧模板是「n 条技能 = n 分」，新口径下空技能 0 分，已经造不出高档位了。
  local function txt145(n)
    local s = "# 方案: 甲"
    for i = 1, n do s = s .. "\n- 技能" .. i .. " | 战斗中 & 可攻击 & 施法中 & 团队蓝量<30" end
    return s
  end
  -- ★自证式造样本：加条数直到**真的落在目标档**（不写死「几分对应第几档」，避免与阈值表形成两份真值）
  local function txtForTier145(want)
    for n = 1, 200 do
      local t = txt145(n)
      local parts = EVAL_SHARE_SEAL_PARTS(t)
      if parts and select(1, EVAL_SHARE_SEAL_TIER(parts.score, parts.nClasses)) == want then
        return t, parts.score
      end
    end
    return txt145(1), 0
  end
  -- ① 读值口（渲染与断言同源）
  local titleSample145 = tostring(EVAL_L(EVAL_TITLE_KEY(2, 1))) -- 真头衔（第 2 档第 1 张）：查得到颜色
  local r5 = EVAL_SHARE_SEAL_ROW(txtForTier145(5), { rank = titleSample145, comment = "天书原文，凡人勿近。" })
  eq(type(r5) == "table", true, "①★★品阶栏读值口算得出来")
  if r5 then
    eq(r5.tier, 5, "①★神级样本 = 第 5 档")
    eq(r5.icon, EVAL_SHARE_SEAL_ICON(5), "①★★★图标 = 该品阶的图标（同一来源，不另写一份）")
    eq(string.find(r5.head, "★[神级秘籍·甲]", 1, true) ~= nil, true, "①★★头一行 = 符号 + [品阶秘籍·名]")
    eq(string.find(r5.meta, "品阶：神级（评分 ", 1, true) ~= nil, true, "①★★含品阶与评分")
    eq(string.find(r5.meta, "覆盖 4/5 类", 1, true) ~= nil, true,
       "①★★★评分文案带**覆盖大类数**（★1.74.19 透明度：封顶规则必须看得见）：" .. tostring(r5.meta))
    ...
    eq(r5.rankColor, EVAL_TITLE_LOOKUP(titleSample145).color, "①★★颜色来源 = EVAL_TITLE_LOOKUP（与档位同源）")
    eq(string.find(r5.commentLine, "天书原文，凡人勿近。", 1, true) ~= nil, true, "①★★含发送端评语")
  end
  for i = 1, 5 do
    local tS, scS = txtForTier145(i)
    local ri = EVAL_SHARE_SEAL_ROW(tS, nil)
    eq(ri ~= nil and ri.tier == i, true, "①★档位与分数对应 " .. i .. "（评分 " .. tostring(scS) .. "）")
  ...
  local rowCustom145 = EVAL_SHARE_SEAL_ROW(txtForTier145(5), { rank = "创世之影", rankColor = "|cffff00ff", comment = "天书原文，凡人勿近。" })
  ...
  local rowOld145 = EVAL_SHARE_SEAL_ROW(txt145(25), { rank = titleSample145, comment = "天书原文，凡人勿近。" })
  ...
  local rHon = EVAL_SHARE_SEAL_ROW(txt145(25), nil)
  ...
  EVAL_PROFILE_TO_TEXT = function() return txt145(25) end -- ★只换**数据来源**，发送路径一行不改
  ...
  eq(table.getn(msgs145) >= 3, true, "③★25 分的方案 = 多片 + 封皮（" .. tostring(table.getn(msgs145)) .. " 条）")
  eq(msgs145[table.getn(msgs145)] == seal145, true, "③★★★B 品阶行是**最后一条**（用户：「方案分享最后一步 展示分享信息」）")
  ...
  eq(type(meta145), "nil", "③★★新格式下接收端不抠身份（" .. tostring(meta145) .. "）")
  ...
  eq(type(meta5) == "table" and tostring(meta5.rank or "") ~= "", true,
     "⑤★★★老格式封皮晚到时**数据照样解析**（" .. tostring(meta5 and meta5.rank) .. "）——只是不上屏")
...
-- 146) ★★★1.73.42j/n 分享文案迁进 Locales 三语言（50 条评语 + 5 品阶 + **75 个头衔**）
--   用户要求：文案走 L() 取键；色码与符号**留在 Share.lua**（那是协议不是文案）；三语言键必须齐。
--   ★为什么还要行为断言：源码检查 LANG KEY CHECK 只认 `L("字面量")`，而这里的键是**拼出来的**
--     （"SEAL_C"..档.."_"..序号 / "TITLE_"..档.."_"..序号）→ 它看不见这些键，必须由本组逐个语言亲自查一遍。
do
  ...
  for t = 1, 5 do for k = 1, 15 do table.insert(keys146, "TITLE_" .. t .. "_" .. k) end end
  eq(table.getn(keys146), 130, "①键总数 = 5 品阶 + 50 评语 + 75 头衔（" .. tostring(table.getn(keys146)) .. "）")
  for _, lg in ipairs(langs146) do
    ...
    eq(table.getn(miss), 0, "①★★★" .. lg .. " 130 个键一个都不缺（缺：" .. table.concat(miss, ",") .. "）")
  ...
  for tk146, tv146 in pairs(titleZh146) do eq(EVAL_LOCALES.zhCN[tk146], tv146, "②★头衔中文定稿 " .. tk146 .. " = " .. tv146) end
  local nTitle146, sameTitle146 = 0, 0
  for t = 1, 5 do
    for k = 1, 15 do
      local tk2 = "TITLE_" .. t .. "_" .. k
      nTitle146 = nTitle146 + 1
      local zhv, env, ruv = EVAL_LOCALES.zhCN[tk2], EVAL_LOCALES.enUS[tk2], EVAL_LOCALES.ruRU[tk2]
      if zhv == env or zhv == ruv or env == ruv then sameTitle146 = sameTitle146 + 1 end
    end
  end
  ...
  for _, lg in ipairs({ "enUS", "ruRU" }) do
    ...
    local tb = EVAL_LOCALES[lg]
    eq(EVAL_L("TITLE_3_1"), tb.TITLE_3_1, "③★★★头衔文案跟着语言（" .. lg .. "：" .. tostring(tb.TITLE_3_1) .. "）")
    eq(EVAL_TITLE_LOOKUP(tb.TITLE_3_1) ~= nil, true, "③★★该语言的头衔名也反查得到档位（回查走同一份语言包）")
    local _, ti146 = EVAL_SHARE_SEAL_TIER(EVAL_SEAL_GOD_SCORE()) -- ★1.74.19 神级门槛现算（原来写死 26）
    ...
    if info146 then
      ...
      local cur146 = EVAL_TITLE_CURRENT()
      if cur146 then eq(string.find(info146.line, tostring(cur146.name), 1, true) ~= nil, true, "③★★整行头衔用该语言文案") end
    end
  end
  ...
  eq(select(2, EVAL_SHARE_SEAL_TIER(EVAL_SEAL_GOD_SCORE())).color, "|cff00bfff", "⑤**亮蓝**色码留在源码（★1.74.22 用户要求改亮蓝）")
  TEST.chat = nil
  print("  分享文案三语言：130 键（5 品阶 + 50 评语 + 75 头衔）逐语言齐全 · 中文原文逐字校验 · 切语言读值口即时生效 · 符号/色码仍在源码")
end

-- 147) ★★★1.73.42n 头衔抽卡：按方案库稀有度晋升 · 每档**只抽一次** · **只升不降** · 彩蛋自定义优先且只有一次
--   用户定稿：「每个档位只抽卡一次.不重复抽卡.唯一不变.」「根据用户当前方案稀有度的数量, 达成档位的需求就晋升. 当我只升不降.」
do
  -- ① 晋升判定（纯函数，直接喂数量进来 → 结果可预测）
  -- ★★★1.74.19 晋升判据**重做**（用户：「增加评级难度」）：数量门槛 {1,2,3,3,2} + **逐级满足**。
  --   旧口径从 2 档到 5 档全是「稀有度 ≥ 该档的方案 ≥1 个」⇒ 头衔满档 = 造 1 个方案就够。
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 0, 0, 0, 0 }), 0, "①一个方案都没有 → 0 档（还没入档）")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 1, 0, 0, 0, 0 }), 1, "①1 个普通 → 1 档")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 1, 0, 0, 0 }), 1,
     "①★★★只有 1 个稀有 → **还是 1 档**（旧口径这里就给 2 档了 —— 这就是「增加难度」）")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 2, 0, 0, 0 }), 2, "①2 个稀有 → 2 档（稀有+ ≥2 达标）")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 0, 2, 0, 0 }), 2, "①★★2 个珍稀 → 仍停在 2 档（珍稀+ 要 ≥3）")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 0, 3, 0, 0 }), 3, "①3 个珍稀 → 3 档")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 0, 0, 2, 0 }), 2, "①★★2 个绝版 → 仍停在 **2 档**（珍稀+ 要 ≥3，逐级被挡）")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 0, 0, 3, 0 }), 4, "①3 个绝版 → 4 档")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 0, 0, 0, 2 }), 2,
     "①★★★2 个神级但前几档数量不够 → **逐级被挡**，只到 2 档（「有神级就满档」这条老路被堵死）")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 0, 0, 0, 1, 2 }), 5, "①★逐级都达标（稀有+≥2 · 珍稀+≥3 · 绝版+≥3 · 神级≥2）→ 5 档")
  eq(EVAL_TITLE_TIER_FROM_COUNTS({ 9, 0, 0, 0, 0 }), 1, "①★普通方案再多也不越档（档位看的是**稀有度**）")
  eq(EVAL_TITLE_REQ(5), 2, "①★门槛表可读：第 5 档要 2 个神级（命令提示读它，不另写一份）")
  eq(EVAL_TITLE_GE({ 2, 3, 0, 1, 4 }, 3), 5, "①「稀有度 ≥ 3」的方案数 = 5")
  -- ② 真实抽卡：换掉方案库（只换**数据来源**，逻辑全走真的）→ 抽一次 → 反复刷新绝不重抽
  local keepProfs147 = EVAL_HELP_CONFIG.war.profiles
  local keepTitle147 = EVAL_HELP_CONFIG.title
  -- ★★★1.74.19 造一个「**落在目标档**」的方案：每条技能带跨类条件（自身 战斗中 / 目标 可攻击 /
  --   技能 施法中 / 队伍 团队蓝量<30）⇒ 单条 = 1 + (1+2+1+3) = 8 分、覆盖 4 个大类（不封顶）。
  --   ★自证式：加条数直到**真的落在 want 档**（不写死「几条技能 = 第几档」，免得与阈值表形成两份真值）。
  --   ★旧写法 `prof147(score)` 是「N 条空技能 = N 分」—— 空技能现在 0 分，已经造不出来了。
  local function profForTier147(want)
    for n = 1, 200 do
      local p = { name = "T" .. tostring(want) .. "_" .. tostring(n), skills = {} }
      for i = 1, n do
        p.skills[i] = { skill = "技能" .. i, groups = {
          { { k = "combat", v = true } },
          { { k = "canAttack", v = true } },
          { { k = "casting" } },
          { { k = "teamMana", op = "<", n = 30 } },
      ...
      if select(1, EVAL_PROFILE_TIER(p)) == want then return p end
    end
    return { name = "T" .. tostring(want), skills = {} }
  end
  EVAL_HELP_CONFIG.title = nil
  -- ★1.74.19 2 档要「稀有及以上 ≥2」→ 得放两个稀有+ 的方案（旧口径一个就够）
  EVAL_HELP_CONFIG.war.profiles = { profForTier147(1), profForTier147(2), profForTier147(2) }
  eq(EVAL_TITLE_REFRESH(), 2, "②★★按方案库算到 2 档")
  local st147 = EVAL_TITLE_STATE()
  eq(type(st147.draws[1]) == "number" and type(st147.draws[2]) == "number", true, "②★★★两个新达到的档**各抽了一次**")
  eq(st147.draws[3], nil, "②★★没达到的档不抽（不预抽）")
  local d1, d2 = st147.draws[1], st147.draws[2]
  eq(d1 >= 1 and d1 <= 15 and d2 >= 1 and d2 <= 15, true, "②★抽到的序号在 1..15 内（每档卡池 15 张）")
  for i = 1, 5 do EVAL_TITLE_REFRESH() end
  st147 = EVAL_TITLE_STATE()
  eq(st147.draws[1] == d1 and st147.draws[2] == d2, true, "②★★★反复刷新**绝不重抽**（每档只抽一次 · 唯一不变）")
  eq(EVAL_TITLE_CURRENT().name, EVAL_L(EVAL_TITLE_KEY(2, d2)), "②★★当前头衔 = **最高档**抽到的那张")
  eq(EVAL_TITLE_CURRENT().tier, 2, "②★当前档位 = 2")
  -- ③ 只升不降：加方案 → 升档（新档抽一次，老档的卡一动不动）；把方案删光 → 不掉档、不换卡
  -- ★1.74.19 满档要**逐级**达标：稀有+≥2 · 珍稀+≥3 · 绝版+≥3 · 神级≥2
  EVAL_HELP_CONFIG.war.profiles = { profForTier147(2), profForTier147(4), profForTier147(5), profForTier147(5) }
  ...
  local d5 = st147.draws[5]
  EVAL_HELP_CONFIG.war.profiles = {}
  eq(EVAL_TITLE_REFRESH(), 5, "③★★★方案删光也**不掉档**（只升不降）")
  eq(EVAL_TITLE_STATE().draws[5], d5, "③★★头衔也不换（唯一不变）")
  eq(EVAL_TITLE_PROGRESS().nextTier, nil, "③★已封顶 → 没有下一档（进度口如实说 nil）")
  -- ④ 彩蛋预留：自定义头衔**优先度最高**，且**只有一次**修改机会
  eq(EVAL_TITLE_SET_CUSTOM("创世之影"), true, "④★★★彩蛋写入自定义头衔成功（第一次）")
  eq(EVAL_TITLE_CURRENT().name, "创世之影", "④★★★自定义头衔**压过**抽到的卡（优先度最高）")
  eq(EVAL_TITLE_CURRENT().custom, true, "④★标记为自定义（渲染可以据此换色/加标记）")
  eq(EVAL_TITLE_SET_CUSTOM("再改一次"), false, "④★★★**只有一次**修改机会（第二次被拒）")
  eq(EVAL_TITLE_CURRENT().name, "创世之影", "④★★拒绝后头衔不变")
  eq(EVAL_TITLE_SET_CUSTOM(""), false, "④★空名字也拒（不许把头衔清成空白）")
  eq(EVAL_TITLE_SET_CUSTOM("带|竖线"), false, "④★已用过 → 带竖线的这次也进不来（只有一次）")
  EVAL_TITLE_CLEAR_CUSTOM()
  eq(EVAL_TITLE_CURRENT().name, EVAL_L(EVAL_TITLE_KEY(5, d5)), "④收尾：清掉彩蛋后回到抽到的卡（诊断口）")
  -- ⑤ 色系：5 档头衔色与 5 档品阶色**逐个不相等**（用户：「不要和方案颜色相同」）
  local tierCols147 = {}
  local repsTier147 = {} -- ★1.74.19 每档代表分现算（原来写死 {3,9,15,21,26}）
  for i = 1, 5 do repsTier147[i] = EVAL_SEAL_TIER_SAMPLE(i) end
  for i = 1, 5 do
    local _, ti147 = EVAL_SHARE_SEAL_TIER(repsTier147[i])
    tierCols147[ti147.color] = true
  end
  local seenCol147, clash147 = {}, ""
  for i = 1, 5 do
    local look147 = EVAL_TITLE_LOOKUP(EVAL_L(EVAL_TITLE_KEY(i, 1)))
    eq(type(look147) == "table" and look147.tier == i, true, "⑤★按名字反查得到档位 " .. i)
    if look147 then
      eq(string.len(tostring(look147.color)) == 10, true, "⑤★头衔色必须 8 位：" .. tostring(look147.color))
      if tierCols147[look147.color] then clash147 = clash147 .. " 撞品阶:" .. look147.color end
      if seenCol147[look147.color] then clash147 = clash147 .. " 自撞:" .. look147.color end
      seenCol147[look147.color] = true
    end
  end
  eq(clash147, "", "⑤★★★头衔色与品阶色不许相同、头衔内部也不许重复（" .. clash147 .. "）")
  eq(EVAL_TITLE_LOOKUP("不是头衔的名字") == nil, true, "⑤★★认不出的名字给 nil（不猜档位、不编颜色）")
  -- ⑥ 评分单一来源：品阶与晋升档用的是**同一个**评分函数
  -- ★1.74.19 夹具要用**真的条件类型**（旧夹具拿数字 1/2/3 当条件，没有 .k ⇒ 新口径下算不出分）
  local fix147 = { skills = { { skill = "甲", groups = { { { k = "canAttack", v = true }, { k = "tHostile", v = true } } } } } }
  eq(EVAL_PROFILE_SCORE(fix147), 5,
     "⑥★★方案表评分 = 1 条技能 + 2 个目标类条件（各 2 分）= 1 + 2 + 2 = 5")
  eq(EVAL_SHARE_SEAL_SCORE("# 方案: 甲\n- 甲 | 可攻击 & 敌对"), EVAL_PROFILE_SCORE(fix147),
     "⑥★★★文本评分与表评分**同源**（同一个 EVAL_PROFILE_SCORE）")
  -- ⑦ 命令接线：/eh go 头衔（接线判据用**落盘字段**当证人，不玩「聊天含子串」那套——M336 教训）
  EVAL_HELP_CONFIG.title = nil
  TEST.chat = ""
  SlashCmdList["EVALHELP"]("go 头衔")
  eq(type(EVAL_HELP_CONFIG.title) == "table", true, "⑦★★★/eh go 头衔 真的执行了（以**落盘字段**为证）")
  eq(string.find(tostring(TEST.chat or ""), "头衔抽卡", 1, true) ~= nil, true, "⑦★★命令把五档抽取记录打出来了")
  TEST.chat = nil
  -- ⑦b ★★★1.74.5 头衔评定**只算自创方案**（用户：「只有自己创建的方案才加入评定分计算」）
  EVAL_HELP_CONFIG.title = nil
  local imp147 = profForTier147(5); imp147.src = "text" -- 一个神级，但来源是「导入/模版/分享」（text）
  EVAL_HELP_CONFIG.war.profiles = { imp147 }
  eq(EVAL_TITLE_COUNTS()[5], 0, "⑦b★★★导入来的神级**不算**进头衔评定")
  eq(EVAL_TITLE_REFRESH(), 0, "⑦b★库里全是导入方案 → 头衔 0 档（还没入档）")
  -- ★1.74.19 2 档要「稀有及以上 ≥2」→ 自创的也得放两个（旧口径一个就够）
  local self147a, self147b = profForTier147(2), profForTier147(2)
  EVAL_HELP_CONFIG.war.profiles = { imp147, self147a, self147b }
  eq(EVAL_TITLE_COUNTS()[2], 2, "⑦b★自创的稀有**算**（src=nil 按手动算）")
  eq(EVAL_TITLE_REFRESH(), 2, "⑦b★★有两个自创稀有 → 2 档（那个导入的神级**不顶上去**）")
  eq(EVAL_TITLE_COUNTS()[5], 0, "⑦b★导入的神级**始终不算**（哪怕和自创的并存）")
  -- ⑧ 还原现场（方案库与存档态都不是本次测试的私产）
  EVAL_HELP_CONFIG.war.profiles = keepProfs147
  EVAL_HELP_CONFIG.title = keepTitle147
  TEST.chat = nil
...
do
  ...
  -- ★1.74.19 造「手动创建的、落在某个**档**」的方案：每条技能带跨类条件
  --   （自身 战斗中 / 目标 可攻击 / 技能 施法中 / 队伍 团队蓝量<30）⇒ 单条 8 分、覆盖 4 类（不封顶）。
  --   ★旧写法是「26 条空技能 = 26 分 = 神级」—— 空技能现在 0 分，闸门那条「有一个神级」就造不出来了。
  --   ★自证式：加条数直到真的落在 wantTier 档（不写死几条 = 第几档）。
  local function manualProf150(nm, wantTier)
    ...
    for n = 1, 200 do
      local p = build(n)
      if select(1, EVAL_PROFILE_TIER(p)) == wantTier then return p end
    end
    return build(1)
  end
  EVAL_HELP_CONFIG.war = { activeProfile = 1, profiles = {
    manualProf150("神级甲", 5), manualProf150("乙", 1), manualProf150("丙", 1) } }
  ...
  local reps150 = {} -- ★1.74.19 每档代表分**现算**（原来写死 {3,9,15,21,26}：阈值一改就错档）
  for i = 1, 5 do reps150[i] = EVAL_SEAL_TIER_SAMPLE(i) end
  for i = 1, 5 do
...
do
  ...
  -- ★1.74.19 造「落在目标档」的方案（每条技能带跨类条件 ⇒ 单条 8 分、覆盖 4 类不封顶）；
  --   旧写法「N 条空技能 = N 分」在新口径下恒为 0 分，造不出 5 档。
  local function profForTier151(want)
    local function build(n)
      local p = { name = "R" .. tostring(want) .. "_" .. tostring(n), skills = {} }
      for i = 1, n do
        p.skills[i] = { skill = "技能" .. i, groups = {
          { { k = "combat", v = true } },
          { { k = "canAttack", v = true } },
  ...
  -- ★满档要**逐级**达标：稀有+≥2 · 珍稀+≥3 · 绝版+≥3 · 神级≥2
  EVAL_HELP_CONFIG.war.profiles = { profForTier151(2), profForTier151(4), profForTier151(5), profForTier151(5) }
  EVAL_HELP_CONFIG.title = nil
...
do
  local reps148 = {} -- ★1.74.19 每档代表分**现算**（原来写死 {3,9,15,21,26}：阈值一改就错档）
  for i = 1, 5 do reps148[i] = EVAL_SEAL_TIER_SAMPLE(i) end
  local okAll148 = true
  for i = 1, 5 do
  ...
  eq(EVAL_SHARE_SEAL_TIER_RGB(5).hex, "|cff00bfff", "①★神级档 = **亮蓝**（RGB 有据）")
...
do
  ...
  local scores158 = {} -- 五档各取一个评分
  for i = 1, 5 do scores158[i] = EVAL_SEAL_TIER_SAMPLE(i) end
  for i = 1, 5 do
...
do
  ...
  local sc159 = {} -- ★1.74.19 每档代表分**现算**（原来写死 {3,9,15,21,26}：阈值一改就错档）
  for i = 1, 5 do sc159[i] = EVAL_SEAL_TIER_SAMPLE(i) end
  for i = 1, 5 do
...
do
  ...
  -- ★★★1.74.19 夹具改成「**落在指定档**」：每条技能带**跨类条件**（自身 战斗中 / 目标 可攻击 /
  --   技能 施法中 / 队伍 团队蓝量<30）⇒ 单条 8 分、覆盖 4 个大类（不封顶）。
  --   ★旧夹具是「n 条技能各配 1 个可攻击」＝只覆盖 1 个大类 → 新口径下被封顶 34（稀有），
  --     无论加多少条都到不了神级 —— 那样「颜色/图标跟着品阶」这条判据就验不出来了。
  local function mk160(want)
    for n = 1, 200 do
      local s = "# 方案: 测160-" .. tostring(want) .. "-" .. tostring(n)
      for k = 1, n do
        s = s .. "\n- 技能" .. tostring(k) .. " | 战斗中 & 可攻击 & 施法中 & 团队蓝量<30"
      end
      local p = EVAL_PROFILE_FROM_TEXT(s)
      if select(1, EVAL_PROFILE_TIER(p)) == want then return p end
    end
    return EVAL_PROFILE_FROM_TEXT("# 方案: 测160-空")
  end
  -- 三个**不同品阶**（否则「颜色跟着品阶」验不出来）：普通 / 珍稀 / 神级
  EVAL_HELP_CONFIG.war = EVAL_HELP_CONFIG.war or {}
  ...
  for i = 1, 3 do
    ...
    -- ★1.74.19 判据与**生产同源**：走单入口 EVAL_PROFILE_TIER（带覆盖封顶）——
    --   只取分数会算出与界面不同的档位（判据自己就成了「另一份实现」）。
    local ti = select(1, EVAL_PROFILE_TIER(prof))
  ...
  local keepScore160 = EVAL_PROFILE_TIER
  EVAL_PROFILE_TIER = function() return nil end -- ★1.74.19 界面侧走的是单入口（见 uiProfileTierRGB）
  EVAL_WAR_TAB_REFRESH()
  ...
  EVAL_PROFILE_TIER = keepScore160
...
do
  ...
  -- ★★★1.74.19 夹具必须用**真的条件类型**（旧夹具拿数字 1..13 当条件 —— 没有 `.k` ⇒ 新口径下算不出分，
  --   两条方案都会落到 0 分、同档，「颜色跟着品阶换」这条判据就废了）。
  --   high164 要落在**神级**（≥120 分 **且** 覆盖 ≥4 个大类）：每条技能带跨类条件
  --   （自身 战斗中 / 目标 可攻击 / 技能 施法中 / 队伍 团队蓝量<30）⇒ 单条 8 分、覆盖 4 类（不封顶）。
  local function crossSkills164(n)
    local sk = {}
    for i = 1, n do
      sk[i] = { skill = "技能" .. i, groups = {
        { { k = "combat", v = true } },
        { { k = "canAttack", v = true } },
        { { k = "casting" } },
        { { k = "teamMana", op = "<", n = 30 } },
  ...
  local low164 = { name = "低配", skills = { { skill = "甲", groups = { { { k = "combat", v = true } } } } } } -- 1 + 1 = 2 分 → 普通
  local high164 = { name = "神档", skills = crossSkills164(15) } -- 15 条 × 8 = 120 分 · 覆盖 4 类 → 神级
  ...
  eq(v2164.tierColor ~= nil and near164(v2164.tierColor[1], rgbHigh164.r)
     ...
     "②★★颜色也跟着换（神级档 = 亮蓝 r=" .. tostring(rgbHigh164.r) .. " b=" .. tostring(rgbHigh164.b) .. "）")
  -- ③ 只改方案**内容**（激活方案没变）也要重算 —— 品阶不缓存
  local keepSkills164 = high164.skills
  high164.skills = { { skill = "乙", groups = { { { k = "combat", v = true } } } } } -- 120 分 → 2 分（降到普通）
  EVAL_HELP_UI_TICK()
  ...
  local keepScore164 = EVAL_PROFILE_TIER
  EVAL_PROFILE_TIER = nil -- ★1.74.19 界面侧走的是单入口
  EVAL_HELP_UI_TICK()
  ...
  EVAL_PROFILE_TIER = keepScore164
  EVAL_HELP_UI_TICK()
...
do
  ...
  --   ★1.74.5 起头衔**只算自创方案**（src ~= text）：夹具这份就是「玩家自创」，标 src="manual" 才算进头衔
  local prof166 = EVAL_PROFILE_FROM_TEXT("# 方案: 头衔166\n- 甲 | 可攻击\n- 乙 | 可攻击\n- 丙 | 可攻击")
  prof166.src = "manual"
  EVAL_HELP_CONFIG.war = { activeProfile = 1, profiles = { prof166 } }
  EVAL_HELP_CONFIG.title = { tier = 0, draws = {} }
...
do
  ...
  -- ★1.73.61 阈值改宽后重挑夹具：2 / 7 / 14 个技能（各 1 条条件）= 4 / 14 / 28 分 → 普通 / 珍稀 / 神级；第 2 个是当前激活
  EVAL_HELP_CONFIG.war = { activeProfile = 2, profiles = { mk167(2), mk167(7), mk167(14) } }
  EVAL_HELP_CONFIG.ui = { enabled = true, subCombat = true, subScheme = true }
  EVAL_HELP_UI_BUILD()
  if not EVAL_TEST_UI_SHOWN() then EVAL_HELP_UI_TOGGLE() end
  ...
  local keepScore167 = EVAL_PROFILE_TIER
  EVAL_PROFILE_TIER = function() return nil end -- ★1.74.19 界面侧走的是单入口
  EVAL_HELP_UI_TICK()
  ...
  EVAL_PROFILE_TIER = keepScore167
  -- ④ 换激活方案 → 下一个心跳底色跟着换（激活更亮）
  EVAL_HELP_CONFIG.war.activeProfile = 1
  EVAL_HELP_UI_TICK()
  local up167c = EVAL_TEST_UI_PROF()
  local p1 = EVAL_HELP_CONFIG.war.profiles[1]
  local rgb1 = EVAL_SHARE_SEAL_TIER_RGB(select(1, EVAL_PROFILE_TIER(p1))) -- ★1.74.19 同源单入口
...
do
  ...
  -- ★1.73.61 阈值改宽后（≤6/12/18/24/≥25），夹具也要选**跨档**的分数：
  --   9 条技能各带 1 个条件 = 18 分 → 珍稀（第 3 档）；④ 再改成 2 分 → 普通 ⇒ 两档不同、才验得出「跟着换」。
  --   ★1.74.5 起头衔**只算自创方案**（src ~= text）：夹具这份就是「玩家自创」，标 src="manual" 才算进头衔
  local prof168 = EVAL_PROFILE_FROM_TEXT("# 方案: 头衔168\n- 甲1 | 可攻击\n- 甲2 | 可攻击\n- 甲3 | 可攻击\n- 甲4 | 可攻击\n- 甲5 | 可攻击\n- 甲6 | 可攻击\n- 甲7 | 可攻击\n- 甲8 | 可攻击\n- 甲9 | 可攻击")
  prof168.src = "manual"
  EVAL_HELP_CONFIG.war = { activeProfile = 1, profiles = { prof168 } }
  EVAL_HELP_CONFIG.title = { tier = 0, draws = {} }
  ...
  profs168[1].skills = keep168 -- 18 分 → 回到珍稀档
  EVAL_HELP_ST_TICK()
...
do
  ...
  -- ★★★1.74.19 造「落在某个**档**」的手动方案：每条技能带**跨类条件**（自身 战斗中 / 目标 可攻击 /
  --   技能 施法中 / 队伍 团队蓝量<30）⇒ 单条 = 1 + (1+2+1+3) = 8 分、覆盖 4 个大类（不封顶）。
  --   ★旧写法是「n 条空技能 = n 分」—— 空技能现在 0 分，闸门那条「有一个神级」就造不出来了。
  --   7 条 = **56 分 ≥ 50 = 神级**（★1.74.21 用户定：神级门槛 50）。
  local KINDS169 = {
    { { k = "combat", v = true } },
    { { k = "canAttack", v = true } },
    { { k = "casting" } },
  ...
  local function mpN169(nm, n) -- n 条带跨类条件的技能（每条 8 分）
    local p = { name = nm, skills = {}, src = "manual" }
    for i = 1, n do p.skills[i] = { skill = "技能" .. i, groups = KINDS169 } end
    return p
  end
  local function mp169(nm) return mpN169(nm, 7) end -- ★1.74.21 神级=50 后改 7 条：7 × 8 = 56 ≥ 50 = 神级
  local function mpNear169(nm) return mpN169(nm, 6) end -- ★1.74.21 神级=50 后改 6 条：6 × 8 = 48 = 绝版（**差 2 分**到神级）
  ...
  set169({ mp169("甲"), mpN169("乙", 1), mpN169("丙", 1) }, 4)
  ...
  set169({ mpNear169("甲"), mpN169("乙", 1), mpN169("丙", 1) }, 5)
  eq(EVAL_TITLE_CREATOR_GATE().ok, false, "①★★★最高 48 分（差 2 分到神级 50）→ 闸门关")
  set169({ mp169("甲"), mpN169("乙", 1), mpN169("丙", 1) }, 5)
  eq(EVAL_TITLE_CREATOR_GATE().ok, true, "①★★★56 分 ≥ 50 = 神级，三条件齐 → 闸门开")
  set169({ mp169("甲"), mpN169("乙", 1) }, 5)
  eq(EVAL_TITLE_CREATOR_GATE().ok, false, "①★★★只有 2 个手动方案 → 闸门关（需 3）")
  set169({ mp169("甲"), mpN169("乙", 1), mpN169("丙", 1) }, 0)
  ...
  set169({ mp169("甲"), mpN169("乙", 1), mpN169("丙", 1), mpN169("丁", 1) }, 5)
  EVAL_HELP_CONFIG.war.profiles[1].src = "text"
  ...
  set169({ mpN169("甲", 5), mpN169("乙", 1) }, 2)
  EVAL_TITLE_CREATOR_CLOSE()
  EVAL_TITLE_CLEAR_CUSTOM()
  TEST.chat = nil
...
do
  ...
  for _, lg in ipairs({ "zhCN", "enUS", "ruRU" }) do
    ...
    for _, op in ipairs({ "IMP", "IGN" }) do
      ...
      -- ② ★★★1.74.5 用户：「取消彩蛋」——**这一条流程**（导入/忽略）不再发反应句；
    --   功能与三语言文案**按用户要求保留**（随后明确「不是把彩蛋功能删除」），只是**不再接线**。
    --   反向哨兵：点 [忽略] 之后，发出的东西里**不许**出现任何 SH_FUN_* 的句子（改成发场景描述）。
    local savedTitle170 = EVAL_HELP_CONFIG.title
    local keepProfs170 = EVAL_HELP_CONFIG.war.profiles
    EVAL_HELP_CONFIG.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 彩蛋哨兵\n- 致死打击 | 可攻击") }
    EVAL_HELP_CONFIG.war.activeProfile = 1
    TEST.runScripts = nil
    EVAL_SHARE_SEND("GUILD")
    ...
    for _, sc in ipairs(TEST.runScripts or {}) do
      local m = string.match(sc, 'SendChatMessage%("(.-)", "GUILD"%)')
      if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs170, m) end
    end
    EVAL_HELP_CONFIG.war.profiles = keepProfs170 -- 还原：收到的方案不算「我的」
    EVAL_HELP_CONFIG.war.activeProfile = 1
    EVAL_SHARE_RESET()
    TEST.inGuild = true
    for _, m in ipairs(msgs170) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_GUILD") end
    eq(EVAL_SHARE_PENDING() ~= nil, true, "②前置：收齐弹出")
    TEST.runScripts = nil
    ...
    for _, sc in ipairs(TEST.runScripts or {}) do
      for t = 1, 5 do
        for s = 1, 5 do
          for k = 1, 10 do
            local probe = string.sub(tostring(L170.SH_FUN_IGN[t][s][k] or ""), 1, 8)
            if probe ~= "" and string.find(sc, probe, 1, true) then leaked170 = sc end
          end
        end
      end
    end
    eq(leaked170 == nil, true, "②★★★取消彩蛋：点 [忽略] 后**不发**任何 SH_FUN_* 反应句（反向哨兵）")
    eq(tostring(EVAL_TEST_SHARE_SCENE_LAST() or "") ~= "", true, "②★改为发「场景描述」（内容与兜底见组 180）")
    TEST.inGuild = false
    EVAL_SHARE_RESET()
    EVAL_HELP_CONFIG.title = savedTitle170
    print("  彩蛋反应：三语言 50 格 × 10 条文案**保留**（结构判据见①）· 当前**未接线**（1.74.5 起导入/忽略改发场景描述）")
  end
...
do
  -- ★1.74.20 喂食助手的配置（开关/食物/喂食技能/图标位置）已改成**角色级存档**：
  --   本组碰的全部是角色键 ⇒ 统一走 EVAL_TB_CHAR_STORE()（下面所有 tb176.tb.x 已改）。
  local charSt176 = EVAL_TB_CHAR_STORE()
  -- 前置：显式建立（不靠别组的残留状态——「桩太宽松」是本项目老坑）
  EVAL_TB_CHAR_STORE().feedPet = false
  EVAL_TB_CHAR_STORE().hhFood, EVAL_TB_CHAR_STORE().hhFoodTex, EVAL_TB_CHAR_STORE().hhSpell = nil, nil, nil
  ...
  EVAL_TB_CHAR_STORE().feedPet = true
  EVAL_HH_TOGGLE()
  ...
  eq(EVAL_TB_CHAR_STORE().hhFood, "熏熊肉", "③★点下拉项 → 食物设置落配置（存的是**名字**）")
  eq(EVAL_TB_CHAR_STORE().hhFoodTex, "texmeat", "③★图标缓存一并记下（图标直接变成该食物）")
  ...
  eq(EVAL_TB_CHAR_STORE().hhFood, "魔法泉水", "⑩★/eh go 喂食 设 <食物名> 走真实命令入口能落配置")
  TEST.chat = nil
  ...
  -- ⑪ 图标随食物走
  EVAL_TB_CHAR_STORE().hhFood, EVAL_TB_CHAR_STORE().hhFoodTex = "熏熊肉", "texmeat"
  EVAL_HH_REFRESH()
  ...
  EVAL_TB_CHAR_STORE().feedPet = true
  -- ① 没有记忆位置 → 正中（独立算式：left = w/2 - 13、top = -(h/2 - 13)，13 = 26/2 —— 图标 1.74.5 起与配置入口同尺寸 26）
  EVAL_TB_CHAR_STORE().hhX, EVAL_TB_CHAR_STORE().hhY = nil, nil
  EVAL_TEST_HH_RESET_UI()
  EVAL_HH_TOGGLE()
  local g13a176 = EVAL_TEST_HH_GEOM()
  eq(g13a176 ~= nil and g13a176.left == 1024 / 2 - 13 and g13a176.top == -(768 / 2 - 13), true,
     "⑬★★默认位置 = 窗口**正中**（left=" .. tostring(g13a176 and g13a176.left) ..
     " top=" .. tostring(g13a176 and g13a176.top) .. "；期望 499 / -371）")
  -- ② 有记忆位置 → 按「中心偏移」还原（左移/上移都验，防方向写反）
  EVAL_TB_CHAR_STORE().hhX, EVAL_TB_CHAR_STORE().hhY = 60, -40
  EVAL_TEST_HH_RESET_UI()
  EVAL_HH_TOGGLE()
  local g13b176 = EVAL_TEST_HH_GEOM()
  -- ★1.74.5 图标边长 36 → 26（与配置入口按钮一致）⇒ 半个边长 18→13，期望值跟着变
  eq(g13b176 ~= nil and g13b176.left == 1024 / 2 + 60 - 13 and g13b176.top == -(768 / 2 - (-40) - 13), true,
     "⑬★★记住的位置按「中心偏移」还原（left=" .. tostring(g13b176 and g13b176.left) ..
     " top=" .. tostring(g13b176 and g13b176.top) .. "；期望 559 / -411）")
  -- ③ 越界回归：离谱的记忆 → 夹回屏幕内（四边都在屏幕里）
  EVAL_TB_CHAR_STORE().hhX, EVAL_TB_CHAR_STORE().hhY = 99999, -99999
  EVAL_TEST_HH_RESET_UI()
  EVAL_HH_TOGGLE()
  ...
  EVAL_TB_CHAR_STORE().feedPet = true
  EVAL_TB_CHAR_STORE().hhFood, EVAL_TB_CHAR_STORE().hhFoodTex = nil, nil
  ...
  -- ★★★1.74.5 用户：「选择框能否在点击框体外部自动关闭?」→ 全屏捕手（层比面板低一级）
  eq(g16a_176.catcherShown == true, true, "⑯★★面板打开 → 捕手已放下（点框外能关）")
  eq(g16a_176.catcherLevel ~= nil and g16a_176.frameLevel ~= nil and g16a_176.catcherLevel < g16a_176.frameLevel, true,
     "⑯★★捕手层必须**低于**面板（" .. tostring(g16a_176.catcherLevel) .. " < " .. tostring(g16a_176.frameLevel) .. "），否则面板自己也点不动")
  ...
  -- ★1.74.5 用户两条样式要求：① 去掉格子自绘边框（图标自带边框）② 图标尺寸 = **55 像素**
  eq(g16a_176.hasEdge == false, true, "⑯★★格子**不自绘边框**（用户：图标已经自带边框了）")
  -- ★1.74.5 用户（截图）：「打开选择框内的图标也太大了，和外层图标尺寸对齐」⇒ 格子=图标=26（与主图标/配置入口同口径）
  eq(g16a_176.iconW == 26 and g16a_176.iconH == 26, true,
     "⑯★★图标尺寸 = **26×26**（与外层悬浮图标/配置入口对齐）：" .. tostring(g16a_176.iconW) .. "×" .. tostring(g16a_176.iconH))
  eq(g16a_176.cellBox == 26 and g16a_176.cellBoxH == 26, true,
     "⑯★格子尺寸 = 图标尺寸（无自绘边框后视觉=点击区，量的是真实控件）")
  -- ★1.74.5 实发现：格子缩到 26 后面板只有 242 宽，底栏「[上翻][下翻] + [选技能][清除][关闭]」需要 278 →
  --   面板宽取 max(网格宽, 底栏最小宽)；这里用**真控件几何**钉住「左组不压右组、按钮不出面板」。
  eq(g16a_176.footDnRight ~= nil and g16a_176.footBLeft ~= nil
     and g16a_176.footDnRight <= g16a_176.footBLeft - 4, true,
     "⑯★★底栏左组不压右组（下翻右缘 " .. tostring(g16a_176.footDnRight) ..
     " ≤ 选技能左缘 " .. tostring(g16a_176.footBLeft) .. " − 4）")
  eq(g16a_176.footW ~= nil and g16a_176.footCloseRight ~= nil and g16a_176.footCloseRight <= g16a_176.footW - 4
     and g16a_176.footDnLeft >= 4, true,
     "⑯★底栏按钮全在面板内（关闭右缘 " .. tostring(g16a_176.footCloseRight) ..
     " ≤ 宽 " .. tostring(g16a_176.footW) .. " − 4；上翻左缘 " .. tostring(g16a_176.footDnLeft) .. "）")
  eq(g16a_176.footW ~= nil and g16a_176.footW >= (10 + 26 + 10) and g16a_176.footW <= 400, true,
     "⑯★面板宽合理（" .. tostring(g16a_176.footW) .. " = max(网格 242, 底栏 278)）")
  -- ★1.74.5 用户截图：「只有 2 件时下方一大片空白」→ 弹窗高度按**实际行数**自适应
  eq(g16a_176.rowsUsed == 1, true, "⑯★★2 件 → 只占 1 行（rowsUsed=" .. tostring(g16a_176.rowsUsed) .. "）")
  eq(g16a_176.frameH == 86, true,
     "⑯★★弹窗高度 = 1 行的高度（**绝对值夹具** 86 = 10+16+4+26+4+16+10），实测 " .. tostring(g16a_176.frameH))
  eq(g16a_176.frameH < g16a_176.hMax, true, "⑯★★比满格矮 → 不再留一大片空白")
  ...
  eq(okPick16_176 == true and EVAL_TB_CHAR_STORE().hhFood == g16a_176.first, true,
     "⑯★★点格子 = 设成喂食食物（" .. tostring(EVAL_TB_CHAR_STORE().hhFood) .. "）")
  eq(EVAL_TEST_HH_GRID().shown == false, true, "⑯★选完自动收起（不用再点关闭）")

  eq(EVAL_TEST_IG_STATE().catcherShown == false, true, "⑯★★收起后面板与**捕手一起**隐藏（绝不留一张吞掉整屏点击的透明件）")
  ...
  eq(g16b_176.rowsUsed == 5 and g16b_176.frameH == 198, true,
     "⑯★★45 件 → 满 5 行、高度回到上限 198（自适应是**双向**的；实测 " .. tostring(g16b_176.frameH) .. "）")
  ...
  -- ★★★走**真实 OnClick** 验「点框体外部自动关闭」
  eq(EVAL_TEST_IG_STATE().catcherShown == true, true, "⑯★★面板开着 → 捕手在")
  local okOut176 = EVAL_TEST_IG_OUTSIDE()
  eq(okOut176 == true and EVAL_TEST_HH_GRID().shown == false, true, "⑯★★★点框体外部 = **自动收起**（真实 OnClick）")
  eq(EVAL_TEST_IG_STATE().catcherShown == false, true, "⑯★★收起后捕手同时隐藏")
  EVAL_HH_GRID_HIDE()
  TEST.tipLines = nil

  -- ⑰ ★★用户实测回归：「开着一键喂食 → 把图标拖好 → /reload → 图标不见了」
  --   根因 = 开关只存状态，图标帧是「点开关那一下」才懒建的，**登录流程里没人重建它**。
  --   修法 = EvalHelp.lua 的 VARIABLES_LOADED 调 EVAL_HH_RESTORE（走真实事件入口验，不直调内部函数）。
  local uw17_176, uh17_176 = rawget(UIParent, "GetWidth"), rawget(UIParent, "GetHeight")
  rawset(UIParent, "GetWidth", function() return 1024 end)
  rawset(UIParent, "GetHeight", function() return 768 end)
  EVAL_TB_CHAR_STORE().feedPet = true
  EVAL_TB_CHAR_STORE().hhX, EVAL_TB_CHAR_STORE().hhY = 60, -40    -- 模拟「已经拖到过这里」（hhSavePos 存的就是这个口径）
  EVAL_TEST_HH_RESET_UI()                 -- 模拟 /reload：帧不存在了
  eq(EVAL_TEST_HH_STATE().built, false, "⑰前置：模拟 /reload 后帧确实不存在（懒建的前提）")
  local initF176 = rawget(_G, "EVAL_HELPInitFrame")
  eq(type(initF176) == "table", true, "⑰前置：登录初始化帧在（下面走**真实事件入口**）")
  local okEv176, fnEv176 = pcall(initF176.GetScript, initF176, "OnEvent")
  eq(okEv176 and type(fnEv176) == "function", true, "⑰前置：init 帧挂了 OnEvent")
  if okEv176 and type(fnEv176) == "function" then fnEv176("VARIABLES_LOADED") end
  local st17_176 = EVAL_TEST_HH_STATE()
  eq(st17_176.built == true, true, "⑰★★★VARIABLES_LOADED 之后图标**重建**了（修的就是「reload 后图标丢失」）")
  eq(st17_176.shown == true, true, "⑰★★重建后图标是**显示**的")
  local g17_176 = EVAL_TEST_HH_GEOM()
  eq(g17_176 ~= nil and g17_176.left == 1024 / 2 + 60 - 13 and g17_176.top == -(768 / 2 - (-40) - 13), true,
     "⑰★★★而且还在**拖动后的位置**（left=" .. tostring(g17_176 and g17_176.left) ..
     " top=" .. tostring(g17_176 and g17_176.top) .. "；期望 559 / -411）")
  -- 反向哨兵：开关关着 → reload 后**一个帧都不建**（这条修法不许把懒加载破坏掉）
  EVAL_TB_CHAR_STORE().feedPet = false
  EVAL_TEST_HH_RESET_UI()
  eq(EVAL_HH_RESTORE(), false, "⑰★开关关着 → 恢复函数直接返回 false（不建帧）")
  eq(EVAL_TEST_HH_STATE().built, false, "⑰★★关着时 reload 后**仍然不建帧**（懒加载成立）")
  -- 幂等：二次事件（PLAYER_ENTERING_WORLD）再恢复一次，位置不漂移
  EVAL_TB_CHAR_STORE().feedPet = true
  EVAL_HH_RESTORE()
  local g17b_176 = EVAL_TEST_HH_GEOM()
  EVAL_HH_RESTORE()
  local g17c_176 = EVAL_TEST_HH_GEOM()
  eq(g17b_176 ~= nil and g17c_176 ~= nil and g17b_176.left == g17c_176.left and g17b_176.top == g17c_176.top, true,
     "⑰★恢复是**幂等**的（二次事件重算位置不漂移：" .. tostring(g17c_176 and g17c_176.left) .. "）")
  rawset(UIParent, "GetWidth", uw17_176)
  rawset(UIParent, "GetHeight", uh17_176)
  EVAL_TEST_HH_RESET_UI()
...
-- ===== 组 177：消耗品助手（1.74.5 用户要求：「参照宠物喂食助手，添加个消耗品助手：多选、横向排列、
--   点击图标使用物品、下拉多选记住状态、重复点击需要选中」；用户对三处语义的选择见下）=====
do
  local fails0 = TESTASSERT_FAILS
  -- ★1.74.20 消耗品助手（含开关/选中列表/贴图缓存/图标位置）已改成**角色级存档**：
  --   本组只碰这些键 ⇒ 直接拿角色表，别再去拿账号表（拿错了就会「测试写账号、生产读角色」而假失败）。
  local tb177 = EVAL_TB_CHAR_STORE()
  local uw177, uh177 = rawget(UIParent, "GetWidth"), rawget(UIParent, "GetHeight")
  rawset(UIParent, "GetWidth", function() return 1024 end)
  rawset(UIParent, "GetHeight", function() return 768 end)
  TEST.time = 1000
  ...
  eq(EVAL_TEST_CH_STATE().built, false, "①★开关关着 → 不建任何帧（懒加载与喂食助手同一纪律）")
  -- ② 勾开关 → 主图标显示；没选中时横排是空的
  tb177.consumable = true
  EVAL_CH_TOGGLE()
  ...
  eq(table.getn(list177) == 2 and list177[1] == "厚皮药水" and list177[2] == "面包", true,
     "③★★多选按点击顺序记住（" .. table.concat(list177, ",") .. "）")
  ...
  --    点格子 = 选中/取消，且**不关面板**
  EVAL_TEST_CH_CLICK_MAIN("RightButton")
  local ig177 = EVAL_TEST_IG_STATE()
  eq(ig177 ~= nil and ig177.shown == true and ig177.key == "consumable" and ig177.mode == "multi", true,
     "⑦★★右键主图标 → 多选网格（key/mode=" .. tostring(ig177 and ig177.key) .. "/" .. tostring(ig177 and ig177.mode) .. "）")
  eq(ig177.selCount == 1, true, "⑦★已选中的那项带角标（selCount=" .. tostring(ig177.selCount) .. "）")

  eq(EVAL_TEST_IG_STATE().catcherShown == true, true, "⑦★消耗品面板打开 → 捕手也在（两个助手共用同一份实现）")
  EVAL_TEST_IG_CLICK(1)
  eq(EVAL_TEST_IG_STATE().shown == true, true, "⑦★★多选模式：点完格子面板**不关**（能连点）")
  eq(table.getn(EVAL_CH_LIST()) == 2, true, "⑦★点格子把第 1 项也选上了")
  EVAL_TEST_IG_CLICK(1)
  eq(table.getn(EVAL_CH_LIST()) == 1, true, "⑦★★重复点同一格 = **取消选中**（用户定的语义）")

  -- ⑦c ★点框体外部自动关闭（消耗品面板同样走共用件）
  EVAL_IG_HIDE() -- ★主图标是**开关**：先确保是关的，再右键开（否则「已开着 + 再右键」= 收起）
  EVAL_TEST_CH_CLICK_MAIN("RightButton")
  eq(EVAL_TEST_IG_STATE().shown == true and EVAL_TEST_IG_STATE().catcherShown == true, true, "⑦c★面板打开 + 捕手放下")
  EVAL_TEST_IG_OUTSIDE()
  eq(EVAL_TEST_IG_STATE().shown == false and EVAL_TEST_IG_STATE().catcherShown == false, true,
     "⑦c★★点框体外部 → 消耗品面板自动关闭（捕手一起收）")
  -- ⑦d 反向哨兵：多选**面板内**点格子不许被捕手关掉
  EVAL_TEST_CH_CLICK_MAIN("RightButton")
  EVAL_TEST_IG_CLICK(2)
  eq(EVAL_TEST_IG_STATE().shown == true, true, "⑦d★★面板内点格子**不关面板**（捕手在面板之下，抢不到）")
  EVAL_IG_HIDE()
  EVAL_CH_CLEAR()
  eq(table.getn(EVAL_CH_LIST()) == 0, true, "⑧★清除 = 清空全部选中")
  -- ⑨ 多选状态落盘 + 登录恢复（走**真实事件入口** EVAL_HELPInitFrame.OnEvent）
  EVAL_CH_TOGGLE_SELECT("面包")
  EVAL_TEST_CH_RESET_UI()
  local initF177 = rawget(_G, "EVAL_HELPInitFrame")
  local okEv177, fnEv177 = pcall(initF177.GetScript, initF177, "OnEvent")
  ...
  tb177.consumable = false
  EVAL_TEST_CH_RESET_UI()
  eq(EVAL_CH_RESTORE() == false and EVAL_TEST_CH_STATE().built == false, true,
     "⑩★★关着时 reload 后仍然不建帧（懒加载成立）")
  -- ⑪ 查不到物品 → 如实失败（不静默）
  tb177.consumable = true
  EVAL_CH_TOGGLE()
  EVAL_CH_TOGGLE_SELECT("不存在的东西")
  TEST.time = 3000 -- ★越过限频窗（限频判据在「找不找得到」**之前** —— 同一时刻点会先被限频拒掉，别把两件事搞混）
  local fail177 = EVAL_TEST_CH_STATE().fail
  EVAL_CH_USE("不存在的东西")
  eq(EVAL_TEST_CH_STATE().fail == fail177 + 1, true, "⑪★背包里找不到 → 失败计数 +1（如实报，不静默）")
  -- ⑫ ★★★用户：「在物品消耗完了之后显示图标灰色」
  --   用完/卖掉后物品从背包消失，但图标要**留着、灰着**（不是凭空不见，也不是还显示成正常白）
  tb177.consumable = true
  EVAL_CH_TOGGLE()
  tb177.chUse = {}
  tb177.chTex = {}
  ...
  eq(sg177.texShown == true and math.abs((sg177.gray or -1) - 0.35) < 0.001
     ...
     "⑫★★★横排那一枚同样**灰显**（三通道 0.35）")
  eq(sg177.found == false, true, "⑫★同时如实标出「不在了」（tooltip 写「已用完或不在背包」）")
  -- 补货 → 立刻恢复白显（灰不是一次性、永久的）
  TEST.bags = { [0 * 100 + 1] = { name = "厚皮药水", tex = "texpotion", count = 3 },
                [0 * 100 + 2] = { name = "面包", tex = "texbread", count = 5 } }
  EVAL_CH_STRIP_REFRESH()
  eq(EVAL_TEST_CH_STATE().mainGray == 1 and EVAL_TEST_CH_STRIP_INFO(1).gray == 1, true, "⑫★★补货后又变回白显")
  -- 反向哨兵：灰必须是「三通道同值」（偏色就不是灰）；且用完时点它仍如实失败
  TEST.bags = {}
  EVAL_CH_STRIP_REFRESH()
  local gr177 = EVAL_TEST_CH_STRIP_INFO(1)
  eq(gr177.gray == gr177.grayG and gr177.gray == gr177.grayB, true, "⑫★灰 = 三通道同值（纯灰，不是染色）")
  TEST.time = 4000
  local fail177b = EVAL_TEST_CH_STATE().fail
  EVAL_CH_USE("面包")
  eq(EVAL_TEST_CH_STATE().fail == fail177b + 1, true, "⑫★灰显时点它 → 仍如实报「使用失败：背包里找不到」")
  TEST.bags = { [0 * 100 + 1] = { name = "厚皮药水", tex = "texpotion", count = 3 },
                [0 * 100 + 2] = { name = "面包", tex = "texbread", count = 5 } }
  tb177.chUse = {}
  tb177.chTex = {}
  ...
  tb177.chUse = {}
  EVAL_TEST_CH_RESET_UI()
  rawset(UIParent, "GetWidth", uw177)
  rawset(UIParent, "GetHeight", uh177)
  TEST.time = nil
  if fails0 == TESTASSERT_FAILS then
    print("GROUP 177 (消耗品助手): PASS")
  end

-- ===== 组 178（1.74.5）：分享「不弹窗」的**每一条**分叉都要落进调试日志 =====
-- 用户：「现在为弹窗原因在日志内显示」——以前只有「弹窗 / 过大 / 自己的回声」三条落盘，
--   最容易踩的「接收开关关着」反而完全静默 ⇒ 报「对方就是不弹窗」时日志里查不到原因。
do
  local fails0 = TESTASSERT_FAILS
  local cfg178 = EVAL_HELP_CONFIG
  local keep178 = cfg178.war and cfg178.war.profiles
  cfg178.war = cfg178.war or {}
  cfg178.share = cfg178.share or {}
  local keepRecv178 = cfg178.share.recv
  -- 造一份**多片**方案（≥2 片），走真实发送路径拿回原文
  local lines178 = {}
  for k = 1, 24 do table.insert(lines178, "- 技能" .. tostring(k) .. " | 可攻击 & 敌对") end
  cfg178.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 日志测试\n" .. table.concat(lines178, "\n")) }
  cfg178.war.activeProfile = 1
  TEST.time = 5000
  TEST.runScripts = nil
  EVAL_SHARE_SEND("PARTY")
  EVAL_TEST_SHARE_DRAIN()
  local msgs178 = {}
  for _, sc in ipairs(TEST.runScripts or {}) do
    local m = string.match(sc, 'SendChatMessage%("(.-)", "PARTY"%)')
    if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs178, m) end
  end
  local function log178() return table.concat(EVAL_HELP_CONFIG.log or {}, "\n") end
  eq(table.getn(msgs178) >= 2, true, "前置：真实分片 ≥2（实际 " .. tostring(table.getn(msgs178)) .. " 片）")
  -- ① 接收开关关着（本轮补的：以前**完全静默**）
  EVAL_SHARE_RESET(); EVAL_HELP_CONFIG.log = {}
  cfg178.share.recv = false
  for _, m in ipairs(msgs178) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  eq(string.find(log178(), "不弹窗：接收开关关着", 1, true) ~= nil, true, "①★★★开关关着 → 日志写明原因（不再静默）")
  eq(EVAL_SHARE_PENDING() == nil, true, "①★对照：确实没弹")
  cfg178.share.recv = true
  -- ② i/n 非法
  EVAL_SHARE_RESET(); EVAL_HELP_CONFIG.log = {}
  EVAL_SHARE_ONMSG("|cff9ad4ff|HEHPF:aa 0/0:0|h[x]|h|r", "彩虹", "CHAT_MSG_PARTY")
  eq(string.find(log178(), "不弹窗：载荷不完整", 1, true) ~= nil, true, "②★i/n 非法 → 日志写明")
  -- ③ 分片数超上限（R4 整笔拒收）
  EVAL_SHARE_RESET(); EVAL_HELP_CONFIG.log = {}
  EVAL_SHARE_ONMSG("|cff9ad4ff|HEHPF:bb 1/999:0|h[x]|h|r", "彩虹", "CHAT_MSG_PARTY")
  eq(string.find(log178(), "不弹窗：分片数超上限", 1, true) ~= nil, true, "③★n>40 整笔拒收 → 日志写明")
  -- ④ hex 还原失败（奇数长度 hex）
  EVAL_SHARE_RESET(); EVAL_HELP_CONFIG.log = {}
  EVAL_SHARE_ONMSG("|cff9ad4ff|HEHPF:cc 1/1:4|h[x]|h|r", "彩虹", "CHAT_MSG_PARTY")
  eq(string.find(log178(), "不弹窗：hex 还原失败", 1, true) ~= nil, true, "④★坏 hex → 日志写明")
  -- ⑤ 正常收齐 → 写「弹出方案分享窗：来自谁 + 频道 + 片数」
  EVAL_SHARE_RESET(); EVAL_HELP_CONFIG.log = {}
  for _, m in ipairs(msgs178) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  eq(string.find(log178(), "弹出方案分享窗", 1, true) ~= nil
     and string.find(log178(), "彩虹", 1, true) ~= nil, true, "⑤★正常收齐 → 日志写明「弹窗 + 来自谁」")
  eq(EVAL_SHARE_PENDING() ~= nil, true, "⑤★对照：真的弹了")
  -- ⑥ 同一笔重复收齐（以前静默 → 会被误判成丢包）
  EVAL_HELP_CONFIG.log = {}
  for _, m in ipairs(msgs178) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  eq(string.find(log178(), "不弹窗：同一笔已弹过", 1, true) ~= nil, true, "⑥★重复收齐 → 日志写明（不是丢包）")
  -- ⑦ 暂未收齐（缓冲保留，晚到仍会弹） + ⑧ 超时丢弃
  EVAL_SHARE_RESET(); EVAL_HELP_CONFIG.log = {}
  TEST.time = 20000
  EVAL_SHARE_ONMSG(msgs178[1], "彩虹", "CHAT_MSG_PARTY") -- 只喂第 1 片
  TEST.time = 20005                                     -- ★空闲 5s：> 冗余窗 2s 但 ≤ 超时 60s（时间单位是「秒」，别给成 23000）
  EVAL_SHARE_ONMSG("|cff9ad4ff|HEHPF:dd 1/40:0|h[x]|h|r", "Zed", "CHAT_MSG_PARTY") -- 这条只为触发 sweep
  eq(string.find(log178(), "不弹窗：暂未收齐", 1, true) ~= nil, true, "⑦★超冗余窗没收齐 → 日志写明（缓冲仍在）")
  TEST.time = 20100                                     -- ★空闲 100s > 超时 60s
  EVAL_SHARE_ONMSG("|cff9ad4ff|HEHPF:ee 1/40:0|h[x]|h|r", "Zed", "CHAT_MSG_PARTY")
  eq(string.find(log178(), "不弹窗：超时丢弃", 1, true) ~= nil, true, "⑧★60 秒还没收齐 → 日志写明「超时丢弃」")
  -- ⑨ 在途笔数超上限（丢最久的一笔）
  EVAL_SHARE_RESET(); EVAL_HELP_CONFIG.log = {}
  TEST.time = 100000
  for k = 1, 6 do
    EVAL_SHARE_ONMSG("|cff9ad4ff|HEHPF:" .. string.format("%02x", k) .. " 1/3:0|h[x]|h|r",
                     "s" .. tostring(k), "CHAT_MSG_PARTY")
  end
  eq(string.find(log178(), "不弹窗：在途笔数超上限", 1, true) ~= nil, true, "⑨★在途超 4 笔 → 日志写明丢了最久的一笔")
  -- 收尾
  EVAL_SHARE_RESET()
  EVAL_HELP_CONFIG.log = {}
  cfg178.share.recv = keepRecv178
  cfg178.war.profiles = keep178
  TEST.time = nil
  if fails0 == TESTASSERT_FAILS then print("GROUP 178 (分享弹窗原因日志): PASS") end
end

-- ===== 组 179（1.74.5）：方案「作者 / 来源」四类 + 三个导入点 + 方案列表 tooltip =====
-- 用户：「给方案增加一个作者类型：自己（自己名字:别称）→ 自创 / 案例模版 → 技能学院 / 别人分享 → 玩家名；
--   记录方案来源，在方案列表 tooltip 显示方案基本信息（评级、作者等）」
do
  local fails0 = TESTASSERT_FAILS
  -- ① 四类标签（唯一实现 EVAL_PROF_AUTHOR_LABEL；含老存档兼容）
  local lSelf179 = EVAL_PROF_AUTHOR_LABEL({ name = "甲", skills = {}, src = "manual", author = "我自己" })
  local lOld179 = EVAL_PROF_AUTHOR_LABEL({ name = "甲", skills = {}, src = "manual" })
  local lTpl179 = EVAL_PROF_AUTHOR_LABEL({ name = "甲", skills = {}, src = "text", author = "技能学院" })
  local lShr179 = EVAL_PROF_AUTHOR_LABEL({ name = "甲", skills = {}, src = "text", author = "Ionol" })
  local lImp179 = EVAL_PROF_AUTHOR_LABEL({ name = "甲", skills = {}, src = "text" })
  -- ★★★用户澄清：「别称的意思就是方案内显示自创（实际值是自己名字）」⇒ 自创那一类**只显示「自创」**
  eq(string.find(lSelf179, "我自己", 1, true) == nil and string.find(lSelf179, "自创", 1, true) ~= nil, true,
     "①★★★自创只显示「自创」（不显示自己的名字）：" .. lSelf179)
  local pSelf179 = { name = "甲", skills = {}, src = "manual", author = "我自己" }
  eq(EVAL_PROF_AUTHOR_LABEL(pSelf179) == lSelf179 and pSelf179.author == "我自己", true,
     "①★★实际值仍记着自己的名字（数据保留），只是不上屏")
  eq(string.find(lOld179, "我自己", 1, true) == nil and string.find(lOld179, "案例模版", 1, true) == nil, true,
     "①★老存档（无 author）也算自创、不当成模版：" .. lOld179)
  eq(string.find(lTpl179, "技能学院", 1, true) ~= nil, true, "①★★案例模版 → 技能学院：" .. lTpl179)
  eq(string.find(lShr179, "Ionol", 1, true) ~= nil, true, "①★★别人分享 → 玩家名：" .. lShr179)
  eq(lImp179 ~= lSelf179 and lImp179 ~= lTpl179, true, "①★无作者（文本粘贴）≠ 自创/模版：" .. lImp179)
  eq(lSelf179 ~= lTpl179 and lTpl179 ~= lShr179 and lShr179 ~= lImp179 and lSelf179 ~= lImp179, true,
     "①★四类标签**互不相同**（语言无关的性质）")
  -- ② 三个导入点各自盖作者
  local cfg179 = EVAL_HELP_CONFIG
  local keep179 = cfg179.war and cfg179.war.profiles
  cfg179.war = cfg179.war or {}
  cfg179.war.profiles = {}
  EVAL_IMPORT_TEXT("# 方案: 粘贴甲\n- 甲 | 可攻击") -- 文本粘贴：不带作者
  local pImp179 = cfg179.war.profiles[table.getn(cfg179.war.profiles)]
  eq(pImp179.author == nil, true, "②★文本粘贴不带作者")
  eq(EVAL_PROF_AUTHOR_LABEL(pImp179) == lImp179, true, "②★粘贴进来的显示「导入」")
  EVAL_IMPORT_TEXT(EVAL_IO_TEMPLATES[1].list[1].text, "技能学院") -- 案例模版（与 IO 窗选单同一调用口径）
  local pTpl179 = cfg179.war.profiles[table.getn(cfg179.war.profiles)]
  eq(pTpl179.author == "技能学院" and pTpl179.src == "text", true, "②★★案例模版导入 → author=技能学院")
  eq(EVAL_PROF_AUTHOR_LABEL(pTpl179) == lTpl179, true, "②★模版导入后标签 = 案例模版：技能学院")
  -- 别人分享：走**真实**接收 + 真弹窗的 [导入] 按钮
  EVAL_SHARE_RESET()
  TEST.runScripts = nil
  cfg179.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 分享甲\n- 甲 | 可攻击 & 敌对\n- 乙 | 怒气>10") }
  cfg179.war.activeProfile = 1
  EVAL_SHARE_SEND("PARTY")
  EVAL_TEST_SHARE_DRAIN()
  local msgs179 = {}
  for _, sc in ipairs(TEST.runScripts or {}) do
    local m = string.match(sc, 'SendChatMessage%("(.-)", "PARTY"%)')
    if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs179, m) end
  end
  for _, m in ipairs(msgs179) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  eq(EVAL_SHARE_PENDING() ~= nil, true, "②前置：收到了别人的分享")
  EVAL_TEST_SHARE_CLICK_IMPORT()
  local pShr179 = cfg179.war.profiles[table.getn(cfg179.war.profiles)]
  eq(pShr179 ~= nil and pShr179.author == "彩虹", true,
     "②★★点弹窗 [导入] → author = 发送者名（实测 " .. tostring(pShr179 and pShr179.author) .. "）")
  -- ★别拿「不同作者名」的两个标签比（那是夹具自己写错）：按**性质**断（含发送者名 + 不是自创/模版/导入）
  local lShr179b = EVAL_PROF_AUTHOR_LABEL(pShr179)
  eq(string.find(lShr179b, "彩虹", 1, true) ~= nil and lShr179b ~= lSelf179 and lShr179b ~= lTpl179 and lShr179b ~= lImp179, true,
     "②★分享导入后标签带发送者名、且与其它三类不同：" .. lShr179b)
  -- ③ 方案列表 tooltip：方案名 + 评级 + 来源 + 技能数（走**真实 OnEnter**；换一组再比）
  -- ★夹具走**真解析器**（手搓 groups={{1}} 会被配置窗渲染行当成条件表 → EVAL_COND_STR 拿到数字当场报错，
  --   这是本项目已记录的坑，我又踩了一次）；src/author 在解析后再盖
  local function mk179(nm, src, author)
    local p = EVAL_PROFILE_FROM_TEXT("# 方案: " .. nm .. "\n- 致死打击 | 可攻击 & 就绪")
    p.src, p.author = src, author
    return p
  end
  cfg179.war.profiles = { mk179("自创甲", "manual", "我自己") }
  cfg179.war.activeProfile = 1
  EVAL_HELP_CFG_SETTAB(2)
  EVAL_WAR_TAB_REFRESH()
  local pb179 = EVAL_HELP_CFGWIN and EVAL_HELP_CFGWIN.profBtns and EVAL_HELP_CFGWIN.profBtns[1]
  eq(pb179 ~= nil, true, "③前置：拿得到方案行的真实按钮")
  local function tipOf179()
    TEST.tipLines = nil
    local ok, fn = pcall(pb179.GetScript, pb179, "OnEnter")
    if not (ok and type(fn) == "function") then return nil end
    fn()
    local out = {}
    for _, t in ipairs(TEST.tipLines or {}) do table.insert(out, tostring(t.text)) end
    TEST.tipLines = nil
    return out
  end
  local tip179 = tipOf179()
  eq(tip179 ~= nil, true, "③前置：方案行挂了 OnEnter")
  local joined179 = table.concat(tip179 or {}, "|")
  eq(string.find(joined179, "自创甲", 1, true) ~= nil, true, "③★tooltip 第一行 = 方案名")
  eq(string.find(joined179, "评级", 1, true) ~= nil, true, "③★★tooltip 有「评级」行（含分数）：" .. joined179)
  eq(string.find(joined179, "分", 1, true) ~= nil, true, "③★评级行带分数")
  eq(string.find(joined179, "来源", 1, true) ~= nil and string.find(joined179, "自创", 1, true) ~= nil
     and string.find(joined179, "我自己", 1, true) == nil, true,
     "③★★★tooltip 来源行 = 「自创」（不显示自己的名字）")
  eq(string.find(joined179, "个技能", 1, true) ~= nil, true, "③★tooltip 有技能数行")
  -- ★换一组再比（项目纪律：判据不能只认一组数据）：作者换成「技能学院」→ 来源行必须跟着变
  cfg179.war.profiles = { mk179("模版乙", "text", "技能学院") }
  EVAL_WAR_TAB_REFRESH()
  local tip179b = table.concat(tipOf179() or {}, "|")
  eq(string.find(tip179b, "技能学院", 1, true) ~= nil and string.find(tip179b, "案例模版", 1, true) ~= nil, true,
     "③★★换一组：来源行变成「案例模版：技能学院」：" .. tip179b)
  eq(string.find(tip179b, "我自己", 1, true) == nil, true, "③★换一组后旧作者名不再出现")
  -- 收尾
  cfg179.war.profiles = keep179
  EVAL_WAR_TAB_REFRESH()
  if fails0 == TESTASSERT_FAILS then print("GROUP 179 (方案作者/来源): PASS") end
end

-- ===== 组 180（1.74.5）：分享「场景描述」—— 接收/拒绝 × 有无目标 = 4 表 × 10 条；头衔按档位染色 =====
-- 用户：「在接收方案完成之后再加一句模版：<头衔> 根据当前所在<地点>、<有无目标> 做场景描述（背景=魔兽世界）」
--   + 「头衔根据等级染色」+ 「头衔/地点/目标 优先级」。
do
  local fails0 = TESTASSERT_FAILS
  local cfg180 = EVAL_HELP_CONFIG
  -- ① 4 张表：分派键 + 各 10 条 + 占位符个数（_IDLE=2 / _TGT=3）逐条核 + 无空条目
  local K180 = { { "imp", false, "SH_SCENE_IMP_IDLE", 2 }, { "imp", true, "SH_SCENE_IMP_TGT", 3 },
                 { "ign", false, "SH_SCENE_IGN_IDLE", 2 }, { "ign", true, "SH_SCENE_IGN_TGT", 3 } }
  for _, it in ipairs(K180) do
    local key, n = EVAL_TEST_SHARE_SCENE_KEY(it[1], it[2])
    eq(key == it[3], true, "①★分派键：" .. tostring(it[1]) .. "/" .. tostring(it[2]) .. " → " .. tostring(key))
    eq(n == 10, true, "①★★" .. key .. " 恰好 10 条（实测 " .. tostring(n) .. "）")
    local tb = EVAL_L(key)
    local badC, badEmpty = 0, 0
    for i = 1, n do
      local s = tostring((type(tb) == "table" and tb[i]) or "")
      local c = 0
      for _ in string.gmatch(s, "%%s") do c = c + 1 end
      if c ~= it[4] then badC = badC + 1 end
      if s == "" then badEmpty = badEmpty + 1 end
    end
    eq(badC == 0, true, "①★★" .. key .. " 每条恰好 " .. tostring(it[4]) .. " 个占位符（错 " .. tostring(badC) .. " 条）")
    eq(badEmpty == 0, true, "①★" .. key .. " 没有空条目")
  end
  -- ② 组成一条真实消息：走**真实接收 + 真弹窗 [导入] 按钮**（顺带证明「彩蛋反应照发 + 场景描述另入队」）
  local keep180 = cfg180.war and cfg180.war.profiles
  cfg180.war = cfg180.war or {}
  cfg180.share = cfg180.share or {}
  cfg180.share.recv = true
  cfg180.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 场景测试\n- 致死打击 | 可攻击 & 敌对") }
  cfg180.war.activeProfile = 1
  EVAL_SHARE_RESET()
  TEST.runScripts = nil
  EVAL_SHARE_SEND("PARTY")
  EVAL_TEST_SHARE_DRAIN()
  ...
  for _, sc in ipairs(TEST.runScripts or {}) do
    local m = string.match(sc, 'SendChatMessage%("(.-)", "PARTY"%)')
    if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs180, m) end
  end
  -- 头衔：给一个**有档位**的（档 2 = 稀有），色码来自 EVAL_TITLE_CURRENT().color（单一来源）
  cfg180.title = { tier = 2, draws = { 1, 1, 1, 1, 1 } }
  local tcur180 = EVAL_TITLE_CURRENT()
  eq(tcur180 ~= nil and type(tcur180.color) == "string", true, "②前置：当前头衔有色码（" .. tostring(tcur180 and tcur180.color) .. "）")
  TEST.paintZone = "暴风城"
  TEST.hasTarget = false
  TEST.curTargetName = nil
  EVAL_HELP_CONFIG.log = {}
  for _, m in ipairs(msgs180) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  eq(EVAL_SHARE_PENDING() ~= nil, true, "②前置：收到分享（有弹窗）")
  local idh180 = EVAL_SHARE_PENDING().idh -- ★点完之后 SH.pending 就清了：先取传输 id，供「头衔包链接」断言用
  TEST.runScripts = nil -- ★只统计「点击之后」发出的话：取消彩蛋后这里必须是 0 条
  EVAL_TEST_SHARE_CLICK_IMPORT() -- 点真按钮：导入 + 场景描述入队（彩蛋反应已按用户要求取消）
  local s180 = tostring(EVAL_TEST_SHARE_SCENE_LAST() or "")
  eq(s180 ~= "", true, "②★★★导入成功后生成了场景描述：" .. s180)
  eq(string.find(s180, tostring(tcur180.name), 1, true) ~= nil, true, "②★★场景描述里有**当前头衔**")
  eq(string.find(s180, tostring(tcur180.color), 1, true) ~= nil, true, "②★★★头衔**按档位染色**（用的是 EVAL_TITLE_CURRENT().color）")
  eq(string.find(s180, "暴风城", 1, true) ~= nil, true, "②★★场景描述里有**当前地点**（GetRealZoneText）")
  -- ★恰好一段色码（客户端会吞多段色码的消息 —— 本项目实测定案）
  local ccount180 = 0
  for _ in string.gmatch(s180, "|c") do ccount180 = ccount180 + 1 end
  eq(ccount180 == 1, true, "②★★整条恰好 **1 段色码**（多段会被客户端吞），实测 " .. tostring(ccount180))
  -- ★★★形态闸门：客户端**不画「有色码但没有链接」的消息**（本项目实测定案，用户实测就是这个原因）
  eq(string.find(s180, "|HEHPF:" .. tostring(idh180) .. " 0/1:0|h[" .. tostring(tcur180.name) .. "]|h|r", 1, true) ~= nil, true,
     "②★★★头衔包成**链接**（与封皮 A 行同款）：色码 + 链接才画得出来")
  -- ★★点场景描述里的头衔链接 → 打开刚收到的那份方案（同一个 EHPF 点击处理；
  --   取消彩蛋后这条覆盖从组 82 搬到这里 —— 它同时守着「场景描述的头衔链接真的可点」）
  eq(EVAL_SHARE_CLICK_OPEN("EHPF:" .. tostring(idh180)), true, "②★★点头衔链接 → 真的打开了接收页")
  eq(EVAL_SHARE_PENDING() ~= nil, true, "②★★接收页真的又弹出来了（SH.pending 重填）")
  -- ★★★1.74.5 用户「取消彩蛋」：导入不再直发反应句，**只把场景描述放进限频队列**（功能与文案保留，见 SH FUN CHECK）
  local log180 = table.concat(EVAL_HELP_CONFIG.log or {}, "\n")
  eq(string.find(log180, "已在PARTY频道发送", 1, true) == nil, true, "②★★★取消彩蛋：不再有「已在X频道发送」的反应句日志")
  eq(table.getn(TEST.runScripts or {}), 0, "②★★★点 [导入] 后 RunScript 0 条（没有同帧直发的东西）")
  eq(string.find(log180, "场景描述入队", 1, true) ~= nil, true, "②★★场景描述走**限频队列**（不与反应同帧倾泻）")
  -- ③ 有目标 → 换 _TGT 表，用目标名
  EVAL_SHARE_RESET(); TEST.runScripts = nil
  -- ★每步把方案库缩回 1 条：导入有 12 条上限，库满了 ok=false 就不会发场景描述（断言会误红）
  cfg180.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 载体" .. tostring(math.random(1000, 9999)) .. "\n- 致死打击 | 可攻击 & 敌对") }
  cfg180.war.activeProfile = 1
  cfg180.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 场景测试二\n- 致死打击 | 可攻击") }
  EVAL_SHARE_SEND("PARTY"); EVAL_TEST_SHARE_DRAIN()
  local msgs180b = {}
  for _, sc in ipairs(TEST.runScripts or {}) do
    local m = string.match(sc, 'SendChatMessage%("(.-)", "PARTY"%)')
    if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs180b, m) end
  end
  TEST.hasTarget = true
  TEST.curTargetName = "野猪"
  TEST.paintZone = "贫瘠之地"
  for _, m in ipairs(msgs180b) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  EVAL_TEST_SHARE_CLICK_IMPORT()
  local s180b = tostring(EVAL_TEST_SHARE_SCENE_LAST() or "")
  eq(string.find(s180b, "野猪", 1, true) ~= nil, true, "③★★有目标 → 用「有目标」表并把**目标名**填进去：" .. s180b)
  eq(string.find(s180b, "贫瘠之地", 1, true) ~= nil, true, "③★地点跟着变")
  -- ④ 拒绝分支 = 另一组文案（4 张表的另一半）
  EVAL_SHARE_RESET(); TEST.runScripts = nil
  -- ★每步把方案库缩回 1 条：导入有 12 条上限，库满了 ok=false 就不会发场景描述（断言会误红）
  cfg180.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 载体" .. tostring(math.random(1000, 9999)) .. "\n- 致死打击 | 可攻击 & 敌对") }
  ...
  for _, sc in ipairs(TEST.runScripts or {}) do
    local m = string.match(sc, 'SendChatMessage%("(.-)", "PARTY"%)')
    if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs180c, m) end
  end
  TEST.hasTarget = false
  TEST.paintZone = "藏宝海湾"
  for _, m in ipairs(msgs180c) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  local before180 = tostring(EVAL_TEST_SHARE_SCENE_LAST() or "")
  EVAL_TEST_SHARE_CLICK_IGNORE()
  local s180c = tostring(EVAL_TEST_SHARE_SCENE_LAST() or "")
  eq(s180c ~= before180 and string.find(s180c, "藏宝海湾", 1, true) ~= nil, true, "④★★拒绝分支用的是**另一组**文案：" .. s180c)
  -- ⑤ 兜底：没入档（无头衔）→ 用玩家名且**不染色**；地点取不到 → 「艾泽拉斯」
  cfg180.title = nil
  local realZone180 = GetRealZoneText
  ...
  for _, sc in ipairs(TEST.runScripts or {}) do
    local m = string.match(sc, 'SendChatMessage%("(.-)", "PARTY"%)')
    if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs180d, m) end
  end
  TEST.hasTarget = false
  for _, m in ipairs(msgs180d) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  EVAL_TEST_SHARE_CLICK_IMPORT()
  local s180d = tostring(EVAL_TEST_SHARE_SCENE_LAST() or "")
  eq(string.find(s180d, "测试玩家", 1, true) ~= nil, true, "⑤★★没入档 → 用**玩家名**顶头衔：" .. s180d)
  local cc180d = 0
  for _ in string.gmatch(s180d, "|c") do cc180d = cc180d + 1 end
  eq(cc180d == 0, true, "⑤★★★没档位就**不猜颜色**（0 段色码）")
  eq(string.find(s180d, "|HEHPF:", 1, true) == nil, true, "⑤★没档位时也不挂链接（退回纯文本，保证能显示）")
  eq(string.find(s180d, EVAL_L("SH_SCENE_ZONE_FALLBACK"), 1, true) ~= nil, true, "⑤★地点取不到 → 兜底地名")
  GetRealZoneText = realZone180
  -- ⑥ 目标在发话那一刻消失 → 整条不发（不拿别的名字硬塞）
  TEST.hasTarget = true
  local realUnitName180 = UnitName
  UnitName = function(u) if u == "target" then return nil end return realUnitName180(u) end
  EVAL_SHARE_RESET(); TEST.runScripts = nil
  -- ★每步把方案库缩回 1 条：导入有 12 条上限，库满了 ok=false 就不会发场景描述（断言会误红）
  cfg180.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 载体" .. tostring(math.random(1000, 9999)) .. "\n- 致死打击 | 可攻击 & 敌对") }
  cfg180.war.activeProfile = 1
  ...
  for _, sc in ipairs(TEST.runScripts or {}) do
    local m = string.match(sc, 'SendChatMessage%("(.-)", "PARTY"%)')
    if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs180e, m) end
  end
  for _, m in ipairs(msgs180e) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  local before180e = tostring(EVAL_TEST_SHARE_SCENE_LAST() or "")
  local qlen180 = EVAL_SHARE_TEST_QUEUE_LEN()
  EVAL_TEST_SHARE_CLICK_IMPORT()
  eq(tostring(EVAL_TEST_SHARE_SCENE_LAST() or "") == before180e and EVAL_SHARE_TEST_QUEUE_LEN() == qlen180, true,
     "⑥★★目标消失 → 这一条**不发**（也不塞别的名字）")
  UnitName = realUnitName180
  -- ⑦ 拿不到传输 id → 退回纯文本（既不上色也不挂链接）：宁可朴素，也**必须看得见**
  TEST.hasTarget = false
  EVAL_SHARE_RESET(); TEST.runScripts = nil
  cfg180.war.profiles = { EVAL_PROFILE_FROM_TEXT("# 方案: 载体无id" .. tostring(math.random(100, 999)) .. "\n- 致死打击 | 可攻击") }
  cfg180.war.activeProfile = 1
  cfg180.title = { tier = 2, draws = { 1, 1, 1, 1, 1 } }
  EVAL_SHARE_SEND("PARTY"); EVAL_TEST_SHARE_DRAIN()
  local msgs180f = {}
  for _, sc in ipairs(TEST.runScripts or {}) do
    local m = string.match(sc, 'SendChatMessage%("(.-)", "PARTY"%)')
    if m and string.find(m, "|HEHPF:", 1, true) then table.insert(msgs180f, m) end
  end
  for _, m in ipairs(msgs180f) do EVAL_SHARE_ONMSG(m, "彩虹", "CHAT_MSG_PARTY") end
  if EVAL_SHARE_PENDING() then EVAL_SHARE_PENDING().idh = nil end -- 模拟极老格式（没有传输 id）
  EVAL_TEST_SHARE_CLICK_IMPORT()
  local s180f = tostring(EVAL_TEST_SHARE_SCENE_LAST() or "")
  eq(string.find(s180f, "|HEHPF:", 1, true) == nil and string.find(s180f, tostring(tcur180.name), 1, true) ~= nil, true,
     "⑦★★拿不到传输 id → 纯文本兜底（无链接、无染色，但能显示）：" .. s180f)
  cfg180.title = nil
  -- 收尾
  EVAL_SHARE_RESET()
  cfg180.war.profiles = keep180
  cfg180.title = nil
  TEST.hasTarget = nil
  TEST.curTargetName = nil
  TEST.paintZone = nil
...
print("ALL TESTS PASS")
print("ALL TESTS PASS")
-- 181) ★★★1.74.6 自身/目标 debuff 支持「负面类型」（用户：「查看自身/目标debuff 条件类型配置项，
--   需要参考队伍debuff 配置支持 debuff 负面类型功能」）
--   与队伍debuff **同一套写法、同一份格式化**：① 解析「名(类型)」/「类型」单独写 ② 求值按类型过滤
--   （名字对上但类型不符 = 没有）③ 名称留空 + 类型 = 「有任意该类型」 ④ 导出走 token / 显示走本地化名
--   ⑤ 编辑窗真的出现类型格（自身/目标两行都要有，buff 行不许有）。
do
  local keep181 = { tex = EVAL_HELP_CONFIG.war.debuffTex, debuffs = TEST.debuffs, hasTarget = TEST.hasTarget }
  -- ① 解析（导入宽松：括号里可以是英文 token、本地化名、斜杠/顿号分隔的一串）
  local c1 = EVAL_PARSE_CONDS("自身debuff:减速(魔法)")[1][1]
  eq(c1.k == "pDebuff" and c1.s == "减速" and c1.dt == "Magic", true,
     "①★★自身debuff 能带类型（got s=" .. tostring(c1.s) .. " dt=" .. tostring(c1.dt) .. "）")
  local c2 = EVAL_PARSE_CONDS("有debuff:减速(魔法/毒) >=2")[1][1]
  eq(c2.k == "hasDebuff" and c2.n == 2 and type(c2.dt) == "table" and c2.dt.Magic == true and c2.dt.Poison == true, true,
     "①★★目标debuff：类型集合 + 层数门槛共存（got n=" .. tostring(c2.n) .. " dt=" .. type(c2.dt) .. "）")
  local c3 = EVAL_PARSE_CONDS("自身debuff:魔法")[1][1]
  eq(c3.k == "pDebuff" and (c3.s == nil or c3.s == "") and c3.dt == "Magic", true,
     "①★★★类型单独写 = 「任意该类型」（名字留空）：s=" .. tostring(c3.s) .. " dt=" .. tostring(c3.dt))
  -- ② 存量写法逐字不变（反向哨兵：老配置导出不能被动过）
  eq(EVAL_GROUP_STR(EVAL_PARSE_CONDS("自身debuff:减速")), "自身debuff:减速", "②★★存量「自身debuff:名」导出逐字不变")
  eq(EVAL_GROUP_STR(EVAL_PARSE_CONDS("自身debuff:减速>=2")), "自身debuff:减速>=2", "②★存量带层数写法不变")
  eq(EVAL_GROUP_STR(EVAL_PARSE_CONDS("有debuff:减速")), "有debuff:减速", "②★目标 debuff 老写法不变")
  eq(EVAL_GROUP_STR(EVAL_PARSE_CONDS("无debuff:减速<3")), "无debuff:减速<3", "②★取反老写法不变")
  -- ③ 求值：真实状态表（减速 2 层、类型 = 诅咒）
  EVAL_HELP_CONFIG.war.debuffTex = { ["减速"] = "texSlow181" }
  TEST.debuffs = { { tex = "texSlow181", apps = 2, type = "Curse" } }
  TEST.hasTarget = true -- ★目标 debuff 扫描要 UnitExists("target") 为真（组 180 收尾把它清成 nil 了）
  EVAL_HELP_UPDATE_STATE()
  eq(EVAL_HELP_STATE.playerDebuffType["texSlow181"], "Curse", "③前置：状态层记下了自身 debuff 的类型（桩如实返回第 3 值）")
  eq(EVAL_HELP_STATE.targetDebuffType["texSlow181"], "Curse", "③前置：目标侧同样记下")
  local function ev181(t)
    local g = EVAL_PARSE_CONDS(t)
    return EVAL_TEST_COND_EVAL_LIVE(g[1][1])
  end
  eq(ev181("自身debuff:减速(诅咒)"), true, "③★★类型相符 → 命中")
  eq(ev181("自身debuff:减速(魔法)"), false, "③★★★名字对上了但类型不符 → 不命中（本次功能的核心）")
  eq(ev181("自身debuff:减速(魔法/诅咒)"), true, "③★★集合里命中任一即为真")
  eq(ev181("自身debuff:减速"), true, "③★没写类型 = 任意（老写法行为一字不变）")
  eq(ev181("自身debuff:诅咒"), true, "③★★★名称留空 + 类型 = 「有任意该类型」")
  eq(ev181("自身debuff:魔法"), false, "③★★名称留空 + 别的类型 → 不命中")
  eq(ev181("自身debuff:诅咒>=2"), true, "③★★名称留空 + 类型 + 层数门槛（2 层够）")
  eq(ev181("自身debuff:诅咒>=3"), false, "③★层数不够 → 不命中")
  eq(ev181("有debuff:减速(魔法)"), false, "③★★目标 debuff 同样按类型过滤")
  eq(ev181("有debuff:减速(诅咒)"), true, "③★★目标 debuff 类型相符 → 命中")
  eq(ev181("无debuff:减速(魔法)"), true, "③★取反方向：类型不符 → 「无」成立")
  eq(ev181("无debuff:减速(诅咒)"), false, "③★取反方向：类型相符 → 「无」不成立")
  -- ④ 导出/显示同一份格式化（token ↔ 本地化名）+ 往返稳定
  local g4 = EVAL_PARSE_CONDS("自身debuff:减速(魔法/毒)")
  eq(EVAL_GROUP_STR(g4), "自身debuff:减速(Magic/Poison)", "④★★导出走英文 token：" .. tostring(EVAL_GROUP_STR(g4)))
  eq(string.find(EVAL_GROUP_STR(g4, true), "魔法", 1, true) ~= nil, true, "④★★界面显示走本地化名")
  eq(EVAL_GROUP_STR(EVAL_PARSE_CONDS(EVAL_GROUP_STR(g4))), EVAL_GROUP_STR(g4), "④★★往返稳定（导出→解析→导出逐字相同）")
  eq(EVAL_GROUP_STR(EVAL_PARSE_CONDS("自身debuff:(魔法)")), "自身debuff:(Magic)", "④★名称留空形态也能往返")
  eq(EVAL_GROUP_STR(EVAL_PARSE_CONDS("有debuff:减速(魔法/毒)")), "有debuff:减速(Magic/Poison)", "④★目标侧同款（这次还顺手补上了 candDebuff 漏掉的类型后缀）")
  -- ⑤ 编辑窗：真实类型格（自身/目标两行都要有；buff 行不许有）
  EVAL_TEST_SE_CLEAR()
  eq(EVAL_TEST_SE_PUSH_COND("pDebuff", "减速", nil, nil, "Magic"), true, "⑤前置：编辑器里放一条「自身debuff」")
  local sh5, tx5 = EVAL_TEST_SE_ROW_DT(1)
  eq(sh5 == true, true, "⑤★★★自身debuff 行出现了类型格（用户要求的正是这个）")
  eq(tx5, EVAL_L("DS_T_MAGIC"), "⑤★格子文案 = 魔法")
  eq(EVAL_TEST_SE_COND_DT(1) == "Magic", true, "⑤★数据侧 dt 真的存住了")
  EVAL_TEST_SE_CLEAR()
  eq(EVAL_TEST_SE_PUSH_COND("hasDebuff", "减速", nil, nil, nil), true, "⑤前置：目标debuff 行")
  eq((EVAL_TEST_SE_ROW_DT(1)) == true, true, "⑤★★★目标debuff 行也有类型格")
  eq(EVAL_TEST_SE_COND_DT(1) == nil, true, "⑤★默认「任意负面」（dt=nil）")
  EVAL_TEST_SE_CLEAR()
  eq(EVAL_TEST_SE_PUSH_COND("hasBuff", "战斗怒吼", nil, nil, nil), true, "⑤前置：buff 行")
  eq((EVAL_TEST_SE_ROW_DT(1)) == false, true, "⑤★buff 行不出现类型格（不误放）")
  -- 收尾
  EVAL_DD_HIDE()
  EVAL_TEST_SE_CLEAR()
  EVAL_HELP_CFG_SETTAB(1)
  EVAL_HELP_CONFIG.war.debuffTex = keep181.tex
  TEST.debuffs = keep181.debuffs
  TEST.hasTarget = keep181.hasTarget
  EVAL_HELP_UPDATE_STATE()
...
do
  -- ★1.74.20 骑乘助手的配置已改成**角色级存档**（同组 176 的理由）。
  local charSt182 = EVAL_TB_CHAR_STORE()
  -- 前置：显式建立（「桩太宽松」是本项目老坑：不靠别组的残留状态）
  EVAL_TB_CHAR_STORE().dismount, EVAL_TB_CHAR_STORE().dismountAuto = false, false
  EVAL_TB_CHAR_STORE().dhX, EVAL_TB_CHAR_STORE().dhY = nil, nil
  ...
  EVAL_DH_TEST_RESET()
  TEST.time = 9500
  EVAL_TB_CHAR_STORE().dismountAuto = true
  ...
  EVAL_TB_CHAR_STORE().dismountAuto = false
  TEST.dhCancel = {}
  EVAL_DH_TEST_RESET()
  TEST.time = 9700
  ...
  EVAL_TB_CHAR_STORE().dismount = true
  EVAL_DH_ENSURE()
  TEST.dhBuffs = { [1] = "坐骑（速度提高60%）" }
  TEST.dhSlotOf = { [0] = 1 }
  TEST.dhCancel = {}
...
do
  ...
  -- ② 消耗品：有 CD 的物品按真实 CD
  EVAL_HELP_CONFIG.tb = EVAL_HELP_CONFIG.tb or {}
  EVAL_TB_CHAR_STORE().consumable = true
  EVAL_TB_CHAR_STORE().chUse = { "法力药水" }
  ...
  -- 收尾
  EVAL_TB_CHAR_STORE().consumable = false
  EVAL_TB_CHAR_STORE().feedPet = false
  EVAL_TB_CHAR_STORE().chUse = nil
  EVAL_TB_CHAR_STORE().hhFood, EVAL_TB_CHAR_STORE().hhFoodTex = nil, nil
  EVAL_HH_TEST_RESET_TIMERS()
  TEST.bags, TEST.spellbook, TEST.hasPet, TEST.targeting = nil, nil, nil, nil
...
do
  EVAL_HELP_CONFIG.tb = EVAL_HELP_CONFIG.tb or {}
  EVAL_TB_CHAR_STORE().consumable = true
  EVAL_TB_CHAR_STORE().chUse = { "面包", "法力药水" }
...
do
  EVAL_HELP_CONFIG.tb = EVAL_HELP_CONFIG.tb or {}
  EVAL_TB_CHAR_STORE().consumable = true
  EVAL_TB_CHAR_STORE().chUse = { "法力药水", "面包" }
  TEST.bags = { [0 * 100 + 1] = { name = "法力药水", tex = "texMana", count = 3, cd = true },
                [0 * 100 + 2] = { name = "面包", tex = "texBread", count = 5 } }
  TEST.time = 5000
...
-- ===== 组 194（1.74.17）：技能列表「上一页/下一页」搬到**底部导航行**（用户：「查看系统记忆和图标库的
--   滚动条实现方式，将技能列表的滚动条方式调整下。在有滚动条的时候上一页和下一页按钮放置在底部
--   案例模版按钮左侧、右对齐」）=====
-- 覆盖：①没溢出 → 两个按钮**隐藏**（没有滚动条就不该出现）
--   ②溢出 → 两个按钮**显示**（这就是「有滚动条」的可视信号）
--   ③★位置：整组右缘停在 [案例模版] **左边**，间隙 == 生产自报的 tailGap
--   ④★同一行：与 [案例模版] **同顶边同高**（比「中线对齐」更强——同高就不存在错位空间）
--   ⑤★在窗口内；两枚不重叠
--   ⑥★**整页步进**：点一下真实 OnClick 走一整页（= 可见行数），不是一行
--   ⑦反向哨兵：旧实现那两个「列表右栏 TOPRIGHT 的 ▲/▼」**不许**再存在
do
  local savedWar194 = EVAL_HELP_CONFIG.war
  local wasVisible194 = EVAL_TEST_CFG_VISIBLE()
  local function mkSkills194(n)
    local t = {}
    for i = 1, n do t[i] = { skill = "技能" .. tostring(i), enabled = true, groups = {} } end
    return t
  end
  if not wasVisible194 then pcall(EVAL_HELP_CFG_TOGGLE) end
  EVAL_HELP_CFG_SETTAB(2) -- 一键宏 Tab（滚动按钮的显隐只在它激活时动）
  local rowsN194 = EVAL_TEST_WAR_ROWS().n
  eq(type(rowsN194) == "number" and rowsN194 > 1, true,
    "前置：拿到技能列表真实槽位数（" .. tostring(rowsN194) .. "）")

  -- ① 没溢出 → 两个按钮都隐藏
  EVAL_HELP_CONFIG.war = { activeProfile = 1, profiles = { { name = "翻页甲", skills = mkSkills194(2) } } }
  EVAL_WAR_TAB_REFRESH()
  local s1_194 = EVAL_TEST_WAR_SCROLL()
  eq(s1_194.on, false, "①★技能数 ≤ 槽位数 → 生产自报「没有滚动条」")
  eq(s1_194.up.shown == false and s1_194.dn.shown == false, true,
    "①★★没有滚动条 → [上一页]/[下一页] **都不显示**（不是永远挂在底部）")

  -- ② 溢出 → 两个按钮出现（★文案是按钮自己的 FontString，也要一起 Show）
  EVAL_HELP_CONFIG.war = { activeProfile = 1, profiles = { { name = "翻页乙", skills = mkSkills194(rowsN194 * 3) } } }
  EVAL_WAR_TAB_REFRESH()
  local s2_194 = EVAL_TEST_WAR_SCROLL()
  eq(s2_194.on, true, "②★技能数 > 槽位数 → 生产自报「有滚动条」")
  eq(s2_194.up.shown == true and s2_194.dn.shown == true, true,
    "②★★有滚动条 → [上一页]/[下一页] **出现**（★按钮本体显示）")
  local upTxtShown194 = false
  do
    local fs = s2_194.up.text
    if fs then
      local oks, sv = pcall(fs.IsShown, fs)
      upTxtShown194 = (oks and sv) and true or false
    end
  end
  eq(upTxtShown194, true, "②★★按钮上的**文字**同样显示（显式清单里按钮与文字一起 Show/Hide，不是空壳）")
  eq(type(s2_194.up.label) == "string" and s2_194.up.label ~= "", true,
    "②★有真文本：" .. tostring(s2_194.up.label) .. " / " .. tostring(s2_194.dn.label))
  eq(s2_194.up.hasClick and s2_194.dn.hasClick, true, "②★两个按钮都挂了真实 OnClick")

  -- ③④⑤ 位置（读 [案例模版] 的**真实几何**，不写死坐标）
  local nav194 = EVAL_TEST_CFG_NAV()
  local tpl194 = nav194 and nav194[1]
  eq(tpl194 ~= nil and type(tpl194.x) == "number" and type(tpl194.y) == "number", true,
    "③前置：拿到 [案例模版] 的真实几何")
  local tplH194 = nil
  do
    local okh, hh = pcall(tpl194.btn.GetHeight, tpl194.btn)
    if okh and type(hh) == "number" then tplH194 = hh end
  end
  eq(type(tplH194) == "number", true, "③前置：拿到 [案例模版] 的真实高度（" .. tostring(tplH194) .. "）")
  eq(s2_194.dn.x + s2_194.dn.w <= tpl194.x - s2_194.tailGap + 0.5, true,
    "③★★★整组右缘停在 [案例模版] **左边**（右缘 " .. tostring(s2_194.dn.x + s2_194.dn.w) ..
    " ≤ 模版左边缘 " .. tostring(tpl194.x) .. " − 间隙 " .. tostring(s2_194.tailGap) .. "）")
  eq(math.abs((tpl194.x - (s2_194.dn.x + s2_194.dn.w)) - s2_194.tailGap) <= 0.5, true,
    "③★★间隙**恰好**等于生产自报的 tailGap —— 右对齐（不是随手留的一条缝）")
  eq(math.abs(s2_194.up.y - tpl194.y) <= 0.01, true,
    "④★★★与 [案例模版] **同顶边**（y " .. tostring(s2_194.up.y) .. " vs " .. tostring(tpl194.y) ..
    "）——同一行，不靠事后补救对齐")
  eq(math.abs(s2_194.up.h - tplH194) <= 0.01, true,
    "④★★与 [案例模版] **同高**（" .. tostring(s2_194.up.h) .. " vs " .. tostring(tplH194) ..
    "）——同高同顶边 ⇒ 中线也必然对齐")
  eq(math.abs(s2_194.up.y - s2_194.dn.y) <= 0.01, true, "④★两枚同顶边（并排成一组）")
  eq(s2_194.up.x + s2_194.up.w <= s2_194.dn.x + 0.5, true,
    "⑤★两枚**不重叠**（[上一页] 右缘 " .. tostring(s2_194.up.x + s2_194.up.w) ..
    " ≤ [下一页] 左端 " .. tostring(s2_194.dn.x) .. "）")
  local w194, h194 = EVAL_TEST_CFG_SIZE()
  eq(s2_194.groupX > 0 and (s2_194.dn.x + s2_194.dn.w) < w194, true,
    "⑤★整组在窗口宽度内（左端 " .. tostring(s2_194.groupX) .. "，右缘 " ..
    tostring(s2_194.dn.x + s2_194.dn.w) .. " < " .. tostring(w194) .. "）")
  eq(s2_194.up.y < 0 and s2_194.up.y > -h194, true,
    "⑤★在窗口高度内（y=" .. tostring(s2_194.up.y) .. "，窗高 " .. tostring(h194) .. "）")
  -- ★★同一行上还有左侧那排开关（启用一键宏/自动攻击/接收方案）——两组必须各占一侧、不打架。
  --   开关行右缘由生产自报（cfgWin.layout.swRight）；★这条同时也是**给未来加开关的人的护栏**：
  --   再往这排加一项、右缘顶到翻页按钮上，这里当场报出来，而不是等用户截图。
  local lay194 = EVAL_TEST_CFG_LAYOUT()
  eq(type(lay194 and lay194.swRight) == "number" and s2_194.groupX >= lay194.swRight, true,
    "⑤★★翻页按钮组与左侧**开关横排**不重叠（翻页左端 " .. tostring(s2_194.groupX) ..
    " ≥ 开关行右缘 " .. tostring(lay194 and lay194.swRight) .. "）")

  -- ⑦ 反向哨兵：旧实现（列表右栏、TOPRIGHT 锚点）不许再存在
  eq(s2_194.up.point == "TOPLEFT" and s2_194.dn.point == "TOPLEFT", true,
    "⑦★★反向哨兵：两枚都是 TOPLEFT（旧的右栏箭头是 TOPRIGHT）——「还是原来那个」当场失败")
  eq(s2_194.up.x < tpl194.x, true, "⑦★★反向哨兵：整组在 [案例模版] **左侧**（不是列表右栏）")

  -- ⑥ 整页步进：点一下真实 OnClick = 一整页
  local dnBtn194 = s2_194.dn.btn
  local dnFn194 = (dnBtn194 and dnBtn194.GetScript) and dnBtn194:GetScript("OnClick") or nil
  eq(type(dnFn194) == "function", true, "⑥前置：[下一页] 暴露真实 OnClick 闭包")
  eq(s2_194.offset == 0, true, "⑥前置：偏移从 0 开始（实测 " .. tostring(s2_194.offset) .. "）")
  if type(dnFn194) == "function" then pcall(dnFn194) end
  local s3_194 = EVAL_TEST_WAR_SCROLL()
  eq(s3_194.offset == rowsN194, true,
    "⑥★★★点一次 [下一页] 走**一整页**（偏移 " .. tostring(s3_194.offset) .. " == 槽位数 " ..
    tostring(rowsN194) .. "）——不是一行")
  local upBtn194 = s3_194.up.btn
  local upFn194 = (upBtn194 and upBtn194.GetScript) and upBtn194:GetScript("OnClick") or nil
  if type(upFn194) == "function" then pcall(upFn194) end
  eq(EVAL_TEST_WAR_SCROLL().offset == 0, true, "⑥★★点一次 [上一页] 回到起点")

  -- 收尾（跨用例状态残留是本项目老坑）
  EVAL_HELP_CONFIG.war = savedWar194
  pcall(EVAL_WAR_TAB_REFRESH)
  if not wasVisible194 and EVAL_TEST_CFG_VISIBLE() then pcall(EVAL_HELP_CFG_TOGGLE) end
  print("  技能列表翻页：没滚动条时隐藏 · 有滚动条时出现在底部 [案例模版] 左侧且右对齐 · 同顶边同高 · 整页步进")
end

-- ===== 组 195（1.74.17）：战斗信息UI 技能图标带**自动换行 + 自适应高度**（用户截图：「战斗UI 每行8个.
--   超过2行的时候更多的就不显示了.需求是需要能自动换行.自适应高度换行图标」）=====
-- 背景：旧实现是**写死的 2 行 16 格**（for i = 1, 16）＋ 帧高按两行预留 → 第 17 条技能起连格子都没有，
--   静默消失（正是用户截图里「超过 2 行就不显示」那个现象）。
-- 覆盖：①每行恰好 8 格 ②池子 = ceil(n/8)*8（不再封顶 16）
--   ③★第 17 格真的存在且被画出来（旧实现在这里就断了）④每行格数 ≤ 8 且行数 == 真实坐标里的行数
--   ⑤★自适应高度：3 行 vs 1 行的帧高差 == 2 × 真实行距（行距从**真控件坐标**读，不用生产常量自证）
--   ⑥★技能数变多 → **tick 自己发现并重建**（不靠谁记得去调 BUILD）⑦反向：变少也要缩回去
--   ⑧反向哨兵：池子不再是恒定 16 格
do
  local savedWar195, savedUI195 = EVAL_HELP_CONFIG.war, EVAL_HELP_CONFIG.ui
  local wasShown195 = EVAL_TEST_UI_SHOWN()
  EVAL_HELP_CONFIG.ui = EVAL_HELP_CONFIG.ui or {}
  EVAL_HELP_CONFIG.ui.subScheme = true -- 方案区开着才建技能带（子开关关 → profCells 根本没建）
  local function mkSkills195(n)
    local t = {}
    for i = 1, n do t[i] = { skill = "格" .. tostring(i), enabled = true, groups = {} } end
    return t
  end
  if not wasShown195 then EVAL_HELP_UI_TOGGLE() end
  eq(EVAL_TEST_UI_SHOWN(), true, "前置：战斗信息UI 已打开")
  local function buildWith195(n) -- 走生产入口：重建 + 保持显示 + 画一遍
    EVAL_HELP_CONFIG.war = { activeProfile = 1, profiles = { { name = "网格测试", skills = mkSkills195(n) } } }
    EVAL_HELP_UI_REBUILD_IF_SHOWN()
    EVAL_HELP_UI_TICK()
    return EVAL_TEST_UI_GRID()
  end

  -- ① 每行 8 格：第 1..8 格同一行（y 相同、逐格右移），第 9 格换行到下一行且回到行首 x
  local g20 = buildWith195(20)
  eq(type(g20) == "table" and g20.cells[1] ~= nil, true, "①前置：拿到技能带的真实网格")
  eq(g20.cols, 8, "①★每行 8 格（用户定的列数）")
  eq(g20.cells[8].y, g20.cells[1].y, "①★★第 1..8 格在**同一行**（y 相同：" .. tostring(g20.cells[1].y) .. "）")
  eq(g20.cells[8].x > g20.cells[1].x, true, "①★同一行逐格右移（第 8 格比第 1 格靠右）")
  eq(g20.cells[9].y < g20.cells[1].y, true,
    "①★★第 9 格**换到下一行**（y " .. tostring(g20.cells[9].y) .. " < 首行 " .. tostring(g20.cells[1].y) .. "）")
  eq(g20.cells[9].x, g20.cells[1].x, "①★换行后回到**行首**（第 9 格 x == 第 1 格 x）")

  -- ②③ 池子按需（不再封顶 16）；第 17 格必须真的存在且被画出来
  eq(g20.count, 24, "②★★20 条技能 → 池子 24 格（ceil(20/8)*8；旧实现封顶 16 → 后 4 条没格子）")
  eq(g20.rows, 3, "②★3 行")
  eq(g20.cells[17] ~= nil, true, "③★★★第 17 格**存在**（旧实现在这里就没了 —— 正是用户截图里那个现象）")
  eq(g20.cells[17] ~= nil and g20.cells[17].shown == true, true, "③★★第 17 格**真的被画出来**（不只是建了没显示）")
  eq(g20.cells[20].shown == true, true, "③★第 20 格（本方案最后一条技能）也画出来了")
  eq(g20.cells[21].shown == false, true, "③★★第 21 格**隐藏**（池子补齐到整行，不多画空格子）")

  -- ④ 行数 == 真实坐标里的行数；每行 ≤ 8 格
  local seen195, nRow195, perRow195 = {}, 0, {}
  for i = 1, g20.count do
    local yy = g20.cells[i] and g20.cells[i].y
    if yy then
      if not seen195[yy] then seen195[yy] = true nRow195 = nRow195 + 1 end
      perRow195[yy] = (perRow195[yy] or 0) + 1
    end
  end
  eq(nRow195, g20.rows, "④★★真实坐标里的行数（" .. tostring(nRow195) .. "）== 生产自报的行数（" .. tostring(g20.rows) .. "）")
  local overMax195 = nil
  for _, cc in pairs(perRow195) do if cc > 8 then overMax195 = cc end end
  eq(overMax195, nil, "④★没有任何一行超过 8 格")

  -- ⑤ 自适应高度：3 行 − 1 行 == 2 × 真实行距（行距从真控件坐标读）
  local pitch195 = g20.cells[1].y - g20.cells[9].y -- 真实行距（第 2 行 − 第 1 行）
  eq(pitch195 > 0, true, "⑤前置：拿到真实行距（" .. tostring(pitch195) .. "）")
  local g8 = buildWith195(8)
  eq(g8.rows == 1 and g8.count == 8, true,
    "⑤★8 条技能 → 1 行 8 格（实测 rows=" .. tostring(g8.rows) .. " count=" .. tostring(g8.count) .. "）")
  eq(type(g20.height) == "number" and type(g8.height) == "number", true, "⑤前置：拿到两次的真实帧高")
  eq(math.abs((g20.height - g8.height) - 2 * pitch195) <= 0.01, true,
    "⑤★★★自适应高度：3 行比 1 行高出**恰好 2 个行距**（实测 " .. tostring(g20.height) .. " − " .. tostring(g8.height) ..
    " = " .. tostring(g20.height - g8.height) .. "，2×行距 " .. tostring(2 * pitch195) .. "）")

  -- ⑥ 技能数变多 → tick 自己发现并重建（不靠调用方记得去 BUILD）
  EVAL_HELP_CONFIG.war = { activeProfile = 1, profiles = { { name = "网格测试", skills = mkSkills195(8) } } }
  EVAL_HELP_UI_REBUILD_IF_SHOWN()
  EVAL_HELP_UI_TICK()
  local g6a = EVAL_TEST_UI_GRID()
  eq(g6a.rows == 1 and g6a.count == 8, true, "⑥前置：8 条 → 1 行 8 格")
  local h6a = g6a.height
  -- ★只改数据、**故意不调 BUILD**：下面的 tick 必须自己重建
  EVAL_HELP_CONFIG.war.profiles[1].skills = mkSkills195(20)
  EVAL_HELP_UI_TICK()
  local g6b = EVAL_TEST_UI_GRID()
  eq(g6b.rows == 3 and g6b.count == 24, true,
    "⑥★★★只改技能数、不调 BUILD → tick **自己重建**（实测 rows=" .. tostring(g6b.rows) .. " count=" .. tostring(g6b.count) .. "）")
  eq(g6b.cells[17] ~= nil and g6b.cells[17].shown == true, true, "⑥★★第 17 格跟着出现")
  eq(g6b.height > h6a, true, "⑥★★帧高跟着长高（" .. tostring(h6a) .. " → " .. tostring(g6b.height) .. "）")

  -- ⑦ 反向：技能数变少也要**缩回去**（不是只增不减）
  EVAL_HELP_CONFIG.war.profiles[1].skills = mkSkills195(8)
  EVAL_HELP_UI_TICK()
  local g7 = EVAL_TEST_UI_GRID()
  eq(g7.rows == 1 and g7.count == 8, true,
    "⑦★★★技能数变少 → tick 同样重建收缩（rows=" .. tostring(g7.rows) .. " count=" .. tostring(g7.count) .. "）")
  eq(type(g7.height) == "number" and math.abs(g7.height - h6a) <= 0.01, true,
    "⑦★★缩回后的帧高 == 1 行时的高度（" .. tostring(g7.height) .. " vs " .. tostring(h6a) .. "）")

  -- ⑧ 反向哨兵：池子不再是恒定 16
  eq(g8.count ~= 16 and g20.count ~= 16, true,
    "⑧★★反向哨兵：池子不再恒定 16 格（1 行=" .. tostring(g8.count) .. "，3 行=" .. tostring(g20.count) .. "）")

  -- 收尾（跨用例状态残留是本项目老坑）
  EVAL_HELP_CONFIG.war, EVAL_HELP_CONFIG.ui = savedWar195, savedUI195
  pcall(EVAL_WAR_TAB_REFRESH)
  if EVAL_TEST_UI_SHOWN() then EVAL_HELP_UI_REBUILD_IF_SHOWN() end -- 按还原后的方案重建，别把测试用的 20 格留给后面
  if not wasShown195 and EVAL_TEST_UI_SHOWN() then pcall(EVAL_HELP_UI_TOGGLE) end
  print("  战斗UI 技能带：每行 8 格自动换行 · 池子按需（不再封顶 16）· 帧高自适应 · 变多变少都由 tick 自动重建")
end

-- ★★1.74.17 修一处**位置事故**:这条断言原先被落在了 print("ALL TESTS PASS") **之后**
--   （没有组头、也没有 do/end 包着，是历史上一次编辑的残渣）。它确实还在跑，但**跑在成功横幅之后**——
--   一旦它失败，屏幕上先出现「ALL TESTS PASS」再出现 ASSERT FAIL（只有看退出码才不会误判）。
--   ★本项目铁律：成功横幅必须是**最后一句**，否则它就不再是「全过」的证据。故上移到横幅之前。
local sd142 = EVAL_HELP_CONFIG.shareSealDemo
eq(type(sd142) == "table" and sd142.n == 10, true, -- ★1.73.42n 12 → 10（5 品阶分享行 + 5 档头衔示意）
   "★★★/eh go 秘籍样例 真的执行了（以**落盘字段**为证：n=" .. tostring(type(sd142) == "table" and sd142.n or "nil") .. "）")

-- ===== 组 196（1.74.19）：**方案评级与头衔晋升的重做**（用户：「审计方案的评级标准.审计玩家头衔的评级标准.
--   增加评级难度」+ 后续逐条定稿）=====
-- 定稿参数（用户逐条确认）：
--   · 档位：≤11 普通 / 12-23 稀有 / 24-35 珍稀 / 36-49 绝版 / **≥50 神级**（1.74.21 用户定）
--   · 评分口径：空技能 0 分；条件按三档加权（自身/技能 1 · 目标/光环 2 · 队伍/候选 3）；
--     单条技能上限 12 分；同一行里**完全一样**的条件只算一次；
--     **★不占动作条的特殊技能无条件也保留 1 分**（用户定：`宠物:攻击`/`取消施法` 这类本来就没条件可配）
--   · 覆盖封顶：覆盖 1/2/3 个大类分别封顶 23/35/49，**≥4 类才不封顶**（才有资格冲神级）
--   · 头衔晋升：数量门槛 {1,2,3,3,2} + **逐级满足**（组 147 已逐条钉边界）
-- 覆盖：①★历史漏洞回归哨兵（25 条空技能不许再是神级）②空技能 0 分 / 特殊技能保留 1 分
--   ③条件按类加权 ④单条上限 12 ⑤完全重复的条件只算一次 ⑥覆盖大类去重统计
--   ⑦覆盖封顶三档 + 4 类不封顶 ⑧单入口与两参式判定一致 ⑨档位表与神级门槛读值口自洽
do
  local function condK(k) return { k = k } end
  local function mkProf(skillGroups) -- skillGroups = { {cond,...}, ... } 每条技能一组条件
    local p = { skills = {} }
    for i = 1, table.getn(skillGroups) do
      p.skills[i] = { skill = "技" .. tostring(i), groups = { skillGroups[i] } }
    end
    return p
  end

  -- ①★★★ 历史漏洞回归哨兵：旧口径「技能条数 + 条件数」下，25 条**无条件**技能 = 25 分 = 神级
  --   （用户真机存档里「合欢宗宝典」就是靠 13 条空「攻击」从珍稀被抬到神级的）
  local empty25 = { skills = {} }
  for i = 1, 25 do empty25.skills[i] = { skill = "攻击", groups = {} } end
  eq(EVAL_PROFILE_SCORE(empty25), 0, "①★★★25 条**空技能** = **0 分**（旧口径 25 分 —— 这条哨兵就是钉死那个漏洞的）")
  eq(select(2, EVAL_PROFILE_TIER(empty25)).name, "普通",
     "①★★★25 条空技能 = **普通**（旧口径是**神级**）—— 「堆空技能到顶」这条路彻底堵死")

  -- ② 空技能 0 分；不占动作条的特殊技能**保留 1 分**（用户 1.74.19 定）
  eq(EVAL_PROFILE_SCORE(mkProf({ {} })), 0, "②★1 条普通技能、零条件 → 0 分")
  local specialP = { skills = { { skill = "宠物:攻击", groups = {} }, { skill = "取消施法", groups = {} },
                                { skill = "普通技能", groups = {} } } }
  eq(EVAL_PROFILE_SCORE(specialP), 2,
     "②★★★**不占动作条的特殊技能**无条件也保留 1 分（宠物:攻击 + 取消施法 = 2；普通技能仍是 0）")
  eq(EVAL_NO_SLOT_OK("宠物:攻击") and EVAL_NO_SLOT_OK("取消施法"), true,
     "②★判据走的是项目唯一来源 EVAL_NO_SLOT_OK（不另抄一份名单）")

  -- ③ 条件按类加权：自身 1 · 技能 1 · 目标 2 · 光环 2 · 队伍/候选 3
  eq(EVAL_COND_WEIGHT("combat"), 1, "③★自身状态类 = 1 分（combat）")
  eq(EVAL_COND_WEIGHT("ready"), 1, "③★技能状态类 = 1 分（ready）")
  eq(EVAL_COND_WEIGHT("canAttack"), 2, "③★目标状态类 = 2 分（canAttack）")
  eq(EVAL_COND_WEIGHT("hasBuff"), 2, "③★光环检查类 = 2 分（hasBuff）")
  eq(EVAL_COND_WEIGHT("teamHp"), 3, "③★队伍/团员类 = 3 分（teamHp）")
  eq(EVAL_COND_WEIGHT("candHp"), 3, "③★候选者类 = 3 分（candHp）")
  eq(EVAL_PROFILE_SCORE(mkProf({ { condK("combat") } })), 2, "③★1 条技能 + 1 个自身条件 = 1 + 1 = 2")
  eq(EVAL_PROFILE_SCORE(mkProf({ { condK("canAttack") } })), 3, "③★1 条技能 + 1 个目标条件 = 1 + 2 = 3")
  eq(EVAL_PROFILE_SCORE(mkProf({ { condK("teamHp") } })), 4, "③★1 条技能 + 1 个队伍条件 = 1 + 3 = 4")

  -- ⑤ 完全重复的条件只算一次
  local many = {}
  for i = 1, 20 do many[i] = condK("combat") end
  eq(EVAL_PROFILE_SCORE(mkProf({ many })), 2, "⑤★★同一条技能里 20 个**完全一样**的条件只算一次（1 + 1 = 2，不是 21）")
  -- ④ 单条技能上限 12 分
  local mixed = {}
  local names = { "combat", "hpPct", "power", "powerPct", "combo", "form", "ready", "usable", "notQueued",
                  "inRange", "casting", "castEl", "castLeft" }
  for i = 1, table.getn(names) do mixed[i] = condK(names[i]) end
  eq(EVAL_PROFILE_SCORE(mkProf({ mixed })), 13,
     "④★★单条技能最多计 **12 分** 的权重（13 个各 1 分的不同条件 → 权重被夹到 12 ⇒ 合计 1+12 = 13）")

  -- ⑥ 覆盖大类：按**类型去重**，落在 5 个大类上
  local covP = mkProf({ { condK("combat") }, { condK("combat") }, { condK("canAttack") }, { condK("hasBuff") } })
  eq(EVAL_PROFILE_COVERAGE(covP), 3, "⑥★★覆盖大类 = 3（自身 A / 目标 B / 光环 C；重复的 combat 只算一次）")
  eq(EVAL_COND_CLASS("combat") == "A" and EVAL_COND_CLASS("canAttack") == "B"
     and EVAL_COND_CLASS("hasBuff") == "C" and EVAL_COND_CLASS("ready") == "D"
     and EVAL_COND_CLASS("teamHp") == "E" and EVAL_COND_CLASS("candHp") == "E", true,
     "⑥★★五个大类的归属：自身 A / 目标 B / 光环 C / 技能 D / 队伍·候选 E")

  -- ⑦ 覆盖封顶：1/2/3 类分别封顶 34/69/119；≥4 类**不封顶**
  local function bigProf(kinds)
    local sk = {}
    for i = 1, 30 do
      local g = {}
      for j = 1, table.getn(kinds) do g[j] = condK(kinds[j]) end
      sk[i] = g
    end
    return mkProf(sk)
  end
  local p1 = bigProf({ "teamHp" })
  local p3 = bigProf({ "teamHp", "canAttack", "hasBuff" })
  local p4 = bigProf({ "teamHp", "canAttack", "hasBuff", "combat" })
  local p2 = bigProf({ "teamHp", "canAttack" })
  eq(EVAL_PROFILE_COVERAGE(p1), 1, "⑦前置：p1 只覆盖 1 类")
  eq(EVAL_PROFILE_SCORE(p1) > 23, true, "⑦前置：p1 原始分 > 23（实测 " .. tostring(EVAL_PROFILE_SCORE(p1)) .. "）")
  eq(EVAL_PROFILE_COVERAGE_CAP(1), 23, "⑦★★覆盖 1 类 → 封顶 **23**（= 稀有上沿，从档位表现算，不写死）")
  eq(EVAL_PROFILE_COVERAGE_CAP(2), 35, "⑦★★覆盖 2 类 → 封顶 **35**（= 珍稀上沿）")
  eq(EVAL_PROFILE_COVERAGE_CAP(3), 49, "⑦★★覆盖 3 类 → 封顶 **49**（= 绝版上沿）")
  eq(EVAL_PROFILE_COVERAGE_CAP(4), nil, "⑦★★★覆盖 4 类 → **不封顶**（神级的实际门槛：得跨类写）")
  eq(select(2, EVAL_PROFILE_TIER(p1)).capped, true, "⑦★★p1 被**封顶**（原始分再高也上不去）")
  eq(select(2, EVAL_PROFILE_TIER(p1)).name, "稀有", "⑦★★★只堆一个大类 → 30 条技能也只能到**稀有**")
  eq(select(2, EVAL_PROFILE_TIER(p3)).name, "绝版", "⑦★★覆盖 3 类 → 天花板到**绝版**（还差一类）")
  eq(select(2, EVAL_PROFILE_TIER(p4)).name, "神级", "⑦★★★覆盖 4 类 → 不封顶 → 才到得了**神级**")
  eq(select(2, EVAL_PROFILE_TIER(p4)).capped, false, "⑦★p4 没被封顶")

  -- ⑧ 单一入口与两参式判定**一致**（封顶只实现一处）
  local ti8a, t8a = EVAL_PROFILE_TIER(p2)
  local ti8b, t8b = EVAL_SHARE_SEAL_TIER(EVAL_PROFILE_SCORE(p2), EVAL_PROFILE_COVERAGE(p2))
  eq(ti8a == ti8b and t8a.capped == t8b.capped, true, "⑧★★单入口与两参式判定结果一致（封顶只有一处实现）")
  eq(select(2, EVAL_SHARE_SEAL_TIER(EVAL_PROFILE_SCORE(p1))).capped, false,
     "⑧★★★**只传分数**（没有覆盖信息）→ **不封顶**：纯分数映射的老调用点不许被误伤")

  -- ⑨ 档位表与神级门槛读值口自洽（命令/闸门/样例都读它们，不写死数字）
  eq(EVAL_SEAL_GOD_SCORE(), 50, "⑨★神级门槛现算 = 50（= 最后一个有上沿的档 +1）")
  eq(select(2, EVAL_SHARE_SEAL_TIER(EVAL_SEAL_GOD_SCORE())).name, "神级", "⑨★门槛分确实落在神级档")
  eq(select(2, EVAL_SHARE_SEAL_TIER(EVAL_SEAL_GOD_SCORE() - 1)).name, "绝版", "⑨★门槛 −1 分 = 绝版（边界两侧都钉）")
  local s1, s5 = EVAL_SEAL_TIER_SAMPLE(1), EVAL_SEAL_TIER_SAMPLE(5)
  eq(select(1, EVAL_SHARE_SEAL_TIER(s1)) == 1 and select(1, EVAL_SHARE_SEAL_TIER(s5)) == 5, true,
     "⑨★★每档代表分现算后仍落在**自己那一档**（样例/命令不再串档）：" .. tostring(s1) .. " / " .. tostring(s5))
  print("  方案评级重做：空技能0分 · 条件按类加权(1/2/3) · 单条上限12 · 完全重复只算一次 · 覆盖封顶(1/2/3类=23/35/49，≥4类不封顶) · 25条空技能不再是神级")
end

-- ===== 组 197（1.74.20）：**配置分角色存** —— 消耗品助手 + 喂食助手 + 骑乘助手整块按角色
--   （用户：「消耗品助手 + 喂食助手 + 骑乘助手整块按角色（含各自悬浮件位置）。配置按照角色存储」）=====
-- 机制：新增**角色级存档** `EVAL_HELP_CHAR`（`## SavedVariablesPerCharacter`，本客户端实测支持 ——
--   UnrealQuest 已在用、磁盘上三个角色三个文件）；`tbCfg()` 返回**路由代理**，角色键落角色表、其余落账号表，
--   于是上百处既有调用点一个字不用改。
-- 覆盖：①角色键清单齐全 ②**路由**（两边互不串）③★★false 经代理读回仍是 false（本轮实测的 and/or 吞 false 坑）
--   ④三个助手真的读角色表（反向哨兵：账号表里的同名键不生效）⑤迁移一次性继承、账号副本保留、不覆盖角色已设值
--   ⑥迁移只在账号表到位时才标记（早调不许立标志，否则种子永久丢）
do
  local keepChar197 = rawget(_G, "EVAL_HELP_CHAR")
  local keepAcct197 = EVAL_HELP_CONFIG and EVAL_HELP_CONFIG.tb
  eq(type(EVAL_TB_CHAR_KEY_LIST) == "function", true, "①前置：拿到角色键清单")
  local keys197 = EVAL_TB_CHAR_KEY_LIST()
  local seen197, nKeys197 = {}, 0
  for i = 1, table.getn(keys197) do seen197[keys197[i]] = true nKeys197 = nKeys197 + 1 end
  local wantKeys197 = { "chUse", "chTex", "consumable", "chX", "chY",
                        "feedPet", "hhFood", "hhFoodTex", "hhSpell", "hhX", "hhY",
                        "dismount", "dismountAuto", "dhX", "dhY" }
  local miss197 = ""
  for i = 1, table.getn(wantKeys197) do
    if not seen197[wantKeys197[i]] then miss197 = miss197 .. " " .. wantKeys197[i] end
  end
  eq(miss197, "", "①★★★三个助手的键**全部**在角色键清单里（缺：" .. (miss197 == "" and "无" or miss197) .. "）")
  eq(nKeys197, table.getn(wantKeys197), "①★清单不多不少 = " .. tostring(table.getn(wantKeys197)) .. " 个（实测 " .. tostring(nKeys197) .. "）")
  eq(EVAL_TB_IS_CHAR_KEY("qchan") == false and EVAL_TB_IS_CHAR_KEY("chatColor") == false, true,
     "①★反向：任务通知/聊天着色这类**行为开关仍是账号级**（用户只要求三个助手按角色）")

  -- ② 路由：代理写角色键 → 角色表；写账号键 → 账号表；两边互不串
  EVAL_TB_CHAR_RESET()
  EVAL_HELP_CONFIG.tb = {}
  local r197 = EVAL_TEST_TB_CFG() -- = 生产入口 tbCfg()（返回路由代理）
  eq(type(r197) == "table", true, "②前置：拿到 tbCfg() 的返回值")
  r197.consumable = true           -- 角色键
  r197.hhFood = "熏熊肉"            -- 角色键
  r197.repair = true               -- 账号键
  r197.qchan = { self = true }     -- 账号键
  local cs197 = EVAL_TB_CHAR_STORE()
  eq(cs197.consumable == true and cs197.hhFood == "熏熊肉", true, "②★★★角色键写进了**角色表**（EVAL_HELP_CHAR.tb）")
  eq(EVAL_HELP_CONFIG.tb.consumable == nil and EVAL_HELP_CONFIG.tb.hhFood == nil, true,
     "②★★★角色键**没有**漏进账号表（漏了就成两份真值）")
  eq(EVAL_HELP_CONFIG.tb.repair == true and type(EVAL_HELP_CONFIG.tb.qchan) == "table", true, "②★★账号键照旧写账号表")
  eq(cs197.repair == nil and cs197.qchan == nil, true, "②★★账号键**没有**漏进角色表")
  eq(r197.consumable == true and r197.repair == true, true, "②★读回来两边都对（代理双向都通）")

  -- ③★★ false 经代理必须还是 false
  r197.repair = false
  r197.feedPet = false
  eq(r197.repair, false, "③★★★账号键写 false → 读回**仍是 false**（吞成 nil 就等于「用户关掉的开关自己又开了」）")
  eq(r197.feedPet, false, "③★★★角色键写 false → 读回**仍是 false**")
  eq(EVAL_HELP_CONFIG.tb.repair, false, "③★底层账号表里也是 false")
  eq(cs197.feedPet, false, "③★底层角色表里也是 false")

  -- ④ 三个助手真的读角色表 + 反向哨兵
  EVAL_TB_CHAR_RESET()
  EVAL_HELP_CONFIG.tb = {}
  cs197 = EVAL_TB_CHAR_STORE()
  cs197.feedPet = true
  cs197.hhFood = "熏熊肉"
  cs197.consumable = true
  cs197.chUse = { "面包" }
  cs197.dismount = true
  eq(EVAL_TEST_HH_STATE().on, true, "④★★喂食助手读的是**角色表**的开关")
  eq(EVAL_CH_LIST()[1], "面包", "④★★消耗品助手读的是**角色表**的选中列表")
  EVAL_TB_CHAR_RESET()
  EVAL_HELP_CONFIG.tb = { feedPet = true, hhFood = "熏熊肉", consumable = true, chUse = { "面包" }, dismount = true }
  eq(EVAL_TEST_HH_STATE().on, false, "④★★★反向哨兵：账号表里同名键**不生效**（助手读的是角色表）")
  eq(EVAL_CH_LIST()[1], nil, "④★★反向哨兵：账号表里的 chUse 不算数")

  -- ⑤ 迁移
  EVAL_TB_CHAR_RESET()
  rawset(_G, "EVAL_HELP_CHAR", { tb = {} }) -- 清掉 tbMigrated，制造「首次登录」
  EVAL_HELP_CONFIG.tb = { feedPet = true, hhFood = "大块野猪肉", consumable = true, chUse = { "森林蘑菇" }, repair = true }
  eq(EVAL_TB_CHAR_MIGRATE(), true, "⑤★★首次迁移执行了")
  local mig197 = EVAL_TB_CHAR_STORE()
  eq(mig197.feedPet == true and mig197.hhFood == "大块野猪肉", true, "⑤★★★喂食助手的老配置**继承**过来了")
  eq(mig197.consumable == true and mig197.chUse and mig197.chUse[1] == "森林蘑菇", true, "⑤★★★消耗品助手的老配置也继承过来了")
  eq(EVAL_HELP_CONFIG.tb.feedPet == true and EVAL_HELP_CONFIG.tb.chUse ~= nil, true,
     "⑤★★账号里那份**副本保留**（它是还没登录过的角色的种子；删了那些角色就什么都没有）")
  eq(mig197.repair == nil, true, "⑤★★账号键**不**被搬进角色表（只搬三个助手的键）")
  eq(EVAL_TB_CHAR_MIGRATE(), false, "⑤★再调一次不重复迁移（每角色只做一次）")
  mig197.hhFood = "我自己设的"
  rawset(_G, "EVAL_HELP_CHAR", { tb = mig197 })
  EVAL_HELP_CONFIG.tb = { hhFood = "账号里的" }
  EVAL_TB_CHAR_MIGRATE()
  eq(EVAL_TB_CHAR_STORE().hhFood, "我自己设的", "⑤★★迁移**不覆盖**角色已设的值")

  -- ⑥ 迁移只在账号表到位时才标记
  rawset(_G, "EVAL_HELP_CHAR", { tb = {} })
  local keepCfg197 = EVAL_HELP_CONFIG
  rawset(_G, "EVAL_HELP_CONFIG", nil)
  eq(EVAL_TB_CHAR_MIGRATE(), false, "⑥★★账号配置还没到位 → 迁移如实返回 false")
  eq(rawget(_G, "EVAL_HELP_CHAR").tbMigrated == nil, true,
     "⑥★★★而且**不标记**（标记了就等于「用一张空表继承出空配置」，老存档的种子永久丢失）")
  rawset(_G, "EVAL_HELP_CONFIG", { tb = { feedPet = true } })
  eq(EVAL_TB_CHAR_MIGRATE(), true, "⑥★★账号到位后再调 → 迁移照常执行")
  eq(EVAL_TB_CHAR_STORE().feedPet, true, "⑥★老配置确实继承到了")

  -- 收尾（跨用例状态残留是本项目老坑）
  rawset(_G, "EVAL_HELP_CONFIG", keepCfg197)
  if keepAcct197 ~= nil then EVAL_HELP_CONFIG.tb = keepAcct197 end
  if keepChar197 ~= nil then rawset(_G, "EVAL_HELP_CHAR", keepChar197) else rawset(_G, "EVAL_HELP_CHAR", nil) end
  EVAL_TB_CHAR_RESET()
  print("  配置分角色：角色键 15 个（三个助手）· tbCfg() 路由代理（false 不被吞）· 助手真的读角色表 · 老配置一次性继承、账号副本保留")
end

-- ===== 组 198（1.74.22）：案例模版收录「神圣风暴」（骑士 · Rainbow 分享）+ 模版条目支持 作者/备注 =====
-- 用户原话：「查看用户配置 → 找到方案配置内的神圣风暴方案，将这个方案保存到案例模版骑士分类下模版。
--   作者备注: Rainbow；案例备注: 来自龙之国度的LM-Rainbow-骑士无私分享」。
-- 判据：① 那条模版真的在骑士组里，且 author/note 两个字段逐字正确；
--   ② 模版文本与导入器**逐字往返**（export(parse(text)) == text，守「手抄/丢行/条件写法漂移」这类静默错误）；
--   ③ 用户方案里最有代表性的两个条件写法在文本里原样存在；
--   ④ 作者署名进「方案来源」标签，且**不带 author 的老模版行为不变**（仍记「技能学院」）；
--   ⑤ 三语言都有 作者/备注 两个标签（悬停两行不会有一个语言显示成键名）。
do
  local pal198, hit198 = nil, nil
  for _, g in ipairs(EVAL_IO_TEMPLATES) do if g.cls == "骑士" then pal198 = g break end end
  eq(pal198 ~= nil, true, "①★骑士模版组存在")
  for _, t in ipairs((pal198 and pal198.list) or {}) do
    if tostring(t.name) == "神圣风暴" then hit198 = t end
  end
  eq(hit198 ~= nil, true, "①★★★「神圣风暴」已收录进骑士模版组")
  eq(hit198 and hit198.author, "Rainbow", "①★★★作者备注 = Rainbow")
  eq(hit198 and hit198.note, "来自龙之国度的LM-Rainbow-骑士无私分享", "①★★★案例备注逐字正确")
  eq(string.len(tostring((hit198 and hit198.desc) or "")) > 0, true, "①★它带说明（悬停能看出它干吗）")
  -- ② 逐字往返 + 条数
  local p198 = EVAL_PROFILE_FROM_TEXT((hit198 and hit198.text) or "")
  eq(p198 ~= nil, true, "②★模版文本能被导入器解析")
  eq(p198 and table.getn(p198.skills), 13, "②★★★13 条技能行（与用户存档那份方案一致）")
  eq(p198 ~= nil and EVAL_PROFILE_TO_TEXT(p198) == hit198.text, true,
     "②★★★export(parse(text)) == text 逐字往返（丢行/改写法都会当场失败）")
  -- ③ 代表性的条件写法原样保留（用户方案里最容易抄错的两处）
  local tx198 = tostring((hit198 and hit198.text) or "")
  eq(p198 and p198.skills[1].skill, "圣光术", "③★第 1 行 = 圣光术")
  eq(string.find(tx198, "自身debuff:(Magic/Curse/Disease)", 1, true) ~= nil, true, "③★★带负面类型的自身 debuff 写法保留")
  eq(string.find(tx198, "有buff:正义圣印[<=5s]", 1, true) ~= nil, true, "③★★剩余时长后缀写法保留")
  eq(string.find(tx198, "目标类型非:人型", 1, true) ~= nil, true, "③★★「目标类型非」写法保留")
  -- ④ 署名进来源标签（钉死语言，用字面量期望，不拿语言包自比）
  local lang198 = EVAL_GET_LANG()
  EVAL_SET_LANG("zhCN")
  eq(EVAL_PROF_AUTHOR_LABEL({ src = "text", author = "Rainbow" }), "分享：Rainbow",
     "④★★带 author 的模版 → 方案来源显示「分享：Rainbow」")
  eq(EVAL_PROF_AUTHOR_LABEL({ src = "text", author = "技能学院" }), "案例模版：技能学院",
     "④★★不带 author 的老模版行为不变（仍记案例模版：技能学院）")
  eq(EVAL_PROF_AUTHOR_LABEL({ src = "text" }), "导入", "④★无作者的文本导入照旧「导入」")
  EVAL_SET_LANG(lang198)
  -- ⑤ 三语言标签（字面量）
  eq((EVAL_LOCALES.zhCN or {}).TPL_AUTHOR, "作者", "⑤★zhCN 作者标签")
  eq((EVAL_LOCALES.zhCN or {}).TPL_NOTE, "备注", "⑤★zhCN 备注标签")
  eq((EVAL_LOCALES.enUS or {}).TPL_AUTHOR, "Author", "⑤★enUS 作者标签")
  eq((EVAL_LOCALES.ruRU or {}).TPL_NOTE ~= nil and string.len(tostring(EVAL_LOCALES.ruRU.TPL_NOTE)) > 0, true, "⑤★ruRU 备注标签在")
  print("  案例模版：神圣风暴（作者 Rainbow / 备注 龙之国度分享）· 13 技能逐字往返 · 作者署名进来源 · 三语言标签齐")
end

-- ===== 组 199（1.74.23）：稀有提醒转播（任务插件弹窗出现那一刻 → 聊天框一行）=====
-- 用户原话：「需求是只要知道他发现稀有的那一时刻.在对话框内输出一段文字」。
-- 判据：① 安装时机 = VARIABLES_LOADED（不能更早：EvalHelp 目录名 E 先于 UnrealQuest 的 U 载入）；
--   ② 主路 = 包住 RareAlert:Show，**原返回值逐个透传** + 转播一行（名字/品阶/码数/还有几只）；
--   ③ 原函数抛错 → 按**原语义抛出去**（不吞别人的错）+ 如实计数；
--   ④ 兜底 = Show 不可包时轮询对方公开读值口（首读只立基准，涨了才转播）；
--   ⑤ 关掉开关 → 一个字节都不输出；⑥ 真实命令入口 /eh go 稀有（状态/关/开/试）；
--   ⑦ 对方缺席 → mode=absent（如实，不报错）；⑧ 三语言 19 个键齐（字面量期望）。
do
  local lang199 = EVAL_GET_LANG()
  EVAL_SET_LANG("zhCN")
  local savedUQ199 = rawget(_G, "UnrealQuest")
  local savedRW199 = (EVAL_HELP_CONFIG and EVAL_HELP_CONFIG.rareWatch)

  local function mkRA199(showImpl)
    local ra = { stats = { alerts = 0 } }
    ra.Show = showImpl or function(self, entry, distance, dx, dy, others)
      self.stats.alerts = self.stats.alerts + 1
      self.lastEntry, self.lastDistance, self.lastOthers = entry, distance, others
      return true
    end
    ra.GetStatus = function(self)
      return { alerts = self.stats.alerts, lastName = self.lastName,
               lastRank = self.lastRank, lastDistance = self.lastDistance }
    end
    ra.TestNearest = function(self)
      self:Show({ unitId = 7, rank = 2, coords = {} }, 30, 0, 0, 0)
      return "测试稀有", nil
    end
    return ra
  end
  ...
  local mods199 = {}
  rawset(_G, "UnrealQuest", { GetModule = function(_, name) return mods199[name] end })
  mods199.Database = { GetUnitName = function(_, id) return (id == 7) and "掘地鼠" or nil end }

  -- ① 安装时机：走**真实** VARIABLES_LOADED 事件入口（不是直调安装函数）
  EVAL_TEST_RW_RESET()
  EVAL_RW.seen, EVAL_RW.fails = 0, 0
  local ra199 = mkRA199()
  mods199.RareAlert = ra199
  local origShow199 = ra199.Show
  local initF199 = rawget(_G, "EVAL_HELPInitFrame")
  eq(type(initF199) == "table", true, "①前置：登录初始化帧在（下面走真实事件入口）")
  local okG199, fnEv199 = pcall(initF199.GetScript, initF199, "OnEvent")
  eq(okG199 and type(fnEv199) == "function", true, "①前置：init 帧挂了 OnEvent")
  if okG199 and type(fnEv199) == "function" then fnEv199("VARIABLES_LOADED") end
  eq(EVAL_TEST_RW_STATE().mode, "hook", "①★★VARIABLES_LOADED 之后**主路**装上了（包住 RareAlert:Show）")
  eq(type(ra199.Show) == "function" and ra199.Show ~= origShow199, true, "①★Show 是被**包装**（换了函数），不是被删掉")

  -- ② 主路：原返回值透传 + 转播一行
  TEST.chat = nil
  local okP199, passthru199 = pcall(ra199.Show, ra199, { unitId = 7, rank = 2, coords = {} }, 87, 10, -5, 2)
  eq(okP199 and passthru199 == true, true, "②★原函数的返回值**逐个透传**（true）")
  eq(ra199.stats.alerts, 1, "②★原函数照常执行（它自己的计数涨了）")
  local chat199 = tostring(TEST.chat or "")
  eq(string.find(chat199, "稀有提醒", 1, true) ~= nil, true, "②★★★聊天框里出现了转播行")
  eq(string.find(chat199, "掘地鼠", 1, true) ~= nil, true, "②★★★行里有稀有名字（从对方 Database:GetUnitName 取）")
  eq(string.find(chat199, "稀有精英", 1, true) ~= nil, true, "②★★品阶文本 = 稀有精英（rank 2）")
  eq(string.find(chat199, "87", 1, true) ~= nil, true, "②★★码数在（87）")
  eq(string.find(chat199, "另有 2 只", 1, true) ~= nil, true, "②★同刻还有 2 只 → 后缀在")
  eq(EVAL_TEST_RW_STATE().seen, 1, "②★已转播计数 +1")
  eq(EVAL_TEST_RW_STATE().lastName, "掘地鼠", "②★读值口记着最后一只")

  -- ③ 透明性：原函数抛错 → 照原样抛出 + 计数（绝不吞）
  EVAL_TEST_RW_RESET()
  local ra199b = mkRA199(function() error("boom199") end)
  mods199.RareAlert = ra199b
  eq(EVAL_RW_INSTALL(), "hook", "③前置：新表也能装上")
  TEST.chat = nil
  local okE199, errE199 = pcall(ra199b.Show, ra199b, { unitId = 7, rank = 4, coords = {} }, 10, 0, 0, 0)
  eq(okE199, false, "③★★原函数抛错 → 我们**照原样抛出去**（吞掉别人的错才是真 bug）")
  eq(string.find(tostring(errE199), "boom199", 1, true) ~= nil, true, "③★抛出的还是原来的错误文本")
  eq(EVAL_TEST_RW_STATE().fails, 1, "③★而且如实计数")
  eq(EVAL_RW.seen, 1, "③★抛错这一次**不**转播（没弹出来的东西不该报）")
  eq(TEST.chat, nil, "③★抛错时聊天框一个字都没有")

  -- ④ 兜底：Show 不可包 → 轮询公开计数
  EVAL_TEST_RW_RESET()
  local ra199c = mkRA199()
  ra199c.Show = "not a function" -- 对方换实现（Show 不再是函数）
  ra199c.lastName, ra199c.lastRank, ra199c.lastDistance = "灰熊", "稀有", 55
  mods199.RareAlert = ra199c
  eq(EVAL_RW_INSTALL(), "poll", "④★★Show 不可包 → 退化为**轮询兜底**")
  eq(EVAL_TEST_RW_STATE().hasTick, true, "④★兜底路径才建 tick 帧")
  TEST.chat = nil
  ra199c.stats.alerts = 5
  eq(EVAL_RW_POLL(), false, "④★首读只立基准（本次登录之前的历史计数不算新弹窗）")
  eq(TEST.chat, nil, "④★★首读一个字都不输出")
  ra199c.stats.alerts = 6
  eq(EVAL_RW_POLL(), true, "④★★计数涨了 → 转播")
  local chat199c = tostring(TEST.chat or "")
  eq(string.find(chat199c, "灰熊", 1, true) ~= nil, true, "④★★兜底路径也用对方给的本地化名字")
  eq(string.find(chat199c, "稀有", 1, true) ~= nil, true, "④★兜底路径带品阶文本")
  eq(string.find(chat199c, "55", 1, true) ~= nil, true, "④★兜底路径带码数")
  eq(EVAL_RW_POLL(), false, "④★同一计数不重复转播")
  -- 节流：0.5 秒内第二次 tick 不干活（高频防护）
  EVAL_RW.next = nil
  EVAL_RW_TICK_STEP()
  ra199c.stats.alerts = 7
  eq(EVAL_RW_TICK_STEP(), false, "④★0.5 秒节流：同刻第二次 tick 不查（也不输出）")

  -- ⑤⑥⑦ 命令入口（项目标准：命令也是接线，必须走真实 SlashCmdList）
  EVAL_TEST_RW_RESET()
  local ra199d = mkRA199()
  mods199.RareAlert = ra199d
  EVAL_RW_INSTALL()
  EVAL_RW.seen = 0
  eq(SlashCmdList ~= nil and type(SlashCmdList["EVALHELP"]) == "function", true, "⑤前置：/eh 命令入口在")
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有 关")
  eq(string.find(tostring(TEST.chat or ""), "关", 1, true) ~= nil, true, "⑤★「稀有 关」有回执")
  TEST.chat = nil
  ra199d.Show(ra199d, { unitId = 7, rank = 3, coords = {} }, 20, 0, 0, 0)
  eq(TEST.chat, nil, "⑤★★开关关着 → 一个字节都不输出")
  eq(EVAL_RW.seen, 0, "⑤★而且转播计数不动")
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有 开")
  ra199d.Show(ra199d, { unitId = 7, rank = 3, coords = {} }, 20, 0, 0, 0)
  eq(string.find(tostring(TEST.chat or ""), "首领", 1, true) ~= nil, true, "⑤★开回来照常输出（rank 3 = 首领）")
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有")
  local st199 = tostring(TEST.chat or "")
  eq(string.find(st199, "稀有提醒转播", 1, true) ~= nil, true, "⑥★「稀有」状态走真实入口")
  eq(string.find(st199, "已转播", 1, true) ~= nil, true, "⑥★报出已转播次数")
  eq(string.find(st199, "用法", 1, true) ~= nil, true, "⑥★带用法行")
  eq(string.find(st199, "最后一只", 1, true) ~= nil, true, "⑥★报出最后一只")
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有 试")
  eq(ra199d.lastEntry ~= nil, true, "⑦★★「试」真的调了对方的测试弹窗（走完整链路）")
  eq(string.find(tostring(TEST.chat or ""), "测试稀有", 1, true) ~= nil, true, "⑦★如实回报测试结果")
  eq(string.find(tostring(TEST.chat or ""), "稀有提醒", 1, true) ~= nil, true, "⑦★★而且这一次的弹窗也被转播了（端到端自证）")
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有 乱写")
  eq(string.find(tostring(TEST.chat or ""), "用法", 1, true) ~= nil, true, "⑦★参数不认识 → 打用法（不静默）")
  eq(string.len("go 稀有"), 9, "⑦★★前缀字节数 = 9（string.sub 是字节下标 —— 1.74.5 在「go 喂食」上正是栽在这里）")

  -- ⑧ 对方缺席：如实记 absent，不报错、不静默
  EVAL_TEST_RW_RESET()
  rawset(_G, "UnrealQuest", nil)
  eq(EVAL_RW_INSTALL(), "absent", "⑧★★任务插件缺席 → 如实记 absent（不报错）")
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有")
  eq(string.find(tostring(TEST.chat or ""), "UnrealQuest", 1, true) ~= nil, true, "⑧★命令里说明是任务插件没装/没启用")
  ...
  eq(EVAL_L("RW_MORE", 3) == "，同刻另有 3 只也进了范围", true, "⑨★带参数的键格式正确（%d 落位）")
    ...
    -- ⑩ 染色（用户：「对稀有精英染色」）：名字按品阶上色，且**整条只有一段 8 位色码**
  TEST.chat = nil
  EVAL_RW.seen = 0
  ra199d:Show({ unitId = 7, rank = 2, coords = {} }, 40, 0, 0, 0)
  local chat199e = tostring(TEST.chat or "")
  eq(string.find(chat199e, "|cffa335ee", 1, true) ~= nil, true, "⑩★★稀有精英（rank 2）的名字 = 紫 a335ee")
  -- ★数色码时只数**转播那一行**：say() 会给每条行首加自己的 EVAL_HELP 前缀色码，那不是这条消息的成分
  local function alertLine199(txt)
    local s = tostring(txt or "")
    local from = 1
    while true do
      local e = string.find(s, "\n", from, true)
      local seg = string.sub(s, from, (e and e - 1) or -1)
      if string.find(seg, "稀有提醒：", 1, true) then return seg end
      if not e then return "" end
      from = e + 1
    end
  end
  -- say() 的行首前缀（|cff66ccffEVAL_HELP:|r）与这条消息在**同一行**，先摘掉它再数
  local body199 = string.gsub(alertLine199(chat199e), "|cff66ccffEVAL_HELP:|r", "", 1)
  local _, codeCnt = string.gsub(body199, "|c%x%x%x%x%x%x%x%x", "")
  eq(codeCnt, 1, "⑩★★★整条转播行**只有一段 8 位色码**（1.73.43h 实测：一条多段会整条不画）")
  TEST.chat = nil
  ra199d:Show({ unitId = 7, rank = 4, coords = {} }, 40, 0, 0, 0)
  eq(string.find(tostring(TEST.chat or ""), "|cff0070dd", 1, true) ~= nil, true, "⑩★稀有（rank 4）= 蓝 0070dd")
  TEST.chat = nil
  ra199d:Show({ unitId = 7, rank = 3, coords = {} }, 40, 0, 0, 0)
  eq(string.find(tostring(TEST.chat or ""), "|cffff8000", 1, true) ~= nil, true, "⑩★首领（rank 3）= 橙 ff8000")
  eq(EVAL_RW_RANK_COLOR(nil), EVAL_RW_RANK_COLOR_FALLBACK, "⑩★品阶未知 → 兜底色（绝不允许没有色码）")
  local n8_199 = 0
  for i = 1, 4 do if string.len(tostring(EVAL_RW_RANK_COLOR(i))) == 10 then n8_199 = n8_199 + 1 end end
  eq(n8_199, 4, "⑩★四档色码逐项都是 |c + 8 位（6 位不解析 / 7 位吞整条）")

  -- ⑪ 链接形态：**只有名字**（★1.74.26 用户定案：不跟那边插件的机制耦合 ⇒ 链接里不再有 areaId/坐标/unitId）
  --   ★⑧ 段把全局 UnrealQuest 置了 nil（缺席用例）⇒ 这里要把它放回来，否则安装会如实报 absent、什么都不发生。
  rawset(_G, "UnrealQuest", { GetModule = function(_, name) return mods199[name] end })
  EVAL_TEST_RW_RESET()
  local ra199e = mkRA199()
  mods199.RareAlert = ra199e
  EVAL_RW_INSTALL()
  TEST.chat = nil
  ra199e:Show({ unitId = 7, rank = 2, coords = { { 90, 90, 14 }, { 12, 12, 14 } } }, 30, 0, 0, 0)
  local chat199f = tostring(TEST.chat or "")
  eq(string.find(chat199f, "|cffa335ee|HEHRW:掘地鼠|h[掘地鼠]|h|r", 1, true) ~= nil, true,
     "⑪★★★名字是「色码 + 链接」形态，且链接 token **只带名字**（EHRW:掘地鼠）")
  eq(string.find(chat199f, "EHRW:14", 1, true), nil, "⑪★★反向哨兵：链接里**不再**出现 areaId/坐标（与那边插件的地图机制彻底解耦）")

  -- ⑫ 点名字 = 选中目标（用户：「调用类似目标函数选中目标就可以了.如果目标不存在就报错.」）
  eq(type(_G.SetItemRef) == "function", true, "⑫前置：客户端的 SetItemRef 在")
  EVAL_TB_SETITEMREF_INSTALL()
  local savedNear199, savedCur199 = TEST.nearby, TEST.curTargetName
  TEST.nearby, TEST.curTargetName = { "掘地鼠" }, "别的怪"
  TEST.byNameArg = nil
  TEST.chat = nil
  local clicked199 = EVAL_TEST_SIR_CLICK("EHRW:掘地鼠")
  eq(clicked199, true, "⑫★点链接被**吃掉**了（没有丢给客户端的物品处理函数）")
  eq(TEST.byNameArg, "掘地鼠", "⑫★★★真的调了 TargetByName(名字)")
  eq(string.find(tostring(TEST.chat or ""), "已选中目标", 1, true) ~= nil, true, "⑫★★选中了就报「已选中目标」")
  eq(EVAL_TEST_RW_STATE().tgtTries >= 1, true, "⑫★尝试被计数")
  -- 反向：选不中 → **报错**（用户要求），不许假装成功、也不许解释一堆机制
  TEST.nearby, TEST.curTargetName = nil, "别的怪"
  TEST.chat = nil
  EVAL_TEST_SIR_CLICK("EHRW:掘地鼠")
  local chat199g = tostring(TEST.chat or "")
  eq(string.find(chat199g, "选不中目标", 1, true) ~= nil, true, "⑫★★★选不中 → **报错**（用户：「如果目标不存在就报错」）")
  eq(string.find(chat199g, "已选中目标", 1, true), nil, "⑫★★★反向哨兵：不许报成功")
  eq(string.find(chat199g, "码", 1, true), nil, "⑫★★反向哨兵：不再夹带距离/机制解释（用户要求逻辑简单）")
  TEST.nearby, TEST.curTargetName = savedNear199, savedCur199

  -- ⑬ 任意键都做同一件事（行为可预测；不再有「右键 = 另一个动作」这种隐藏分支）
  TEST.nearby, TEST.curTargetName = { "掘地鼠" }, "别的怪"
  TEST.byNameArg = nil
  TEST.chat = nil
  -- 走**真实** SetItemRef 包装（按钮参数直接透传）
  local sir199 = _G.SetItemRef
  local okR199 = pcall(sir199, "EHRW:掘地鼠", "[链接]", "RightButton")
  eq(okR199, true, "⑬★右键走真实 SetItemRef 不报错")
  eq(TEST.byNameArg, "掘地鼠", "⑬★★右键也是「选中目标」这一个动作")
  TEST.nearby, TEST.curTargetName = savedNear199, savedCur199

  -- ⑭ 命令入口：稀有 目标 / 链接（项目标准：命令也是接线）
  TEST.nearby, TEST.curTargetName = { "掘地鼠" }, "别的怪"
  TEST.byNameArg = nil
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有 目标")
  eq(TEST.byNameArg, "掘地鼠", "⑭★/eh go 稀有 目标 = 选中本局那只")
  eq(string.find(tostring(TEST.chat or ""), "已选中目标", 1, true) ~= nil, true, "⑭★并如实回报")
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有 链接")
  eq(EVAL_TEST_RW_STATE().linkOn, false, "⑭★稀有 链接 → 关掉了链接开关")
  TEST.chat = nil
  ra199e:Show({ unitId = 7, rank = 2, coords = { { 12, 12, 14 } } }, 30, 0, 0, 0)
  local chat199h = tostring(TEST.chat or "")
  eq(string.find(chat199h, "|H", 1, true), nil, "⑭★★链接关掉后**没有** |H（退回已验证的「只有色码」形态）")
  eq(string.find(chat199h, "|cffa335ee", 1, true) ~= nil, true, "⑭★但染色还在（退的是链接，不是功能）")
  SlashCmdList["EVALHELP"]("go 稀有 链接")
  eq(EVAL_TEST_RW_STATE().linkOn, true, "⑭★再点一次又开回来")
  TEST.nearby, TEST.curTargetName = savedNear199, savedCur199

  -- ⑯ 目标探针（自检命令）：用**当前目标自己的名字**（必然存在、必然在附近）验接口能不能用 ——
  --   这是唯一能区分「客户端只认附近」与「TargetByName 对本插件是空操作」的实验。
  TEST.nearby, TEST.curTargetName = { "掘地鼠" }, "掘地鼠"
  TEST.byNameArg = nil
  TEST.chat = nil
  SlashCmdList["EVALHELP"]("go 稀有 目标探针")
  local chat199p = tostring(TEST.chat or "")
  eq(string.find(chat199p, "TargetByName", 1, true) ~= nil, true, "⑯★探针先报出接口存在与否")
  eq(TEST.byNameArg, "掘地鼠", "⑯★★探针真的拿「当前目标的名字」去选回来（必然存在且必然在附近）")
  eq(string.find(chat199p, "接口", 1, true) ~= nil, true, "⑯★★给出明确判定：接口正常 / 是空操作")
  eq(string.find(chat199p, "本局那只是", 1, true) ~= nil, true, "⑯★并打印本局那只的名字（与屏幕上看到的逐字对照）")
  TEST.nearby, TEST.curTargetName = savedNear199, savedCur199
-- 收尾：替换全局必须 save/restore（1.68.2 教训）+ 清掉转播状态
  EVAL_TEST_RW_RESET()
  EVAL_RW.seen, EVAL_RW.fails = 0, 0
  EVAL_RW.lastName, EVAL_RW.lastRankText, EVAL_RW.lastDistance = nil, nil, nil
  EVAL_RW.next = nil
  if savedUQ199 == nil then rawset(_G, "UnrealQuest", nil) else rawset(_G, "UnrealQuest", savedUQ199) end
  EVAL_RW_SET(savedRW199 ~= false)
...
function EVAL_TEST_192_CHAINKINDS()
-- ⑧y ★1.75.18 用户要求「任务线->档位过滤使用装备那边的种类过滤」
--   ① 任务线视图的种类与装备视图**同一份状态**（DS.qpKinds，界面不再有档位筛）
--   ② 真选一个种类 → 列表只剩「奖励里真有该种类」的线（判据在数据层 EVAL_QC_SEARCH）
--   ③ 空集 = 不过滤（与装备视图同语义）
EVAL_QP_SHOW()
...
EVAL_QP_SET_KINDS({})
local kAll = table.getn(EVAL_QP_HITS())
-- 找一种「确实被任务线奖励」的种类（由数据现算，不写死种类名）
local kK = nil
local kChains = EVAL_QC_LIST() or {}
for i = 1, table.getn(kChains) do
  local ids = EVAL_QC_CHAIN_REWARDS(kChains[i].k, 99) or {}
  for j = 1, table.getn(ids) do
    local it = EVAL_QC_ITEM(ids[j]) or EVAL_QC_BULK_ITEM(ids[j])
    if it then
      local kk = EVAL_QC_ITEM_KIND(it)
      if kk ~= "" then kK = kk break end
    end
  end
  if kK then break end
end
eq(type(kK) == "string", true, "⑧y前置：找到一种被任务线奖励的种类（" .. tostring(kK) .. "）")
EVAL_QP_SET_KINDS({ [kK] = true })
local kHit = EVAL_QP_HITS()
local kBad = 0
for i = 1, table.getn(kHit) do
  local rw = (kHit[i].c or {}).rw or {}
  local okOne = false
  for j = 1, table.getn(rw) do
    local it = EVAL_QC_ITEM(rw[j]) or EVAL_QC_BULK_ITEM(rw[j])
    if it and EVAL_QC_ITEM_KIND(it) == kK then okOne = true break end
  end
  if not okOne then kBad = kBad + 1 end
end
eq(table.getn(kHit) >= 1 and kBad == 0, true,
  "⑧y★★按种类「" .. tostring(kK) .. "」筛任务线 → 只剩奖励里有该种类的线（越界 " .. tostring(kBad) ..
  " · 共 " .. tostring(table.getn(kHit)) .. " 条）")
eq(table.getn(kHit) < kAll, true, "⑧y★筛选真的收窄了（" .. tostring(kAll) .. " → " .. tostring(table.getn(kHit)) .. "）")
EVAL_QP_SET_KINDS({})
eq(table.getn(EVAL_QP_HITS()) == kAll, true, "⑧y★清空种类 → 回到全部（空集 = 不过滤）")
end

function EVAL_TEST_192_PLACEHOLDER()
-- ⑧z ★1.75.18 用户要求：没扫描出装备时用**宏 961 图标**占位 + 提示「数据更新中」；
--   **主动点击** = 把这件装备插到检索队列**最前面**优先扫描；扫到之后自动换成真图标与链接信息。
EVAL_QP_SHOW()
EVAL_QP_SET_TAB("chain")
EVAL_QP_SET_QUERY("")
EVAL_QP_SET_FILTER("ALL", "ALL")
EVAL_QP_SET_KINDS({})
EVAL_QP_PAGE(-999)
local kRow, kSlot, kId = nil, nil, nil
for i = 1, 17 do
  for j = 1, 6 do
    local id = EVAL_QP_ROW_RW_ID(i, j)
    if type(id) == "number" and not (TEST.itemInfo and TEST.itemInfo[id]) then
      local isPh = EVAL_QP_TEST_ROWPH(i, j)
      if isPh then kRow, kSlot, kId = i, j, id break end
    end
  end
  if kRow then break end
end
eq(kRow ~= nil, true, "⑧z前置：这一页找到「未扫描出」的奖励格（画占位图）")
if kRow then
  local kWant = nil
  if type(GetMacroIconInfo) == "function" then
    local ok, t = pcall(GetMacroIconInfo, 961)
    if ok then kWant = t end
  end
  local kPh, kTex = EVAL_QP_TEST_ROWPH(kRow, kSlot)
  eq(kPh == true and type(kTex) == "string" and kTex ~= "", true, "⑧z★★占位图真的画出来了（" .. tostring(kTex) .. "）")
  if type(kWant) == "string" and kWant ~= "" then
    eq(kTex == kWant, true, "⑧z★★占位图 = 游戏内**宏 961** 图标（" .. tostring(kTex) .. " vs " .. tostring(kWant) .. "）")
  end
  local kBtn = EVAL_QP_TEST_ROWPHBTN(kRow, kSlot)
  eq(kBtn ~= nil, true, "⑧z占位格有真按钮（悬停/点击挂在它上面）")
  local kOnEnt = (kBtn and kBtn.GetScript) and kBtn:GetScript("OnEnter") or nil
  local kShow0 = (TEST.gtCalls and TEST.gtCalls.show) or 0
  if type(kOnEnt) == "function" then kOnEnt() end
  eq((TEST.gtCalls and TEST.gtCalls.show or 0) > kShow0, true, "⑧z★★悬停占位格 → 出「数据更新中」提示（原生 tooltip 真被调用）")
  -- 点击 → 插到**队首** + 计数 +1
  local kPri0 = EVAL_QP_REQ_STATE().pri
  local kOnClk = (kBtn and kBtn.GetScript) and kBtn:GetScript("OnClick") or nil
  eq(type(kOnClk) == "function", true, "⑧z占位格挂了 OnClick")
  if type(kOnClk) == "function" then kOnClk() end
  local kQ = EVAL_QP_REQ_QUEUE()
  eq(kQ[1] == kId, true, "⑧z★★点占位 → 这件装备插到**队首**（队首 " .. tostring(kQ[1]) .. " · 期望 " .. tostring(kId) .. "）")
  eq(EVAL_QP_REQ_STATE().pri == kPri0 + 1, true, "⑧z★优先刷新计数 +1")
  -- 扫到之后：不再是占位、换成真图标（点击回到装备详情那条路）
  TEST.itemInfo = TEST.itemInfo or {}
  TEST.itemInfo[kId] = { name = "扫到了", q = 3, t = "武器", st = "剑" }
  EVAL_QP_REFRESH()
  local kPh2, kTex2 = EVAL_QP_TEST_ROWPH(kRow, kSlot)
  eq(kPh2 == false, true, "⑧z★★扫到之后不再是占位图")
  eq(type(kTex2) == "string" and kTex2 ~= kTex, true, "⑧z★★扫到之后换成真图标（" .. tostring(kTex2) .. "）")
  TEST.itemInfo[kId] = nil
end
end

function EVAL_TEST_192_QUEUEORDER()
-- ⑧aa ★1.75.19 用户实测「装备视图能及时显示图标，任务线这边不能」：
--   任务线**一行最多 6 个奖励图标**（装备视图每行 1 个）⇒ 队列若按行排，前几行先把 24 格占满，
--   后面的行要等好几轮重绘才轮得到。判据 = **按列排队**：某行第二件必须排在**更靠后那行的第一件**之后。
EVAL_QP_SHOW()
EVAL_QP_SET_TAB("chain")
EVAL_QP_SET_QUERY("")
EVAL_QP_SET_FILTER("ALL", "ALL")
EVAL_QP_SET_KINDS({})
-- 等级档拨到 60+（奖励最密集的档：首屏 9 行共 66 件）——真点下拉，顺带验 F1 在最左
EVAL_DD_TEST_RESET_ANCHOR()
local kF1 = EVAL_QP_TEST_F1()
local kF1On = (kF1 and kF1.GetScript) and kF1:GetScript("OnClick") or nil
if type(kF1On) == "function" then kF1On() end
local kLvTxts = EVAL_DD_TEST_VISIBLE_TEXTS()
local kLv60 = nil
for i = 1, table.getn(kLvTxts or {}) do if tostring(kLvTxts[i]) == "60+" then kLv60 = i break end end
if kLv60 then EVAL_DD_TEST_CLICK(kLv60) end
eq(kLv60 ~= nil, true, "⑧aa前置：等级下拉里有 60+ 档")
-- 把这一页的奖励全部当作**未缓存**（桩缓存清掉 → 队列会重新收满，情形可控）
if type(TEST.itemInfo) == "table" then
  for i = 1, 17 do
    for j = 1, 6 do
      local id = EVAL_QP_ROW_RW_ID(i, j)
      if type(id) == "number" then TEST.itemInfo[id] = nil end
    end
  end
end
EVAL_QP_REFRESH()
-- 判据：把队列里的 id 映射回 **(槽位, 行)** 坐标，必须按「槽位升序 → 同行按行升序」排。
--   ★不能拿「该行第一个未缓存件」当坐标 —— 第一件可能已在缓存里，那它本来就在后面的槽位上
--   （首版就是这么误判的：把 slot2 的件当成 slot1，断言自然失败）。
local kCoord = {}
for i = 1, 17 do
  for j = 1, 6 do
    local id = EVAL_QP_ROW_RW_ID(i, j)
    if type(id) == "number" and kCoord[id] == nil then kCoord[id] = { s = j, r = i } end
  end
end
local kQ = EVAL_QP_REQ_QUEUE()
-- ★同一件装备可能同时是好几行的奖励（幂等去重后队列里只有一条）⇒ 用它的**最小槽位**判定，
--   否则会把它当成「后面那行的第 2 格」而误判（首版就是这么出现 6 处假逆序的）。
local kMinSlot = {}
for i = 1, 17 do
  for j = 1, 6 do
    local id = EVAL_QP_ROW_RW_ID(i, j)
    if type(id) == "number" and (kMinSlot[id] == nil or j < kMinSlot[id]) then kMinSlot[id] = j end
  end
end
local kLast1, kFirst2, kSeen = 0, 999, 0
for i = 1, table.getn(kQ) do
  local s = kMinSlot[kQ[i]]
  if s then
    kSeen = kSeen + 1
    if s == 1 and i > kLast1 then kLast1 = i end
    if s == 2 and i < kFirst2 then kFirst2 = i end
  end
end
if kSeen >= 3 then end -- （调试用的队列转储已移除；判据只看 kLast1 < kFirst2）
eq(kSeen >= 3 and kLast1 < kFirst2, true,
  "⑧aa★★队列**按列**排队：所有「每行第一件」都要排在「第二件」之前（末件1 @" .. tostring(kLast1) ..
  " · 首件2 @" .. tostring(kFirst2) .. " · 落在本页 " .. tostring(kSeen) .. " 件）")
-- 收尾：等级档拨回「全部」（不清会让后面的 ⑧d 之类按链名检索全部落空 —— 上一版就漏了这一步）
EVAL_DD_TEST_RESET_ANCHOR()
local kF1b = EVAL_QP_TEST_F1()
local kF1bOn = (kF1b and kF1b.GetScript) and kF1b:GetScript("OnClick") or nil
if type(kF1bOn) == "function" then kF1bOn() end
EVAL_DD_TEST_CLICK(1)
EVAL_QP_SET_FILTER("ALL", "ALL")
EVAL_QP_SET_KINDS({})
end

function EVAL_TEST_192_QUEUECOL1()
-- ⑧bb ★1.75.20 用户实测「装备视图能及时显示图标，任务线这边不能」的**结构性**判据：
--   24 格上限若在**入队时**就按行序施加（1.75.20 之前的写法），前几行（一行最多 6 个奖励）
--   就把队列占满 ⇒ 排在后面的行**连排队资格都没有**，事后重排也救不回来，图标迟迟不出。
--   判据 = 整页收尾截断之后：① **每一个有奖励的可见行的第一件**都在队列里；
--   ② 且它们正好占据**队首那一段**（按列排队的定义 = 所有行的第 1 件排在一起、内部按行序）。
--   ★用 60+ 档：首屏 9 行有奖励、共 66 件 —— 唯一会超过 24 格的档（10-19 档首屏只有 2 行有奖励，
--     行序与列序没差别，**测不出**这个缺陷）。
EVAL_QP_SHOW()
EVAL_QP_SET_TAB("chain")
EVAL_QP_SET_QUERY("")
...
EVAL_QP_SET_KINDS({})
EVAL_DD_TEST_RESET_ANCHOR()
local qbF1 = EVAL_QP_TEST_F1()
local qbF1On = (qbF1 and qbF1.GetScript) and qbF1:GetScript("OnClick") or nil
if type(qbF1On) == "function" then qbF1On() end
local qbTxts = EVAL_DD_TEST_VISIBLE_TEXTS()
local qb60 = nil
for i = 1, table.getn(qbTxts or {}) do if tostring(qbTxts[i]) == "60+" then qb60 = i break end end
if qb60 then EVAL_DD_TEST_CLICK(qb60) end
eq(qb60 ~= nil, true, "⑧bb前置：等级下拉里有 60+ 档")
-- 整页奖励全部当**未缓存**（桩缓存清掉 ⇒「件件都还没扫出来」的情形可控）
if type(TEST.itemInfo) == "table" then
  for i = 1, 17 do
    for j = 1, 6 do
      local id = EVAL_QP_ROW_RW_ID(i, j)
...
EVAL_QP_REFRESH()
-- 期望值只从**原生数据**推（每行的第一件），与生产排序逻辑不同源
local qbWant, qbSeen, qbRows = {}, {}, 0
for i = 1, 17 do
  for j = 1, 6 do
    local id = EVAL_QP_ROW_RW_ID(i, j)
    if type(id) == "number" then
      if j == 1 then qbRows = qbRows + 1 end
      if j == 1 and not qbSeen[id] then
        qbSeen[id] = true
        qbWant[table.getn(qbWant) + 1] = id
      end
      break
    end
  end
end
local qbQ = EVAL_QP_REQ_QUEUE()
local qbSet = {}
for i = 1, table.getn(qbQ) do qbSet[qbQ[i]] = true end
local qbMiss = 0
for i = 1, table.getn(qbWant) do if not qbSet[qbWant[i]] then qbMiss = qbMiss + 1 end end
eq(table.getn(qbWant) >= 5 and qbMiss == 0, true,
  "⑧bb★★每一个有奖励的可见行的**第一件**都要排进队列（60+ 首屏 " .. tostring(qbRows) ..
  " 行有奖励 · 期望入队 " .. tostring(table.getn(qbWant)) .. " 件 · 缺 " .. tostring(qbMiss) .. " 件）")
local qbBadAt = 0
for i = 1, table.getn(qbWant) do
  if qbQ[i] ~= qbWant[i] then qbBadAt = i break end
end
eq(qbBadAt == 0, true,
  "⑧bb★★队首那一段必须正好是「按行序的各行第一件」（前 " .. tostring(table.getn(qbWant)) ..
  " 位 · 首个不符 @" .. tostring(qbBadAt) .. " · 队列共 " .. tostring(table.getn(qbQ)) .. " 件 ≤ 24）")
eq(table.getn(qbQ) <= 24, true, "⑧bb上限仍然生效（排序后截到 24 · 实际 " .. tostring(table.getn(qbQ)) .. "）")
-- 收尾：等级档拨回「全部」（不清会让后面的 ⑧d 之类按链名检索全部落空）
EVAL_DD_TEST_RESET_ANCHOR()
local qbF1b = EVAL_QP_TEST_F1()
local qbF1bOn = (qbF1b and qbF1b.GetScript) and qbF1b:GetScript("OnClick") or nil
if type(qbF1bOn) == "function" then qbF1bOn() end
EVAL_DD_TEST_CLICK(1)
EVAL_QP_SET_FILTER("ALL", "ALL")
EVAL_QP_SET_KINDS({})
end
...
-- ★★省时开关（1.75.21）：本块约 1300 行、**每次刷新都要在 fengari 里跑一遍「673 条任务线全量检索
--   + 17 行重绘」**，实测独占全套 28s 里的 ~22s。日常小改（不碰任务线/弹窗）用 `node check.js --quick`
--   把它整块跳过（`EVAL_TEST_SKIP_HEAVY=true`，由 test_engine.js 的 --quick 注入）；
--   ★**发版前、改动任务线/弹窗/数据层时，必须跑全量**（`node check.js`）。
do
  if EVAL_TEST_SKIP_HEAVY then
    print("GROUP 192 (任务线推荐弹窗): SKIPPED（省时模式：跳过的正是任务线弹窗断言 —— 改动这块时请跑全量）")
  else
  local fails0 = TESTASSERT_FAILS
  ...
  -- ★1.75.10 需求反转：任务线不再「S 档优先」，改成**按等级排序**（用户要求），行文字 =
  --   [档位·稀有度] 名字 · 地区 · 等级区间，且**按稀有度染色**。
  eq(string.find(t3, "[", 1, true) == 1, true, "⑦任务线首行以 [档位·稀有度] 开头（" .. tostring(t3) .. "）")
  eq(string.find(t3, "·", 1, true) ~= nil, true, "⑦行里带稀有度标签（" .. tostring(t3) .. "）")
  eq(type(r3 and r3.key) == "string", true, "⑦任务线行带链键")
  -- 行色必须等于**数据层**给的稀有度色（界面写死颜色 = 变异必存活）
  do
    local tok, er, eg, eb = EVAL_QP_RARITY_OF(r3.key)
    local cr, cg, cb = r3.text:GetTextColor()
    local near = function(a, b) return type(a) == "number" and type(b) == "number" and math.abs(a - b) < 0.01 end
    eq(near(cr, er) and near(cg, eg) and near(cb, eb), true,
      "⑦★★行色 == 数据层稀有度色（" .. tostring(tok) .. " 实际 " .. tostring(cr) .. "/" .. tostring(cg) .. "/" .. tostring(cb) .. "）")
  end
  -- 排序：前若干行的等级必须单调不降（等级是第一排序键）
  do
    local prev, rising = nil, true
    for i = 1, math.min(10, table.getn(h3)) do
      local lo = h3[i].c and tonumber(h3[i].c.lo) or nil
      if lo and prev and lo < prev then rising = false end
      if lo then prev = lo end
    end
    eq(rising, true, "⑦★★任务线按等级排序（前 10 行 lo 单调不降）")
  end
  ...
  EVAL_TEST_192_CHAINKINDS()
  EVAL_TEST_192_PLACEHOLDER()
  EVAL_TEST_192_QUEUEORDER()
  EVAL_TEST_192_QUEUECOL1()
  ...
  -- ★1.75.13 探针**自查行**：报出任务线**全量条数**（派生事实 ⇒ 用户一眼看出载入的是新版还是旧副本；
  --   旧实现写死 200 条 × 等级升序 = 只出 200 条、最高 hi≈40，曾被误认为「数据没收集齐」）。（自成 do…end：200 local 上限）
  do
    local kAllQ = EVAL_QC_SEARCH("", 0, nil) or {}
    local kWant = tostring(table.getn(kAllQ))
    local kFound = false
    for i = 1, table.getn(probe) do
      local ln = tostring(probe[i] or "")
      if string.find(ln, "任务线列表", 1, true) and string.find(ln, kWant, 1, true) then kFound = true end
    end
    eq(table.getn(kAllQ) > 200 and kFound, true,
      "⑧e2 ★探针必须报出任务线**全量**条数（期望含 " .. kWant .. "；旧版恒 200 ⇒ 分辨载入的是哪一版）")
  end
  ...
  -- ⑧o ★1.75.10 三条用户要求：① 任务线按等级排序 ② 按稀有度用**魔兽品质色**染色（橙=传说风剑之类）
  --      ③ 任务线**奖励行**配图标（与武器列表同一套）+ 链接 tooltip。★整段自带 do…end（见 200 local 说明）
  do
    -- ① 调色板 = 魔兽品质色（字面夹具，独立于生产表；挡住「随手改色」与「界面写死色」）
    local PAL = {
      LEG = { 1.00, 0.50, 0.00 }, EPIC = { 0.64, 0.21, 0.93 }, RARE = { 0.00, 0.44, 0.87 },
      UNCOMMON = { 0.12, 1.00, 0.00 }, COMMON = { 1.00, 1.00, 1.00 }, POOR = { 0.62, 0.62, 0.62 },
    }
    local pBad, keys = {}, EVAL_QC_RARITY_KEYS()
    for i = 1, table.getn(keys) do
      local r, g, b = EVAL_QC_RARITY_RGB(keys[i])
      local e = PAL[keys[i]]
      if not (e and math.abs(r - e[1]) < 0.005 and math.abs(g - e[2]) < 0.005 and math.abs(b - e[3]) < 0.005) then
        table.insert(pBad, keys[i])
      end
    end
    eq(table.getn(keys) == 6, true, "⑧o★稀有度共 6 档（实际 " .. tostring(table.getn(keys)) .. "）")
    eq(table.getn(pBad) == 0, true, "⑧o★★颜色 = 魔兽品质色（异常 " .. table.concat(pBad, ",") .. "）")
    -- ② 传说（橙）必须真的落在「风剑/橙杖」这类线上（用户点名）
    --   ★注意：不能用 EVAL_QC_SEARCH("",500) —— 它会截断（22 策展 + 688 自动线 ⇒ 500 条装不下，
    --     60 级的风剑线正好被切掉，第一版就踩了这个坑）。直接遍历两份数据源，不设上限。
    local kAll = {}
    do
      local cs = EVAL_QC_LIST(nil)
      for i = 1, table.getn(cs) do kAll[table.getn(kAll) + 1] = cs[i] end
      local ss = EVAL_QC_SERIES_LIST()
      for i = 1, table.getn(ss) do kAll[table.getn(kAll) + 1] = ss[i] end
    end
    local legN, legHit = 0, false
    for i = 1, table.getn(kAll) do
      local rec = kAll[i]
      if EVAL_QC_RARITY(rec) == "LEG" then
        legN = legN + 1
        local hay = tostring(rec.n or "")
        local qs = rec.qs or {}
        for j = 1, table.getn(qs) do
          local q = (type(EVAL_QC_BULK_QUEST) == "function") and EVAL_QC_BULK_QUEST(qs[j]) or nil
          hay = hay .. " " .. tostring((q and q.n) or EVAL_QC_QUEST_NAME(qs[j]) or "")
        end
        if string.find(hay, "逐风者", 1, true) or string.find(hay, "雷霆之怒", 1, true) or string.find(hay, "埃提耶什", 1, true) then
          legHit = true
        end
      end
    end
    eq(legN >= 1, true, "⑧o★★任务线里有**传说（橙）**档（实际 " .. tostring(legN) .. " 条 · 共 " .. tostring(table.getn(kAll)) .. " 条线）")
    eq(legHit, true, "⑧o★★★传说档落在风剑/橙杖这类线上（用户点名「橙色给史诗风剑之类的任务」）")
    -- ③ 奖励行图标 + 链接 tooltip
    EVAL_QP_SET_TAB("chain")
    EVAL_QP_SET_QUERY("")
    EVAL_QP_LIST()
    local kz = nil
    -- ★1.75.14 排序改成**跨块按最低等级**后，首行不再必然是「有奖励的策展链」（首行现在是 4-5 级的小线）
    --   ⇒ 本组要验的是「**有装备奖励的**线的详情里奖励行有没有图标/tooltip」：从命中列表里挑第一条**确实有奖励**的线。
    local kHitsO = EVAL_QP_HITS() or {}
    for i = 1, table.getn(kHitsO) do
      local c = kHitsO[i] and kHitsO[i].c
      local k = c and c.k
      if type(k) == "string" then
        local rids = EVAL_QC_CHAIN_REWARDS(k, 1)
        if type(rids) == "table" and table.getn(rids) > 0 then kz = k break end
      end
    end
    eq(type(kz) == "string", true, "⑧o前置：拿到一条**带装备奖励**的链的键")
    EVAL_QP_DETAIL(kz)
    eq(EVAL_QP_MODE(), "detail", "⑧o进任务线详情")
    local kRew, kRewId = nil, nil
    for i = 1, 15 do
      local id = EVAL_QP_DET_ITEM(i)
      if type(id) == "number" then kRew, kRewId = i, id break end
    end
    eq(type(kRewId) == "number", true, "⑧o★详情里有奖励行且带装备 id（实际 " .. tostring(kRewId) .. "）")
    local kIcon, kHov = EVAL_QP_TEST_DETICONROW(kRew), EVAL_QP_TEST_DETHOVER(kRew)
    eq(kIcon ~= nil and kHov ~= nil, true, "⑧o★奖励行有图标控件与悬停热区")
    eq(kHov and type(kHov:GetScript("OnEnter")) == "function" and type(kHov:GetScript("OnLeave")) == "function", true,
      "⑧o★★奖励行挂了 Enter/Leave（链接 tooltip）")
    eq(kHov and type(kHov:GetScript("OnClick")) == "function", true, "⑧o★奖励行可点（进该装备详情）")
    TEST.itemInfo = TEST.itemInfo or {}
    TEST.itemInfo[kRewId] = { name = "测试装备", q = 2, t = "护甲", st = "布甲" }
    EVAL_QP_REFRESH()
    local kIcon2 = EVAL_QP_TEST_DETICONROW(kRew)
    eq((kIcon2 and kIcon2.IsShown and kIcon2:IsShown()) and true or false, true,
      "⑧o★★客户端认识该装备 → 图标签出来了（与武器列表同效果）")
    local gtBefore = (TEST.gtCalls and TEST.gtCalls.show) or 0
    local kOnEnter = kHov and kHov:GetScript("OnEnter")
    if type(kOnEnter) == "function" then kOnEnter() end
    eq(((TEST.gtCalls and TEST.gtCalls.show) or 0) > gtBefore, true, "⑧o★★悬停奖励行 → 真的弹了物品详情 tooltip")
    local kOnLeave = kHov and kHov:GetScript("OnLeave")
    if type(kOnLeave) == "function" then kOnLeave() end
    local kOnClick = kHov and kHov:GetScript("OnClick")
    if type(kOnClick) == "function" then kOnClick() end
    eq(EVAL_QP_ITEM_ID(), kRewId, "⑧o★★点奖励行 → 进该装备的详情（装备↔任务双向）")
    TEST.itemInfo[kRewId] = nil
    EVAL_QP_SET_TAB("chain")
    EVAL_QP_LIST()
  end

  -- ⑧p ★1.75.10 追加要求：「同个任务线如果装备奖励多个就都显示，并排在任务标题右边、左对齐」，
  --      且能**预览链接**（悬停出物品 tooltip）。★整段自带 do…end（200 local 说明）
  do
    EVAL_QP_SET_TAB("chain")
    EVAL_QP_SET_QUERY("")
    EVAL_QP_LIST()
    -- 找一行**有多件奖励**的链（用户要求多个都显示）
    local kRow, kIds = nil, nil
    for i = 1, 8 do
      local id1, id2 = EVAL_QP_ROW_RW_ID(i, 1), EVAL_QP_ROW_RW_ID(i, 2)
      if type(id1) == "number" and type(id2) == "number" then kRow, kIds = i, { id1, id2 } break end
    end
    eq(kRow ~= nil, true, "⑧p★★任务线行带**多件**奖励图标（找到第 " .. tostring(kRow) .. " 行）")
    if kRow then
      -- 客户端认识这些装备时：图标必须真的画出来、热区可点（与武器列表同一套 qpItemTex）
      TEST.itemInfo = TEST.itemInfo or {}
      for j = 1, table.getn(kIds) do
        TEST.itemInfo[kIds[j]] = { name = "测试奖励" .. tostring(j), q = 3, t = "武器", st = "剑" }
      end
      EVAL_QP_REFRESH()
      local slot1 = EVAL_QP_TEST_ROW_RW(kRow, 1)
      eq(slot1 ~= nil and slot1.tex ~= nil and slot1.btn ~= nil, true, "⑧p奖励槽是真控件（图标 + 热区）")
      eq((slot1 and slot1.tex:IsShown()) and true or false, true, "⑧p★★奖励图标签出来了（第一件）")
      local slot2 = EVAL_QP_TEST_ROW_RW(kRow, 2)
      eq((slot2 and slot2.tex:IsShown()) and true or false, true, "⑧p★★第二件奖励也并排显示（多个都显示）")
      -- 预览链接：悬停 → 物品 tooltip
      local okS, onEnter = pcall(slot1.btn.GetScript, slot1.btn, "OnEnter")
      eq(okS and type(onEnter) == "function", true, "⑧p奖励图标挂了 OnEnter（预览链接）")
      local gt0 = (TEST.gtCalls and TEST.gtCalls.show) or 0
      if type(onEnter) == "function" then onEnter() end
      eq(((TEST.gtCalls and TEST.gtCalls.show) or 0) > gt0, true, "⑧p★★悬停奖励图标 → 真的弹了物品链接 tooltip")
      local okL, onLeave = pcall(slot1.btn.GetScript, slot1.btn, "OnLeave")
      if okL and type(onLeave) == "function" then onLeave() end
      -- 点击 → 进该装备详情
      local okC, onClick = pcall(slot1.btn.GetScript, slot1.btn, "OnClick")
      eq(okC and type(onClick) == "function", true, "⑧p奖励图标可点")
      if type(onClick) == "function" then onClick() end
      eq(EVAL_QP_ITEM_ID(), kIds[1], "⑧p★★点奖励图标 → 进该装备详情")
      for j = 1, table.getn(kIds) do TEST.itemInfo[kIds[j]] = nil end
    end
    -- 反向哨兵：切到装备视图后，**同一行**的奖励槽必须被清掉（行池复用，残留就是 bug）
    EVAL_QP_SET_TAB("item")
    EVAL_QP_SET_QUERY("")
    EVAL_QP_LIST()
    local stale, staleShown = 0, 0
    for i = 1, 8 do
      if type(EVAL_QP_ROW_RW_ID(i, 1)) == "number" then stale = stale + 1 end
      local s = EVAL_QP_TEST_ROW_RW(i, 1)
      if s and s.tex and s.tex:IsShown() then staleShown = staleShown + 1 end
    end
    eq(stale == 0 and staleShown == 0, true,
      "⑧p★★★装备视图里不许残留任务线奖励图标（id 残留 " .. tostring(stale) .. " · 仍显示 " .. tostring(staleShown) .. "）")
    -- 文本宽度纯函数（本机 API 表里没有 GetStringWidth ⇒ 估算兜底：中文整宽、ASCII 半宽）
    eq(EVAL_QP_TEXT_W("", 9) == 0, true, "⑧p空串宽度 0")
    eq(EVAL_QP_TEXT_W("abcd", 9) == 16, true, "⑧p西文按半宽（4 × floor(9/2)=4 = 16）")
    eq(EVAL_QP_TEXT_W("中文四字", 9) == 36, true, "⑧p中文按**整宽**（4 字 × 9 = 36，不是按字节 108）")
    EVAL_QP_SET_TAB("chain")
    EVAL_QP_SET_QUERY("")
    EVAL_QP_LIST()
  end

  -- ⑧q ★1.75.11 用户实测「等级筛选 -> 无法点击」：真点那个按钮 → 真下拉 → 真选档 → 列表真的筛。
  --  根因 = 处理函数里 `local labels` 遮蔽了外层的 labels ⇒ EVAL_DD_OPEN 收到 nil（静默无反应）。
  --  ★本段自带 do…end（200 local 说明）
  do
    EVAL_QP_SHOW()
    EVAL_QP_SET_TAB("item")
    EVAL_QP_SET_QUERY("")
    EVAL_DD_TEST_RESET_ANCHOR()
    local kF1 = EVAL_QP_TEST_F1()
    eq(kF1 ~= nil, true, "⑧q等级筛选按钮存在")
    local kF1On = (kF1 and kF1.GetScript) and kF1:GetScript("OnClick") or nil
    eq(type(kF1On) == "function", true, "⑧q等级筛选挂了 OnClick")
    if type(kF1On) == "function" then kF1On() end
    eq(EVAL_DD_TEST_SHOWN(), true, "⑧q★★真点「等级」→ 下拉真的打开了（旧实现收到 nil，点了毫无反应）")
    -- 下拉里必须是 7 个等级档（ALL + 10-19/20-29/30-39/40-49/**50-59**/**60+**，字面数）
    --  ★1.75.15 用户要求「等级过滤增加个独立档位 60」⇒ 旧判据「6 档、最高 50+」当场反转。
    local kTxts = EVAL_DD_TEST_VISIBLE_TEXTS()
    eq(type(kTxts) == "table" and table.getn(kTxts) == 7, true,
      "⑧q★★下拉里 7 个等级档（实际 " .. tostring(type(kTxts) == "table" and table.getn(kTxts) or "nil") .. "）")
    eq(type(kTxts) == "table" and tostring(kTxts[6]) == "50-59" and tostring(kTxts[7]) == "60+", true,
      "⑧q★★末两档必须是 50-59 与 60+（实际 " .. tostring(type(kTxts) == "table" and kTxts[6] or "nil") .. " / " ..
      tostring(type(kTxts) == "table" and kTxts[7] or "nil") .. "）")
    -- 真选第 2 项（L1 = 10-19）→ 列表只出该区间（字面 10/19，独立于生产表）
    EVAL_DD_TEST_CLICK(2)
    local hq, kQBad = EVAL_QP_HITS(), 0
    for i = 1, table.getn(hq) do
      local lo = hq[i].lo or 0
      if lo < 10 or lo > 19 then kQBad = kQBad + 1 end
    end
    eq(table.getn(hq) >= 1 and kQBad == 0, true,
      "⑧q★★真选「10-19」→ 列表只剩该区间（越界 " .. tostring(kQBad) .. " · 共 " .. tostring(table.getn(hq)) .. " 件）")
    -- ★1.75.15 真选**末档 60+** → 列表只剩 hi ≥ 60 的线（装备视图按来源任务等级：lo 落在 60+）
    EVAL_DD_TEST_RESET_ANCHOR()
    if type(kF1On) == "function" then kF1On() end
    EVAL_DD_TEST_CLICK(7)
    local h6 = EVAL_QP_HITS()
    local kBad6 = 0
    for i = 1, table.getn(h6) do if (h6[i].lo or 0) < 60 then kBad6 = kBad6 + 1 end end
    eq(table.getn(h6) >= 1 and kBad6 == 0, true,
      "⑧q★★真选「60+」→ 装备列表只剩 60 级来源（越界 " .. tostring(kBad6) .. " · 共 " .. tostring(table.getn(h6)) .. " 件）")
    EVAL_QP_SET_FILTER("ALL", "ALL")
    EVAL_QP_SET_QUERY("")
  end

  -- ⑧u ★1.75.15 用户要求「**任务线**过滤页增加个等级过滤」：任务线视图此前根本没有等级过滤
  --   （F1 被档位、F2 被阵营占了）⇒ 用空着的 F3 位。判据走**真按钮 → 真下拉 → 真筛**。★自带 do…end
  do
    EVAL_QP_SHOW()
    EVAL_QP_SET_TAB("chain")
    EVAL_QP_SET_QUERY("")
    EVAL_DD_TEST_RESET_ANCHOR()
    local kF3 = EVAL_QP_TEST_F1()
    eq(kF3 ~= nil, true, "⑧u任务线视图有第三个过滤按钮（等级）")
    local kF3On = (kF3 and kF3.GetScript) and kF3:GetScript("OnClick") or nil
    eq(type(kF3On) == "function", true, "⑧u等级过滤挂了 OnClick")
    if type(kF3On) == "function" then kF3On() end
    eq(EVAL_DD_TEST_SHOWN(), true, "⑧u★★真点任务线「等级」→ 下拉打开（旧实现点了没反应/根本没这个按钮）")
    local kL6 = EVAL_DD_TEST_VISIBLE_TEXTS()
    eq(type(kL6) == "table" and table.getn(kL6) == 7 and tostring(kL6[7]) == "60+", true,
      "⑧u★★任务线等级档也是 7 档、末档 60+（实际 " .. tostring(type(kL6) == "table" and table.getn(kL6) or "nil") .. "）")
    -- 真选「60+」（第 7 项）→ 列表只剩**区间与 60 相交**的线（重叠判据）
    EVAL_DD_TEST_CLICK(7)
    local kC6 = EVAL_QP_HITS()
    local kC6Bad = 0
    for i = 1, table.getn(kC6) do
      local c = kC6[i].c or {}
      if (tonumber(c.hi) or 0) < 60 then kC6Bad = kC6Bad + 1 end
    end
    eq(table.getn(kC6) >= 1 and kC6Bad == 0, true,
      "⑧u★★真选「60+」→ 任务线只剩 hi≥60 的线（越界 " .. tostring(kC6Bad) .. " · 共 " .. tostring(table.getn(kC6)) .. " 条）")
    -- 反向哨兵：重叠判据 —— 「50-59」档必须**含**跨档长线（lo<50 但 hi≥50），不是「lo 落在档内」
    EVAL_DD_TEST_RESET_ANCHOR()
    if type(kF3On) == "function" then kF3On() end
    EVAL_DD_TEST_CLICK(6)
    local kC5 = EVAL_QP_HITS()
    local kCross, kC5Bad = 0, 0
    for i = 1, table.getn(kC5) do
      local c = kC5[i].c or {}
      local lo, hi = tonumber(c.lo) or 0, tonumber(c.hi) or 0
      if hi < 50 or lo > 59 then kC5Bad = kC5Bad + 1 end
      if lo < 50 and hi >= 50 then kCross = kCross + 1 end
    end
    eq(table.getn(kC5) >= 1 and kC5Bad == 0, true,
      "⑧u★★真选「50-59」→ 只剩区间相交的线（越界 " .. tostring(kC5Bad) .. " · 共 " .. tostring(table.getn(kC5)) .. " 条）")
    eq(kCross >= 1, true, "⑧u★★「50-59」必须含跨档长线（lo<50 且 hi≥50 的有 " .. tostring(kCross) .. " 条 = 重叠判据）")
    EVAL_QP_SET_FILTER("ALL", "ALL")
    EVAL_QP_SET_QUERY("")
  end

  -- ⑧v ★1.75.15 策展链扩到 62 条（30-60 经典线）后必须**去重**：
  --   同一条任务线若既在策展链又在自报系列里，列表会出现两行同名（用户根本分不清）。★自带 do…end
  do
    EVAL_QP_SHOW()
    EVAL_QP_SET_TAB("chain")
    EVAL_QP_SET_QUERY("")
    EVAL_QP_SET_FILTER("ALL", "ALL")
    -- 等级档拨回「全部」（上一段⑧u 停在 50-59，不拨回来这里就是筛选后的条数）
    EVAL_DD_TEST_RESET_ANCHOR()
    local kF3b = EVAL_QP_TEST_F1()   -- ★1.75.18 等级筛选挪到了最左（F1）
    local kF3bOn = (kF3b and kF3b.GetScript) and kF3b:GetScript("OnClick") or nil
    if type(kF3bOn) == "function" then kF3bOn() end
    EVAL_DD_TEST_CLICK(1)
    local kHitsV = EVAL_QP_HITS() or {}
    -- 判据口径：**跨集合**不许同名（策展链 ↔ 自报系列各出一条同名 = 用户看到两行）。
    --   ★同一集合内的同名是站点事实（同名的联盟版/部落版是两个不同系列），策展那侧已用
    --   「（联盟）/（部落）」后缀区分，不在这里判。
    local kCur, kSer = {}, {}
    for i = 1, table.getn(kHitsV) do
      local e = kHitsV[i] or {}
      local nm = tostring((e.c or {}).n or "")
      if nm ~= "" then if e.kind == "series" then kSer[nm] = true else kCur[nm] = true end end
    end
    local kCrossDup = 0
    for nm in pairs(kCur) do if kSer[nm] then kCrossDup = kCrossDup + 1 end end
    eq(kCrossDup == 0, true,
      "⑧v★★策展链与自报系列不得同名共存（去重失效 ⇒ " .. tostring(kCrossDup) .. " 个名字两处都在）")
    eq(table.getn(kHitsV) == table.getn(EVAL_QC_LIST()) + EVAL_QC_SERIES_COUNT(), true,
      "⑧v★列表条数 == 策展链 + 自报系列（" .. tostring(table.getn(kHitsV)) .. " vs " ..
      tostring(table.getn(EVAL_QC_LIST())) .. "+" .. tostring(EVAL_QC_SERIES_COUNT()) .. "）")
    -- 反向哨兵：策展链必须**够多**（用户要求「30-60 的经典任务线」——只加几条不算做完）
    local kLv30 = 0
    local kListV = EVAL_QC_LIST() or {}
    for i = 1, table.getn(kListV) do if (tonumber(kListV[i].lo) or 0) >= 30 then kLv30 = kLv30 + 1 end end
    eq(table.getn(kListV) >= 55 and kLv30 >= 25, true,
      "⑧v★★策展链已扩到 30-60 经典线（共 " .. tostring(table.getn(kListV)) .. " 条 · 其中 lo≥30 的 " .. tostring(kLv30) .. " 条）")
  end

  -- ⑧r ★1.75.11 用户要求：「装备和任务线滚动机制参考抓宠助手的滚动方式，顺滑滚动」
  --   抓宠帮手 = 滚轮一格滚 **1 行**（细调）；这里再加像素滑动动画。★整段自带 do…end（200 local 说明）
  do
    EVAL_QP_SHOW()
    EVAL_QP_SET_TAB("item")
    EVAL_QP_SET_QUERY("")
    EVAL_QP_SET_FILTER("ALL", "ALL")
    EVAL_QP_PAGE(-999)                          -- 回到第一页
    local kOff0 = EVAL_QP_OFF()
    -- 滚轮一格 = 1 行（不是整页）
    EVAL_QP_SCROLL(1)
    eq(EVAL_QP_OFF() == kOff0 + 1, true,
      "⑧r★★滚轮一格只滚 1 行（" .. tostring(kOff0) .. " → " .. tostring(EVAL_QP_OFF()) .. " · hits=" .. tostring(table.getn(EVAL_QP_HITS())) .. " · tab=" .. tostring(EVAL_QP_TAB()) .. "）")
    -- 动画：起步错位 = 一行高（19px 字面），随后缓动归零并摘钩
    local kAnimStart = EVAL_QP_ANIM()
    eq(kAnimStart >= 15 and kAnimStart <= 20, true,
      "⑧r★★滑动动画起步 ≈ 一行高（实际 " .. tostring(kAnimStart) .. "）")
    local kAnim = EVAL_QP_TEST_ANIMFRAME()
    eq(kAnim ~= nil, true, "⑧r动画帧存在")
    local kTick = (kAnim and kAnim.GetScript) and kAnim:GetScript("OnUpdate") or nil
    eq(type(kTick) == "function", true, "⑧r动画帧挂了 OnUpdate")
    local kSteps, kPrev = 0, math.abs(EVAL_QP_ANIM())
    while kSteps < 40 do
      local t = kAnim:GetScript("OnUpdate")
      if type(t) ~= "function" then break end
      t()                                      -- ★真机形态：OnUpdate 零参数
      kSteps = kSteps + 1
      local a = math.abs(EVAL_QP_ANIM())
      if a > kPrev then eq(false, true, "⑧r动画必须单调收敛（第 " .. tostring(kSteps) .. " 帧反而变大）") break end
      kPrev = a
    end
    eq(EVAL_QP_ANIM() == 0, true, "⑧r★★缓动到零（" .. tostring(kSteps) .. " 帧内收敛）")
    eq(kAnim:GetScript("OnUpdate") == nil, true, "⑧r★★归零后**摘钩**（不常驻空转）")
    -- 边界：顶部再往上滚 = 不动（也不启动动画）
    --   ★先把手上的动画跑完：整页跳**本身也会滑动**（qpAnimKick），不排空就分不清是谁的动画
    local kDrain = function()
      local fr = EVAL_QP_TEST_ANIMFRAME()
      local n = 0
      while fr and n < 60 do
        local t = fr:GetScript("OnUpdate")
        if type(t) ~= "function" then break end
        t()
        n = n + 1
      end
    ...
    eq(kDrain() == 0, true, "⑧r整页跳的滑动也能收敛到 0")
    EVAL_QP_PAGE(-999)
    eq(kDrain() == 0, true, "⑧r回首页的动画收敛")
    EVAL_QP_SCROLL(-1)
    eq(EVAL_QP_OFF() == 0 and EVAL_QP_ANIM() == 0, true,
      "⑧r到顶不再滚（off=" .. tostring(EVAL_QP_OFF()) .. " · anim=" .. tostring(EVAL_QP_ANIM()) .. "）")
    -- 任务线视图同样按 1 行滚（用户点名「装备和任务线」）
    EVAL_QP_SET_TAB("chain")
    EVAL_QP_SET_QUERY("")
    local kCOff = EVAL_QP_OFF()
    EVAL_QP_SCROLL(1)
    eq(EVAL_QP_OFF() == kCOff + 1, true, "⑧r★★任务线视图也是 1 行/格（" .. tostring(kCOff) .. " → " .. tostring(EVAL_QP_OFF()) .. "）")
    -- ★1.75.13 用户实测：「任务线最高只有 30 级 —— 是不是没收集齐？」
    --   真凶 = 列表写死 cap=200 × **等级升序** ⇒ 30 级以上的线全被截掉（数据层其实有 688 条、到 60 级）。
    --   判据三条：① 全量（> 200）② 真能翻到 50+ 的线 ③ cap 语义（0=不限 / 小 cap 如实报 extra）。（自成 do…end：200 local 上限）
    do
      local kHits = EVAL_QP_HITS() or {}
      local kAll = table.getn(kHits)
      eq(kAll > 200, true, "⑧r★★任务线列表必须全量（旧实现 cap=200 按等级截掉 30 级以上；实得 " .. tostring(kAll) .. "）")
      local kTop = 0
      for i = 1, kAll do
        local c = kHits[i] and kHits[i].c
        local hi = c and tonumber(c.hi) or 0
        if hi > kTop then kTop = hi end
      end
      eq(kTop >= 50, true, "⑧r★★列表里必须真的能翻到 50+ 的任务线（实得最高 hi=" .. tostring(kTop) .. "）")
      local kUncap = EVAL_QC_SEARCH("", 0, nil) or {}
      local kCap5, kExtra5 = EVAL_QC_SEARCH("", 5, nil)
      kCap5 = kCap5 or {}
      eq(table.getn(kUncap) > 200, true, "⑧r★cap=0 = 不限条数（实得 " .. tostring(table.getn(kUncap)) .. "）")
      eq(table.getn(kCap5) == 5 and (kExtra5 or 0) > 0, true,
        "⑧r★小 cap 如实截断并报 extra（得 " .. tostring(table.getn(kCap5)) .. " · extra=" .. tostring(kExtra5) .. "）")
    end
    -- ★1.75.14 用户问「任务线有按照任务最低等级排序吗?」——
    --   旧实现 = 「策展链自己排好」+「自报系列自己排好」**直接拼接** ⇒ 第 22/23 条之间 30 → 4 的回头
    --   （用户截图正中：策展链尾巴 27/29/30 级，紧接着自报系列 4-5 级）。（自成 do…end：200 local 上限）
    do
      local kL = EVAL_QC_SEARCH("", 0, nil) or {}
      local kN, kBad, kPrev = table.getn(kL), 0, -1
      local kFirstSeries, kLastChain = kN + 1, 0
      for i = 1, kN do
        local e = kL[i] or {}
        local lo = tonumber(e.c and e.c.lo) or 99
        if lo < kPrev then kBad = kBad + 1 end
        kPrev = lo
        if e.kind == "series" and i < kFirstSeries then kFirstSeries = i end
        if e.kind == "chain" and i > kLastChain then kLastChain = i end
      end
      eq(kN > 200 and kBad == 0, true,
        "⑧t ★★任务线列表必须**跨块**按最低等级升序（旧实现拼接 ⇒ 30→4 回头；越界 " .. tostring(kBad) .. " 处 · 共 " .. tostring(kN) .. " 条）")
      -- 反向哨兵：两类来源必须**混排**（不混排 = 又退回「两块拼接」）
      eq(kFirstSeries < kLastChain, true,
        "⑧t ★两类来源必须混排（首条系列 @" .. tostring(kFirstSeries) .. " 要早于最后一条策展链 @" .. tostring(kLastChain) .. "）")
      -- 截断必须在排序**之后**：cap=5 的头部必须与全量头部**逐条相同**（先截断后排序 ⇒ 头部是策展链 27-30 级）
      local kC5 = EVAL_QC_SEARCH("", 5, nil) or {}
      local kSame = table.getn(kC5) == 5
      for i = 1, table.getn(kC5) do
        local a, b = kC5[i] or {}, kL[i] or {}
        if tostring(a.name) ~= tostring(b.name) or tonumber(a.c and a.c.lo) ~= tonumber(b.c and b.c.lo) then kSame = false end
      end
      eq(kSame, true, "⑧t ★cap 在排序**之后**生效（前 5 条必须与全量头部逐条相同）")
    end
    -- 详情视图：长链能滚到底（旧实现只看得到第一屏）
    local kLong = nil
    local kSer = EVAL_QC_SERIES_LIST()
    for i = 1, table.getn(kSer) do
      if table.getn(kSer[i].steps) >= 10 then kLong = "s:" .. tostring(i) break end
    end
    if kLong then
      EVAL_QP_DETAIL(kLong)
      eq(EVAL_QP_MODE(), "detail", "⑧r进入长链详情")
      eq(EVAL_QP_DET_MAX() > 0, true, "⑧r★★长链详情可滚（最大偏移 " .. tostring(EVAL_QP_DET_MAX()) .. " 行）")
      eq(EVAL_QP_DET_OFF() == 0, true, "⑧r新开详情从顶部开始")
      EVAL_QP_SCROLL(1)
      eq(EVAL_QP_DET_OFF() == 1, true, "⑧r★详情滚轮也是 1 行/格")
      EVAL_QP_SCROLL(999)
      eq(EVAL_QP_DET_OFF() == EVAL_QP_DET_MAX(), true,
        "⑧r★★能滚到最后一屏（" .. tostring(EVAL_QP_DET_OFF()) .. " = " .. tostring(EVAL_QP_DET_MAX()) .. "）")
      local kDetRows = select(2, EVAL_QP_ROWS())
      local kLastFs = EVAL_QP_TEST_DET(kDetRows)
      local kLastTxt = (kLastFs and kLastFs.GetText) and select(1, kLastFs:GetText()) or ""
      eq(type(kLastTxt) == "string" and string.len(kLastTxt) > 0, true,
        "⑧r★★滚到底时最后一行有内容（不越界、不空白）")
    else
      eq(false, true, "⑧r找不到 ≥10 步的长链（数据缺失）")
    end
    EVAL_QP_LIST()
    EVAL_QP_SET_TAB("item")
    EVAL_QP_SET_QUERY("")
  ...
  -- ⑧s ★1.75.12 用户四条：① 装备→任务线信息要带**任务等级** + 左侧**黄色感叹号** ② 点放大镜不再红字
  --      ③ 点放大镜**自动打开数据检索 tab** 并检索 ④ 检索只针对**当前可见**装备、限频、不顺序全量
  --  ★整段自带 do…end（200 local 说明）
  do
    -- ② 下拉 nil 守卫（真机红字 `bad argument #1 to 'getn'` 的兜底）
    local okNil = pcall(EVAL_DD_OPEN, EVAL_QP_TEST_F1(), nil, function() end)
    eq(okNil, true, "⑧s★★ EVAL_DD_OPEN 收到 nil 条目表**不报错**（真机红字兜底）")
    local okF = pcall(DD_FILTER, nil, nil, "", false)
    eq(okF, true, "⑧s★ DD_FILTER 收到 nil 也不炸")
    if okF then
      local r0 = DD_FILTER(nil, nil, "", false)
      eq(type(r0) == "table" and table.getn(r0) == 0, true, "⑧s★ nil → 如实返回空表（不是崩）")
    end
    -- ① 任务行：带 Lv + 左侧黄色感叹号
    EVAL_QP_SHOW()
    EVAL_QP_SET_TAB("item")
    ...
    local kRow = nil
    for i = 1, 8 do
      local r = EVAL_QP_TEST_ROW(i)
      if r and type(r.item) == "number" then kRow = r.item break end
    end
    eq(type(kRow) == "number", true, "⑧s前置：拿到一件装备")
    EVAL_QP_ITEM_DETAIL(kRow)
    eq(EVAL_QP_MODE(), "detail", "⑧s进装备详情")
    local kQuestIcon, kLvRow = nil, nil
    for i = 1, 15 do
      local w = EVAL_QP_ZOOM_WORD(i)
      local fs = EVAL_QP_TEST_DET(i)
      local txt = (fs and fs.GetText) and select(1, fs:GetText()) or ""
      if type(w) == "string" and w ~= "" then
        if kQuestIcon == nil then kQuestIcon = EVAL_QP_TEST_DETICONROW(i) end
        if string.find(tostring(txt), "Lv", 1, true) then kLvRow = i end
      end
    end
    eq(kQuestIcon ~= nil, true, "⑧s★★任务行左侧有图标控件")
    eq((kQuestIcon and kQuestIcon:IsShown()) and true or false, true, "⑧s★★任务行的黄色感叹号画出来了")
    eq((kQuestIcon and kQuestIcon.GetTexture and kQuestIcon:GetTexture() or "") ~= "", true,
      "⑧s★感叹号用的是真贴图（" .. tostring(kQuestIcon and kQuestIcon:GetTexture()) .. "）")
    eq(kLvRow ~= nil, true, "⑧s★★任务行带**任务等级**（Lv）—— 用户要求")
    -- ★链步骤行（"1. 名字"）也要带等级 —— 挑一件「策展链奖励」的装备（只有它的详情里才列链步骤）
    do
      local kCur = EVAL_QC_LIST(nil)
      local kCurItem = nil
      for i = 1, table.getn(kCur) do
        local rw = (kCur[i] and kCur[i].rw) or {}
        if table.getn(rw) > 0 then kCurItem = rw[1] break end
      end
      eq(type(kCurItem) == "number", true, "⑧s前置：拿到一件策展链奖励装备")
      if type(kCurItem) == "number" then
        EVAL_QP_ITEM_DETAIL(kCurItem)
        local kStepLv = nil
        for i = 1, 15 do
          local fs = EVAL_QP_TEST_DET(i)
          local txt = (fs and fs.GetText) and select(1, fs:GetText()) or ""
          if string.find(tostring(txt), "^%d+%.") and string.find(tostring(txt), "Lv", 1, true) then kStepLv = i break end
        end
        eq(kStepLv ~= nil, true, "⑧s★★「1. 名字」步骤行也带等级（用户圈的就是这一行）")
        -- 该行同样有黄色感叹号
        local kIcon2 = EVAL_QP_TEST_DETICONROW(kStepLv or 1)
        eq((kIcon2 and kIcon2:IsShown()) and true or false, true, "⑧s★步骤行左侧也是感叹号")
      end
    end
    -- ③ 点放大镜 → 检索词送出去 + **切到数据检索 tab**
    local kZoomWord = EVAL_QP_ZOOM_WORD(kLvRow or 1)
    eq(type(kZoomWord) == "string" and kZoomWord ~= "", true, "⑧s★该行有跳转词（" .. tostring(kZoomWord) .. "）")
    local kSavedTab = EVAL_HELP_CFG_SETTAB
    local kTabCalled = nil
    if type(kSavedTab) == "function" then
      EVAL_HELP_CFG_SETTAB = function(n) kTabCalled = n return true end
    end
    local kZBtn = EVAL_QP_TEST_ZOOM(kLvRow or 1)
    local kZOn = (kZBtn and kZBtn.GetScript) and kZBtn:GetScript("OnClick") or nil
    if type(kZOn) == "function" then kZOn() end
    eq(kTabCalled == 4, true, "⑧s★★点放大镜**自动切到数据检索 tab**（第 " .. tostring(kTabCalled) .. " 个）")
    eq(EVAL_DS_LAST_QUERY(), kZoomWord, "⑧s★检索词跟着走（" .. tostring(EVAL_DS_LAST_QUERY()) .. "）")
    if type(kSavedTab) == "function" then EVAL_HELP_CFG_SETTAB = kSavedTab end
    -- ④ 只检索当前可见：队列上限 = 一页行数；全是缓存时**一件都不排**
    EVAL_QP_LIST()
    EVAL_QP_SET_TAB("item")
    EVAL_QP_SET_QUERY("")
    local kRows = select(1, EVAL_QP_ROWS())
    local kSt = EVAL_QP_REQ_STATE()
    eq(type(kSt) == "table", true, "⑧s★请求状态读值口存在")
    eq(kSt.queued <= kRows, true,
      "⑧s★★待请求数 ≤ 一页行数（" .. tostring(kSt.queued) .. " ≤ " .. tostring(kRows) .. "：不做全量顺序检索）")
    eq(kSt.rate >= 2.0, true, "⑧s★★限频 ≥ 2 秒/件（实际 " .. tostring(kSt.rate) .. "，用户要求「不要太频繁」）")
    -- 全是缓存 → 零请求（缓存生效的正面证据）
    TEST.itemInfo = TEST.itemInfo or {}
    local kSeed = {}
    for i = 1, kRows do
      local r = EVAL_QP_TEST_ROW(i)
      if r and type(r.item) == "number" then
        TEST.itemInfo[r.item] = { name = "缓存" .. tostring(i), q = 3, t = "武器", st = "剑" }
        kSeed[r.item] = true
      end
    end
    EVAL_QP_REFRESH()
    local kSt2 = EVAL_QP_REQ_STATE()
    eq(kSt2.queued == 0, true, "⑧s★★整页都在客户端缓存里 → **一件都不请求**（实际 " .. tostring(kSt2.queued) .. "）")
    eq(kSt2.visible >= 1, true, "⑧s★这一屏真的画了装备（" .. tostring(kSt2.visible) .. " 件）")
    for id in pairs(kSeed) do TEST.itemInfo[id] = nil end
  end

  -- ⑧w ★1.75.16 用户实测「任务线当前页**不再检索装备**了」——根因不在「队列装了什么」，
  --   而在**泵有没有真的把队列抽走**：旧判据只断言「队列 ≤ 一页」「限频 ≥2s」，
  --   泵即使彻底停摆这些判据照样全绿 ⇒ 这里直接驱动**真泵体**，用桩时间验限频与诚实失败。
  --   ★本段自带 do…end（200 local 说明）
  do
    local kKeepTime = TEST.time
    TEST.time = 100000
    EVAL_QP_SHOW()
    EVAL_QP_SET_TAB("item")
    EVAL_QP_SET_QUERY("")
    EVAL_QP_SET_FILTER("ALL", "ALL")
    EVAL_QP_PAGE(-999)
    local kS0 = EVAL_QP_REQ_STATE()
    eq(kS0.queued >= 1, true, "⑧w前置：这一屏有未缓存装备待请求（" .. tostring(kS0.queued) .. " 件）")
    -- ① 时间够（≥ 限频）→ 抽走一件、req +1（**泵真的活着**），而且真的拿物品链接去要了
    TEST.wttHyperN = 0
    TEST.time = 100000 + 10
    EVAL_QP_TEST_PUMP_TICK()
    local kS1 = EVAL_QP_REQ_STATE()
    eq(kS1.req == kS0.req + 1 and kS1.queued == kS0.queued - 1, true,
      "⑧w★★★泵真的会抽队列（req " .. tostring(kS0.req) .. "→" .. tostring(kS1.req) ..
      " · 队列 " .. tostring(kS0.queued) .. "→" .. tostring(kS1.queued) .. "）")
    local kLink = (TEST.wttHyper or {})[1] or ""
    eq(string.find(kLink, "item:", 1, true) == 1, true,
      "⑧w★★★请求真的走了物品链接（隐形 tooltip SetHyperlink：" .. tostring(kLink) .. "）")
    -- ② 时间不够 → 一件都不抽（限频真的生效，用户要求「不要太频繁」）
    TEST.time = TEST.time + 0.5
    EVAL_QP_TEST_PUMP_TICK()
    local kS2 = EVAL_QP_REQ_STATE()
    eq(kS2.req == kS1.req, true, "⑧w★★0.5 秒后再 tick **不抽**（限频 " .. tostring(kS2.rate) .. "s/件）")
    -- ③ 拿不到工具柄 → **不消费队列**（绝不静默丢件）+ 如实记 blocked
    local kSavedHandle = EVAL_WTT_HANDLE
    EVAL_WTT_HANDLE = function() return nil end
    TEST.time = TEST.time + 10
    local kQ0 = EVAL_QP_REQ_STATE()
    EVAL_QP_TEST_PUMP_TICK()
    local kS3 = EVAL_QP_REQ_STATE()
    eq(kS3.queued == kQ0.queued and kS3.blocked == kQ0.blocked + 1, true,
      "⑧w★★★拿不到工具柄时**不丢队列**（队列 " .. tostring(kQ0.queued) .. "→" .. tostring(kS3.queued) ..
      " · blocked " .. tostring(kQ0.blocked) .. "→" .. tostring(kS3.blocked) .. "）")
    if kSavedHandle ~= nil then EVAL_WTT_HANDLE = kSavedHandle end
    TEST.time = kKeepTime
  end
...
do
  ...
  -- ③ 排序（★1.75.10 需求反转，用户要求「任务线也按等级排序」）：
  --   旧口径 = 档位优先（S→A→B，榜首必为 S）；新口径 = **等级 → 稀有度 → 原序**。
  local kOrder, kPrevLo, kPrevR, kOrderOK = true, nil, nil, true
  local kRarW = { LEG = 1, EPIC = 2, RARE = 3, UNCOMMON = 4, COMMON = 5, POOR = 6 }
  for i = 1, table.getn(kList) do
    local lo = tonumber(kList[i].lo) or 99
    local rw = kRarW[EVAL_QC_RARITY(kList[i])] or 9
    if kPrevLo then
      if lo < kPrevLo then kOrderOK = false end
      if lo == kPrevLo and kPrevR and rw < kPrevR then kOrderOK = false end
    end
    kPrevLo, kPrevR = lo, rw
  end
  eq(kOrderOK, true, "③★★链按等级排序（同级内部按稀有度：传说在前）")
  -- 榜首 = 全表里等级最低的那条（独立算一遍最小值比对，不用生产排序自证）
  local kMin = 999
  for i = 1, table.getn(kList) do
    local lo = tonumber(kList[i].lo) or 99
    if lo < kMin then kMin = lo end
  end
  eq(tonumber(kList[1] and kList[1].lo) == kMin, true,
    "③★榜首是**最低等级**的链（榜首 lo=" .. tostring(kList[1] and kList[1].lo) .. " · 全表最小 " .. tostring(kMin) .. "）")
  ...
  -- ★1.75.14 排序改成「跨块按最低等级」后，**首个命中的未必是策展链**（可能是含该步骤的自动任务线）
  --   ⇒ 判据改锚在**命中的名字**（名字里必须出现关键词），不再钉死 kind（旧判据会被排序变化打歪）。
  local kS2ok = false
  if s2 and s2[1] then
    local kHay = tostring(s2[1].name or "") .. "|" .. tostring(s2[1].c and s2[1].c.n or "")
    if string.find(kHay, "尖牙德鲁伊", 1, true) ~= nil then kS2ok = true end
  end
  eq(kS2ok, true, "⑤按任务名搜到（尖牙德鲁伊）")
  local s3 = EVAL_QC_SEARCH("西部荒野法杖")
  eq(s3 and s3[1] and s3[1].kind, "item", "⑤按奖励物品名搜到且 kind=item")
  -- ★1.75.15 策展链扩到 62 条（30-60 经典线）后，**cap=500 会截掉高等级那批**（按等级排序 ⇒ 60 级的排在最后）
  --   ⇒ 这里必须用 cap=0（不限）才谈得上「策展链一条不少」。
  local s4 = EVAL_QC_SEARCH("", 0)
