...
- **当前版本（源码唯一真值）**：`EvalHelp.lua` 的 `local VERSION` == `EvalHelp.toc` 的 `## Version` = **1.75.5**（v1.75.4 已发布）
...
1. **1.75.5** — 🗺 **探索层原值只在会话内存 · 关图即清空 · 每次开图重新读并重新缩**（用户提问 + 用户要求三连定案）：
  ★① **定位信息不落存档**（用户提问：「这个探索层每个地区的定位是从缓存读取的吗?理论上每次打开地图都要重新计算的，不需要缓存区记录这个定位信息?」）⇒ 记录只活在 `SMFIT.recMap[地图身份]`（会话内存，切走存回/切回取出）；老键 `mapFitOrig`/`mapFitVer` **一次性清掉、不再写版本戳**（判据 =「老键还在就丢」，天然幂等）。
  ★② **关图即清空 + 开图重新获取并重新缩放**（用户原话：「每次地图关闭都把原值数据清理,不要保存这个值.每次打开地图.都重启获取原值.重新缩放大地图」）：清空放**关图那一刻**（`smFitDropOnClose`，地图不可见、不闪、不跟客户端版式计算抢）；**顺序铁律 = 先还回自然档、再清空**，且还回要**按本档口径严格自验**（`|读回−原值| ≤ 0.75`）—— 两种读回口径在数值上一样，只信 `smFitRestore` 的宽松判定会放过「客户端把我们的写盖掉」⇒ **验不过绝不清理**（保留记录 + 如实播报）。真值 `SM_CFG.mapFitFresh`（**nil = 开**；`/ehm mapfit fresh on|off` 回退）。
  ★③ 修「**/reload 时原值为 0 也被当成原值抓取完成**」（用户报障：「这个流程本身就不对」）：`todo` 空两来法必须分开 —— 「目标都已有记录」才落闩；「**候选一个都不在用**（客户端还没摆版式/贴图）」= **一条都没读到 ⇒ 绝不落闩**，交 **1 秒一次的有界重试**（真机实证随后自动补抓 8 条）。窗口收口措辞**必须与实际条数挂钩**（0 条时改说「还没读到」）。
  ★④ 抓原值**不再每帧重跑**、地图缩放不再抖（1.75.4 事故收口）：先筛候选再动缩放 · 空候选当场落闩 · 瞬态 `es=1.000` 不折算 · 折算读回**两种口径都认**；新增**取原值窗口**（本图无原值 ⇒ 先不缩放、自然档读一次，有界 2s；★不折算的档**不许**开这个窗口）。
  ★⑤ 修「**探索层贴图跑到游戏画面上**」：写锚点**必须先解析成帧对象**（本客户端对「字符串 / nil 相对帧」= **锚到屏幕**，纹理就逃出地图窗口画在世界上；`smFitRestore` 一直是解析成对象的 ⇒ 这正是「关功能能救回来」的真因）。
  ★⑥ 新增手动适配命令 **`/ehm fitnow`**（别名 适配/修正）：开图（`ShowUIPanel`，不用 Toggle）→ 自然档 1.00 → **等满 2s** → 逐个打印原值 → 套缩放并打印「设置值/自身 GetScale/父链连乘」→ 折算并逐条打印「原值→写入」；期间 `smFixActive` 让自动逻辑**整段让位**、跑完（**出错也**）必放掉。播报分**安静档（默认：每次开图 2 行、关图 1 行）+ 详细档**（`/ehm mapfit verbose on`，不跨会话；真出问题仍出声）。
  ★判据 = 组 253 + **组 254/255/256** + `SM MAP KEY CHECK` ⑯⑰⑱。★按用户要求（「不要跑测试」）本轮 **test_engine/check.js 未跑**，新判据未跑验证。
2. **1.75.4** — 🔔 **稀有转播=通知（不受总闸门）+ 探索层原值带地图身份**：稀有提醒转播改走**不门控的强制出口** `EVAL_SAY_FORCE` —— 它是**通知**不是调试日志（真因：`cfg.log.on=false` 把 `say`/`logLine` 一起静音 ⇒ 用户报「检测不到稀有通知」，**日志环里也查不到任何证据**，两个证据同时消失）；受**自己的**开关 `cfg.rareWatch` 管，★总闸门语义一个字不改（组 252① 有反向哨兵）。
...
3. **1.75.3** — 🎯 **选取目标一族 + 修「自动攻击抢目标」+ 围攻·交战人数 + 任务线数据补完**：「选取目标 → **玩家的目标**」= `AssistByName`（与「指定名称」同族：同一份 `TARGET_SEL`/同名下拉/同输入弹窗，**只差客户端函数**；官方只承诺「附近」⇒ 用 `UnitExists` 兜底**如实失败**）+ 条件「**目标玩家:X**」「**目标死亡**」（★五处注册各有判据：类型表/分组/三语/求值/文本往返）。
...
4. **1.75.2** — 🎯 **追踪类型条件 + 「调试日志」= 聊天输出总闸门 + 缩放大地图探索层折算收口**：条件「自身状态 → **追踪类型**」（下拉单选 · `TRK.hint` **13 条实证**候选表 · 纹理→种类**全等比对** · 贵调用只在追踪变化时做一次 · 导出走稳定 id、界面走本地化标签），由 `TRACK ICON CHECK` 钉在本机图标清单上。
...
5. **1.75.1** — 🔀 **两支合流 + 图层拖拽宽高只给聊天窗 + 宠物动作条**：`probe/map-scale`（框拖拽 / 图层工具 / 缩放大地图）与任务线两支**合流到 master**（5 处冲突逐条审过、3 个合并副作用修净：OnUpdate 真零形参 · `EVAL_HH_PETDECOR` 补 `hhHasPet()` 门 · 参考卷打包脚本补回 quest/ 三行），并立新铁律 **`TEST LOAD ORDER CHECK`**（harness 里 `test_assert.lua` 必须排在所有 `tools/*.lua` 之后 —— 工具模块在**载入期**自登记 `EVAL_TB_MOD_ROWS`，顺序反了组 200 会「模块完全正常却 got=false」）。
...
6. **1.75.0** — 🧭 **数据检索新增「任务线」+ 地图标注右侧的「任务线推荐」弹窗**（`quest/` 独立目录）：**最终口径（数字一律从数据现算）** —— 策展经典链 **62** 条 + 自动任务线 **611** 条（**界面合计 673** 条 · ≥10 步 18 · 开门/史诗 17 · 最长 24 步）+ 全量 **751** 条带装备奖励任务 / **1262** 件物品（装备视图列出 **990** 件），覆盖 **4-60 级**，逐条抓自 `database.emberveil.org`（★首版 22 链 / 88 任务 / 62 奖励已被 1.75.9 全量采集取代）；**奖励优先**（精良武器排最前）+ **完整上下级步骤**；奖励行悬停出原生物品详情、步骤行点击进任务详情；抓取脚本（`fetch.js`/`chains.js`/`build.js`）**与插件零耦合**；数据自带 ⇒ **UnrealQuest 缺席照样能查**；★**弹窗 v2**（数据检索 · 地图标注右侧）：**装备优先**（按可获得等级列装备 + 真实物品图标 → 点装备**反查来源任务线**与完整步骤 → **[在数据检索中查询]** 直连 `EVAL_DS_SEARCH_NAME`）+ 两个视图 Tab（装备/任务线）+ 9 行分页列表 + ▲▼/滚轮 + 四档筛选（等级/类型/档位/阵营）+ 拖动柄；默认关、OnHide 收下拉。
...
7. **1.74.30** — ⏱️ **射击计时 + 条件「距下次射击」**：探针真机定案（法术频道含「自动射击」的原文 = 锚点、`UnitRangedDamage[1]` = 射速 2.09s，与近战**双锚点互不干扰**）；数值条件 `shotLeft`（文本 `距射击<0.5`、初始值 0）；修「切换条件类型把 30.0 带进时间型」；消耗品助手左键不再弹窗。
8. **1.74.29** — 🖼️ 战斗UI **激活方案格品阶色边框** + 两个助手弹窗候选**原生物品详情** + 喂食助手 `GetPetHappiness()` **快乐度边框**（开心亮绿 / 一般金 / 不开心红 / 无宠物默认金）。
9. **1.74.28** — 🧺 两个助手弹窗候选**按物品类型过滤**（品质 → `GetItemInfo(链接)` → **自建 tooltip 兜底并自愈客户端缓存**；药草矿物材料等全剔，判不出不剔）+ 验证命令。
10. **1.74.27** — 🧹 稀有提醒转播**提取为独立模块** `tools/RareWatch.lua`（主程序只留两个接线点）+ **工具箱开关**（打包基准 69）。
...
> ★★**后续（1.75.5）已把这里那份「存档按地图分桶」整个去掉**：定位信息改为**只存会话内存**（`SMFIT.recMap`）——
> 原因与全案见 **R20d**（用户提问定案：「不需要缓存区记录这个定位信息?」）。下面保留 1.75.13 当时的实现原文作为历史。
...
### R20d. 「抓原值每帧重跑 ⇒ 地图缩放每帧抖」+ **去掉持久化缓存**（1.75.5）· 压缩版
- **用户报障**（1.75.4 发布后）：「我换了一个账号之后 之前账号正常的那个用户现在地图缩放不正常了.看看日志.是否缩放信息持久化有问题?」＋补充观察「**其他几个账号、其他地图都正常**」；随后提问定案：「这个探索层每个地区的定位是从缓存读取的吗? 理论上每次打开地图都要重新计算的. 不需要缓存区记录这个定位信息?」
- ★★★**真机证据**（`LIHAIBOAS2` 15:28 那份存档）：① `mapFitTrace` **60 条环在 1 秒内被刷满**（3920753.09 → 3920754.08），全是同一条「抓原值：本图不用的 1 个叠加层已跳过」⇒ `smFitCapture()` 被**每帧**调用（≈55–60 次/秒）；② 而它内部会把世界地图缩放**临时 `SetScale(1)` 再放回** ⇒ **地图缩放每帧抖一次**（面板 0.70、地图看着没缩放 = 用户说的「缩放不正常」）；③ 折算口径被搅成 `es=1.000`（设置值 0.7）⇒ 8 个叠加层被反复重写（聊天每 3 秒一条「叠加层适配」刷屏）；④ `mapFitOrig["DunMorogh:256x256"]` 里 8 条**互不相同**（Overlay6 200×185@314,-311 · Overlay8 240×185@155,-403 …，与客户端补丁数据对得上）⇒ **记录本身是对的**，问题在「记录已齐」时的代码路径。
- ★★★**根因**（1.75.4 引入）：`smFitCapture` 只在 `n > 0`（真抓到新记录）时落闩；**存档里原值已齐**时 `n = 0` ⇒ `captured` 永远 false ⇒ tick 门 `if (not captured) and mapAge >= 0.4` **每帧**放行。**触发条件精确到「该图已有完整原值」= 同一张图第二次打开 / `/reload` 后再开** —— 这正好解释了用户的观察：其他账号/其他地图**还没记过** ⇒ `n > 0` ⇒ 落闩 ⇒ 正常；「之前正常的号」上一版会话已经把那张图记进去了 ⇒ 这次登录命中。
- ★★★**修法四条**：① **先筛候选、再动缩放**（`todo` 列表排在 `SetScale(1)` **之前**：没候选就连读都不读）；② **候选为空 ⇒ 当场落闩**（`SMFIT.captured = true` 后 return，一个缩放都不碰）；③ 落闩判据 = 「本轮目标**全部**拿到」（`n >= table.getn(todo)`；有目标读不出来 ⇒ 不落闩，但由 tick 的**有界重试** `SMFIT_CAP_GAP = 1s` 兜，绝不每帧）；④ 另两条硬化：**瞬态 `es=1.000` 而设置值≠1 ⇒ 暂缓折算**（否则同一次开图会先写一遍未折几何）+ 折算的幂等判据**两种读回口径都认**（`okA` 读回=逻辑 / `okB` 读回=逻辑×es；旧写法只认前者 ⇒ 在会级联的本客户端上**每一拍都判「要改」**并重写）。
- ★★★**架构简化（用户提问定案）**：**定位信息不落存档**。理由三条：① 客户端**每次开图都会自己重摆**这批纹理 ⇒ 开图那一刻的活值就是原值；② 存盘只会带来脏值（旧 schema 按名字记一份 = 跨图污染 = R20c；版本戳迁移；以及本条这场「存档已齐 ⇒ 空转」）；③ 唯一必须记住的是**本会话内按图**那一份。落地 = **`SMFIT.recMap[地图身份]`**（切走存回、切回取出 —— 不记的话切回时会把**折后值当原值再折一遍**，即 R20 的「缩两遍」）；老存档 `mapFitOrig`/`mapFitVer` **一次性清掉**、**不再写版本戳**（判据 = 「老键还在就丢」，天然幂等）；`smFitApply` 不再有「从存档采纳」那条路 ⇒ **唯一写入点是 `smFitCapture`**。
- ★★**判据** = **组 254**（一次抓齐即落闩 · 30 拍零 `SetScale` · 瞬态 es 不折算 · 读回含缩放**写过之后不再重写** · 读不出来时有界重试 ≤3 次/1.5s · 空候选零缩放写 · `/reload` 后重抓且**一个字节都不落存档**）+ **`SM MAP KEY CHECK`** 新段（顺序/落闩/有界重试/瞬态/两种口径/**不落存档 + 老键清理**）与 `SM FIT CASCADE CHECK` 的改写（apply 不许新建记录、唯一写入点是 capture）；组 253⑤b 改为 `EVAL_SM_TEST_MAPFIT_PLANT` 种记录（= 旧的「存档里种脏记录」的等价物）。模块组号清单 `SM GROUP ROSTER CHECK` 的 `WANT` 加 **254**。
- ★★★**变异 5/5**，其中两条是**当场漏网 → 补判据**（教训比代码值钱）：**M1**（空候选不落闩）：第一版漏网 —— 夹具是**同会话**抓齐的，走的是 `n >= todo` 那条落闩路，压根没进「空候选」分支 ⇒ 补「**切回本会话已记过的图**」这条真场景；**M4**（删掉 `okB`）：第一版漏网 —— **没写过**时两种口径算出的是同一个数（`live = 逻辑×es = 原值×es = want`）⇒ 补「先把逻辑值改成 999 逼出一次折算，再验第二拍 `changed == 0`」。★另记一个**正则回溯陷阱**：`SM_CFG.mapFitVer\s*=\s*(?!nil)` 会被 `~= nil` 里的 `=` 命中（`\s*` 回溯）⇒ 改成「先把合法 `= nil` 抹掉，再看还有没有赋值」。
- ⏳**仍未闭环**：R20 的定案是「本客户端**自己会**把父帧缩放级联到这批纹理上」⇒ 默认档 `fold` 就是**第二遍**；但 1.75.9 那次把默认改成「听探测」后用户立刻报「正常开启都不生效」。本轮**不动默认档**（只把折算做稳 + 两种读回口径都认），真机上用 `/ehm mapfit dump` + `/ehm mapfit mode nofold|fold` 各看一次「探索层与底图对不对得齐」即可定档。
