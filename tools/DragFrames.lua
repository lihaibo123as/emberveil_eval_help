...
local function dfRosterOnUpdate(a1)
  if DF.rosterLeft <= 0 then return end
  local dt = dfNum(a1, nil)
  if not dt then dt = dfNum(arg1, 0.05) end
...
local function dfKeepOnUpdate(a1)
  if DF.keepDone then return end
  local dt = dfNum(a1, nil)
  if not dt then dt = dfNum(arg1, 0.05) end
  if (not dt) or dt < 0 then dt = 0.05 end
...
local function dfBindDragButton(b)
  ...
  b:SetScript("OnUpdate", function(a1)
